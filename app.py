...
import dash
from dash import html as dash_html, dcc
...
def verificar_e_incrementar_uso(incrementar=True):
    ...
    try:
        if not os.path.exists(config.USAGE_COUNT_FILE):
            with open(config.USAGE_COUNT_FILE, 'w') as f: f.write('0')
        ...
        # Incrementa apenas se solicitado
        if incrementar:
            uso_atual += 1
...
try:
    ...
    print(f"!!!!!! [app.py] ERRO CRÍTICO AO CRIAR LAYOUT !!!!!!")
