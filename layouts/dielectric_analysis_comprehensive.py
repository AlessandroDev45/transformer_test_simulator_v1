...
def create_dielectric_comprehensive_layout():
    ...
    try:
        ...
        if not verificador_instance.is_valid():
            log.warning("VerificadorTransformador is invalid upon creation in create_dielectric_comprehensive_layout.")
        else:
            log.info("VerificadorTransformador instantiated successfully for comprehensive analysis.")
    except ImportError:
        log.error("Failed to import VerificadorTransformador in create_dielectric_comprehensive_layout.")
    except Exception as e:
        log.critical(f"CRITICAL error instantiating VerificadorTransformador: {e}", exc_info=True)
        return dbc.Alert(f"Erro crítico ao carregar dados das normas: {e}. Verifique a configuração e os arquivos.",
                         color="danger", style=COMPONENTS['alert'])
    ...
    comprehensive_layout = html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardHeader(
                        html.H5("Parâmetros Selecionados", className="text-center m-0"),
                        style={**COMPONENTS['card_header'], "backgroundColor": COLORS['primary']}
                    ),
                    dbc.CardBody([
                        html.Div(id="selected-params-display", className="px-2") # Conteúdo preenchido por callback
                    ], style={**COMPONENTS['card_body'], "padding": "0.75rem"})
                ], style={**COMPONENTS['card'], "height": "100%", "boxShadow": "0 3px 5px rgba(0,0,0,0.2)"})
            ], width=9, className=SPACING['col_padding']),

            # Coluna 2 (1/4) - Título e Descrição (como antes)
            dbc.Col([
                dbc.Card([
                    dbc.CardHeader(
                        html.H5("Análise Dielétrica Completa", className="text-center m-0"),
                        style={**COMPONENTS['card_header'], "backgroundColor": COLORS['info']}
                    ),
                    dbc.CardBody([
                        html.P(
                        ...
                        html.Div([
                            dbc.Button(
                                [html.I(className="fas fa-search-plus me-1"), "Analisar Detalhes"],
                                id="analisar-detalhes-button",
                                color="primary",
                                className="w-100 mb-2",
                                style={**TYPOGRAPHY['button'], "fontSize": "0.85rem", "padding": "0.5rem 1rem"}
                            ),
                            # Botão para forçar carregamento (ajustar se necessário)
                            dbc.Button(
                                [html.I(className="fas fa-sync-alt me-1"), "Forçar Carregamento Dados"],
                            ...
                            dbc.Button(
                                [html.I(className="fas fa-arrow-left me-1"), "Voltar"],
                    ...
                    ], style={**COMPONENTS['card_body'], "height": "100%", "display": "flex", "flexDirection": "column", "justifyContent": "space-between"})
                ], style={**COMPONENTS['card'], "height": "100%", "boxShadow": "0 3px 5px rgba(0,0,0,0.2)"})
            ], width=3, className=SPACING['col_padding'])
        ], className=SPACING['row_margin']),
        ...
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    dbc.CardHeader(html.H5("Análise AT", className="text-center m-0"), style={**COMPONENTS['card_header'], "backgroundColor": COLORS['primary']}),
                    dbc.CardBody([dcc.Loading(html.Div(id="comparison-output-at"), type="circle", color=COLORS['primary'])], style={**COMPONENTS['card_body'], "padding": "0.75rem", "maxHeight": "calc(100vh - 350px)", "overflowY": "auto"})
                ], style={**COMPONENTS['card'], "height": "100%", "boxShadow": "0 3px 5px rgba(0,0,0,0.2)"})
            ], md=4, className=SPACING['col_padding']),
            dbc.Col([
                dbc.Card([
                    dbc.CardHeader(html.H5("Análise BT", className="text-center m-0"), style={**COMPONENTS['card_header'], "backgroundColor": COLORS['secondary']}),
                    dbc.CardBody([dcc.Loading(html.Div(id="comparison-output-bt"), type="circle", color=COLORS['secondary'])], style={**COMPONENTS['card_body'], "padding": "0.75rem", "maxHeight": "calc(100vh - 350px)", "overflowY": "auto"})
                ], style={**COMPONENTS['card'], "height": "100%", "boxShadow": "0 3px 5px rgba(0,0,0,0.2)"})
            ], md=4, className=SPACING['col_padding']),
            dbc.Col([
                dbc.Card([
                    dbc.CardHeader(html.H5("Análise Terciário", className="text-center m-0"), style={**COMPONENTS['card_header'], "backgroundColor": COLORS['info']}),
                    dbc.CardBody([dcc.Loading(html.Div(id="comparison-output-terciario"), type="circle", color=COLORS['info'])], style={**COMPONENTS['card_body'], "padding": "0.75rem", "maxHeight": "calc(100vh - 350px)", "overflowY": "auto"})
                ], style={**COMPONENTS['card'], "height": "100%", "boxShadow": "0 3px 5px rgba(0,0,0,0.2)"})
            ], md=4, className=SPACING['col_padding'])
        ], className=SPACING['row_margin']),

        # Área de Mensagens/Alertas (como antes)
        html.Div(id='alert-container-comprehensive', style={"position": "fixed", "bottom": "20px", "right": "20px", "zIndex": "1000", "maxWidth": "400px"})
    ])
...
def create_sequence_graph(sequence_data, title):
    ...
    fig.add_trace(go.Scatter(
        x=x_values, y=y_values, mode='lines', name='Sequência',
        line=dict(color=COLORS['primary'], width=2, shape='hv'),
        text=text_values, hoverinfo='text'
    ))
    if dp_markers_x:
        fig.add_trace(go.Scatter(
            x=dp_markers_x, y=dp_markers_y, mode='markers', name='Medição DP',
            marker=dict(symbol='star', size=10, color=COLORS['warning'], line=dict(width=1, color=COLORS['text_light'])),
            hoverinfo='text', text=['Medição de DP' for _ in dp_markers_x]
        ))
    fig.update_layout(
        title=title, xaxis_title='Tempo (s)', yaxis_title='Tensão (kV / %)',
        template='plotly_dark', plot_bgcolor=COLORS['background_card'], paper_bgcolor=COLORS['background_card'],
        font=dict(color=COLORS['text_light']), margin=dict(l=40, r=40, t=50, b=40),
