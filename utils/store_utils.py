...
def prepare_data_for_store(data: Any, store_id: str = None) -> Any:
    ...
    try:
        ...
        if is_json_serializable(converted_data):
            ...
            log.error(f"[STORE UTILS] Failed to convert data for {store_name} to serializable format")
...
def update_store_data(current_data: Dict[str, Any], new_data: Dict[str, Any], store_id: str = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not isinstance(current_data, dict):
            log.warning(f"[STORE UTILS] Current data for {store_name} is not a dict, creating new dict")
            result = {}
        else:
