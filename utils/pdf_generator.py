...
from reportlab.platypus import (
    SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer, PageBreak, KeepTogether, Flowable
)
...
class HRFlowable(Flowable):
    ...
    def __init__(self, width, thickness=0.5, color=reportlab_colors.grey, spaceBefore=2, spaceAfter=2):
...
def create_pdf_table(table_data, col_widths=None, title="Tabela", styles=None, doc_width=None):
    ...
    if isinstance(table_data, dict):
        ...
        for k, v in sorted_items:
            ...
            val_p = create_styled_paragraph(v, table_cell_style_right) # Assume values are right-aligned
            formatted_table_data.append([key_p, val_p])

    # Handle List of Lists (assuming first row is header)
    elif isinstance(table_data, list) and table_data and all(isinstance(row, list) for row in table_data):
        ...
        for row_idx, row in enumerate(table_data[1:]):
            if len(row) == num_cols:
                ...
                formatted_row.extend([create_styled_paragraph(cell, table_cell_style_right) for cell in row[1:]])
                formatted_table_data.append(formatted_row)
            else:
                log.warning(f"Linha {row_idx+1} com {len(row)} colunas (esperado {num_cols}) ignorada na tabela '{title}': {row}")
    ...
    if col_widths:
        if len(col_widths) != num_cols:
            log.warning(f"Número de col_widths ({len(col_widths)}) != colunas ({num_cols}) para '{title}'. Usando distribuição padrão.")
            col_widths = None # Force recalculation
        elif sum(col_widths) > doc_width + 1: # Allow small tolerance
             log.warning(f"Soma de col_widths ({sum(col_widths)}) excede largura da página ({doc_width}) para '{title}'. Usando distribuição padrão.")
             col_widths = None
    ...
    table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), reportlab_colors.HexColor(config.colors.get('reportlab_header_bg', '#26427A'))),
        ('TEXTCOLOR', (0, 0), (-1, 0), reportlab_colors.white), # Use white directly
        ('ALIGN', (0, 0), (-1, 0), 'CENTER'),
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, 0), 9),
        ('BOTTOMPADDING', (0, 0), (-1, 0), 4),
        ('BACKGROUND', (0, 1), (-1, -1), reportlab_colors.HexColor(config.colors.get('reportlab_cell_bg', '#F8F9FA'))),
        ...
        ('GRID', (0, 0), (-1, -1), 0.5, reportlab_colors.HexColor(config.colors.get('reportlab_grid', '#ADB5BD'))),
...
def generate_pdf(report_data_formatted: dict, buffer: BytesIO):
    ...
    if not isinstance(report_data_formatted, dict):
        ...
        except Exception: pass # Ignora se nem isso funcionar
        return

    try:
        doc = SimpleDocTemplate(buffer, pagesize=portrait(letter),
                                rightMargin=0.5*inch, leftMargin=0.5*inch,
                                topMargin=0.75*inch, bottomMargin=1.0*inch) # Margem inferior maior para footer
        elements = []
        ...
        title_style = ParagraphStyle(
            name='ReportTitle', parent=styles['h1'], alignment=TA_CENTER,
            spaceAfter=12, fontSize=16,
            textColor=reportlab_colors.HexColor(config.colors.get('reportlab_primary', '#26427A'))
        )
        section_title_style = ParagraphStyle(
            name='SectionTitle', parent=styles['h2'], alignment=TA_LEFT,
            spaceBefore=10, spaceAfter=4, fontSize=12, leading=14,
            textColor=reportlab_colors.HexColor(config.colors.get('reportlab_primary', '#26427A')),
            borderPadding=2
        )
        subsection_title_style = ParagraphStyle(
            name='SubsectionTitle', parent=styles['h3'], alignment=TA_LEFT,
            spaceBefore=8, spaceAfter=3, fontSize=10, leading=12,
            textColor=reportlab_colors.HexColor(config.colors.get('reportlab_secondary', '#495057')),
            leftIndent=10 # Indenta subseções
        )
        normal_style = ParagraphStyle(
            name='Normal', parent=styles['Normal'], fontSize=9, leading=11,
        ...
        pdf_styles = {
            'TableHeader': ParagraphStyle(name='TableHeader', parent=normal_style, fontName='Helvetica-Bold', alignment=TA_CENTER, textColor=reportlab_colors.white, fontSize=9),
            'TableCellLeft': ParagraphStyle(name='TableCellLeft', parent=normal_style, alignment=TA_LEFT, fontSize=8),
            'TableCellRight': ParagraphStyle(name='TableCellRight', parent=normal_style, alignment=TA_RIGHT, fontSize=8)
        }
        hr_line = HRFlowable(width=doc_width, thickness=0.5, color=reportlab_colors.HexColor(config.colors.get('reportlab_grid', '#ADB5BD')), spaceBefore=1, spaceAfter=5)
        ...
        def add_report_content(content_dict, level=1):
            ...
            current_title_style = title_style_map.get(level, subsection_title_style) # Default a subseção
            ...
            for title, content in content_dict.items():
                ...
                if isinstance(content, dict):
                    ...
                    # Se for um dicionário simples (Param:Val), cria uma tabela
                         ...
                         table = create_pdf_table(content, title=title, styles=pdf_styles, doc_width=doc_width)
                         if table:
                ...
                # Se for uma string (texto simples)
                     ...
                     log.warning(f"Tipo de conteúdo não suportado em add_report_content para '{title}': {type(content)}")

            # Retorna elementos para o nível atual (para chamadas recursivas)
            return section_elements

        # --- Construir Documento PDF ---
        elements.append(create_styled_paragraph("Relatório de Simulação de Ensaios de Transformador", title_style))
        ...
        for section_title in section_order:
             ...
             if section_data:
                 ...
                 if section_title in ['Análise Dielétrica', 'Tensão Aplicada', 'Tensão Induzida', 'Resultados de Impulso'] and not dielectric_tests_started:
                     elements.append(PageBreak())
                     elements.append(create_styled_paragraph("Resultados dos Ensaios Dielétricos", title_style))
                     elements.append(Spacer(1, 6*mm))
                 ...
                 section_content_elements = add_report_content({section_title: section_data}, level=1)
                 ...
                 log.info(f"Seção '{section_title}' não encontrada ou vazia nos dados formatados. Pulando.")
        ...
        try:
            ...
            doc_err = SimpleDocTemplate(buffer, pagesize=portrait(letter))
            elements_err = [Paragraph(f"ERRO AO GERAR PDF: {html.escape(str(e))}<br/><br/>Verifique os logs do servidor para detalhes.")]
            doc_err.build(elements_err)
