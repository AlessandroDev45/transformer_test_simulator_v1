...
def standard_transformer_info_callback(transformer_data, module_name, function_name, app_instance):
    ...
    if not check_store_exists(transformer_data, "transformer-inputs-store"):
        ...
        if mcp_available:
            ...
            if not check_store_exists(transformer_data, "transformer-inputs-store"):
                ...
                return html.Div("Dados do transformador não disponíveis", className="alert alert-warning")
        else:
            return html.Div("Dados do transformador não disponíveis", className="alert alert-warning")
    ...
    try:
        ...
        return html.Div(f"Erro ao criar painel de informações: {str(e)}", className="alert alert-danger")

def standard_store_update_callback(store_data, new_data, module_name, function_name, app_instance, store_id):
    """
    ...
    try:
        ...
        log_detailed(
            ...
            data={
                'execution_time_ms': round(execution_time * 1000, 2),
                'updated_store_keys': list(updated_store.keys()) if isinstance(updated_store, dict) else None
            }
        )
...
def standard_load_from_store_callback(store_data, module_name, function_name, app_instance, store_id, pathname=None, expected_pathname=None):
    """
