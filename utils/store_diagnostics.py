...
def convert_numpy_types(obj: Any, debug_path: str = "") -> Any:
    ...
    if isinstance(obj, dict):
        ...
        return {k: convert_numpy_types(v, f"{debug_path}.{k}" if debug_path else k) for k, v in obj.items()}
    elif isinstance(obj, list):
        debug_log(f"Convertendo lista com {len(obj)} itens")
        return [convert_numpy_types(item, f"{debug_path}[{i}]" if debug_path else f"[{i}]")
                for i, item in enumerate(obj)]
    elif isinstance(obj, tuple):
        debug_log(f"Convertendo tupla com {len(obj)} itens")
        return tuple(convert_numpy_types(item, f"{debug_path}[{i}]" if debug_path else f"[{i}]")
                     for i, item in enumerate(obj))
    ...
    # Tipos numpy básicos
        ...
        try:
            ...
            try:
                result = []
                for i, item in enumerate(obj):
                    result.append(convert_numpy_types(item, f"{debug_path}[{i}]" if debug_path else f"[{i}]"))
                return result
    ...
    # Tipos datetime
        ...
        debug_log(f"Convertendo datetime -> string ISO")
    ...
    # Objetos com métodos de conversão
        ...
        debug_log(f"Convertendo objeto com tolist() -> lista")
        try:
            return convert_numpy_types(obj.tolist(), debug_path)
        except Exception as e:
    ...
    # Objetos customizados
        ...
        debug_log(f"Convertendo objeto customizado via __dict__")
        try:
            # Verifica se o __dict__ não está vazio
            if not obj.__dict__:
                debug_log(f"__dict__ vazio, usando str(obj)", "warning")
                return str(obj)
    ...
    # Verificação final de serializabilidade
        ...
        try:
            ...
            debug_log(f"Objeto não serializável, convertendo para string: {type(obj).__name__}", "warning")
            try:
                return str(obj)
            except Exception as e:
...
def check_store_data(store_data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    for store_name in EXPECTED_STORES:
        ...
        store_report = {
            ...
            "is_empty": store_content is None or (isinstance(store_content, dict) and not store_content),
            "type": original_type,
        ...
        if not store_report["exists"]:
            ...
            store_report["problems"].append(f"Store '{store_name}' contém dados não serializáveis (mesmo após tentativa de conversão)")
            ...
            if isinstance(store_content, dict):
                ...
                for key, value in converted_content.items():
                    if not is_json_serializable(value): # Verifica o valor já convertido
                        store_report["problems"].append(f"  - Chave '{key}' ainda contém valor não serializável: {type(value)}")

        # Log para cada store verificado
        log.debug(f"[STORE CHECK] {store_name}: Existe={store_report['exists']}, Vazio={store_report['is_empty']}, Tipo={store_report['type']}, Ser.(Original)={store_report['serializable_as_is']}, Ser.(Conv.)={store_report['serializable_after_conversion']}, Chaves={len(store_report['keys'])}")
...
def fix_store_data(store_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for store_name, store_content in store_data.items():
        if store_content is None:
            ...
            print(f"Store '{store_name}': Dict com {len(store_content)} chaves: {list(store_content.keys())}")
        else:
            print(f"Store '{store_name}': {type(store_content).__name__}")

    for store_name, store_content in store_data.items():
        print(f"\n--- Processando store: {store_name} ---")
        log.debug(f"[STORE FIX] Processando store: {store_name} (Tipo original: {type(store_content).__name__})")

        if store_content is None:
            fixed_data[store_name] = {} # Trata None como dict vazio
            log.debug(f"[STORE FIX] {store_name} era None, definido como {{}}.")
            print(f"Store vazio (None), definido como {{}}.")
            continue

        try:
            # Tenta converter tipos problemáticos
            print(f"Convertendo tipos não serializáveis...")
            converted_content = convert_numpy_types(store_content)
            log.debug(f"[STORE FIX] Tipo após conversão numpy/datetime: {type(converted_content).__name__}")
            ...
            if is_json_serializable(converted_content):
                ...
                print(f"✓ Dados corrigidos e serializáveis com sucesso.")
            else:
                # Se ainda não for serializável, identifica os problemas
                print(f"⚠ Dados ainda não serializáveis após conversão inicial.")
                log.warning(f"[STORE FIX] Dados para '{store_name}' ainda não serializáveis após conversão.")
                ...
                try:
                    print(f"Tentando conversão mais agressiva (str para objetos complexos)...")
                    ...
                    def deep_convert_fallback(obj, path=""):
                        ...
                        if isinstance(obj, dict):
                            debug_log(f"Convertendo dict com {len(obj)} chaves")
                            return {k: deep_convert_fallback(v, f"{path}.{k}" if path else k) for k, v in obj.items()}
                        elif isinstance(obj, list):
                            debug_log(f"Convertendo lista com {len(obj)} itens")
                            return [deep_convert_fallback(item, f"{path}[{i}]" if path else f"[{i}]")
                                    for i, item in enumerate(obj)]
                        elif isinstance(obj, tuple):
                            debug_log(f"Convertendo tupla com {len(obj)} itens")
                            return tuple(deep_convert_fallback(item, f"{path}[{i}]" if path else f"[{i}]")
                                        for i, item in enumerate(obj))
                        elif isinstance(obj, np.ndarray):
                    ...
                    if is_json_serializable(aggressive_converted):
                        ...
                        log.info(f"[STORE FIX] Dados para '{store_name}' corrigidos com conversão agressiva.")
                        print(f"✓ Dados corrigidos com conversão agressiva.")
                    else:
                        ...
                        log.error(f"[STORE FIX] Falha na serialização para '{store_name}' mesmo após conversão agressiva.")
                        print(f"✗ Falha na serialização mesmo após conversão agressiva.")
                    ...
                    fixed_data[store_name] = error_info
                    log.error(f"[STORE FIX] Erro na conversão agressiva para '{store_name}': {conv_err}")
            ...
            fixed_data[store_name] = error_info
            log.error(f"[STORE FIX] Erro inesperado ao tentar corrigir '{store_name}': {e}", exc_info=True)
...
def log_store_diagnostics(store_data: Dict[str, Any]) -> None:
    ...
    print(f"\n\n{'='*80}")
    print(f"DIAGNÓSTICO DOS STORES")
    print(f"{'='*80}")
...
def prepare_session_data(store_data_list: List[Any], store_names: List[str]) -> Dict[str, Any]:
    ...
    """
    print(f"\n{'='*80}")
    print(f"PREPARE_SESSION_DATA INICIADO")
    print(f"{'='*80}")
    ...
    for name, data in session_data.items():
        ...
        print(f"{name}: Serializável como está: {serializable_as_is}, Após conversão: {serializable_after_conversion}")
        ...
        if not serializable_after_conversion and data is not None:
            print(f"  ⚠ AVISO: Dados não serializáveis mesmo após conversão!")
            if isinstance(data, dict) and data:
                # Verifica cada chave individualmente
                print(f"  Verificando chaves problemáticas:")
                for key, value in data.items():
                    if not is_json_serializable(value, attempt_conversion=True):
                        print(f"    - Chave '{key}' contém valor não serializável: {type(value).__name__}")
                        ...
                        if isinstance(value, dict):
                            for subkey, subvalue in value.items():
                                if not is_json_serializable(subvalue, attempt_conversion=True):
                                    print(f"      - Subchave '{key}.{subkey}' contém valor não serializável: {type(subvalue).__name__}")
    ...
    for name, data in fixed_data.items():
        try:
            ...
            log.error(f"[PREPARE] Store {name} ainda não é serializável após todas as correções: {e}")
    ...
    print(f"\n{'='*80}")
    print(f"PREPARE_SESSION_DATA CONCLUÍDO")
    print(f"{'='*80}")
