...
def calculate_empty_losses(tensao_bt: float, inducao: float, peso_nucleo: float, tipo_transformador: str = 'Trifásico') -> tuple[float, float, float]:
    """
    ...
    try:
        ...
        if tensao_bt_f <= 0 or inducao_f <= 0 or peso_nucleo_f <= 0:
            log.error(f"Valores inválidos para cálculo de perdas em vazio: V={tensao_bt_f}, B={inducao_f}, m={peso_nucleo_f}")
            return None, None, None
        ...
        corrente_excitacao = potencia_mag / (tensao_bt_f * 1000 * sqrt_3) if tensao_bt_f != 0 else 0  # A
        
        log.info(f"Perdas em vazio calculadas: P0={perdas_vazio_kw:.2f}kW, I0={corrente_excitacao:.2f}A, Q0={potencia_mag_kvar:.2f}kVAr")
...
def calculate_load_losses(corrente: float, impedancia_percent: float, tensao: float, temperatura_teste: float = 75, temperatura_referencia: float = 75) -> tuple[float, float, float]:
    """
    ...
    try:
        ...
        if corrente_f <= 0 or impedancia_f <= 0 or tensao_f <= 0:
            log.error(f"Valores inválidos para cálculo de perdas em carga: I={corrente_f}, Z%={impedancia_f}, V={tensao_f}")
            return None, None, None
        ...
        log.info(f"Perdas em carga calculadas: Pk={perdas_carga_corrigidas:.2f}kW, Vcc={tensao_cc:.2f}kV, Steste={potencia_teste:.2f}MVA")
