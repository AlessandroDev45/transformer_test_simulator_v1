...
def format_parameter_value(param_value, precision: int = 2, unit: str = "", none_value: str = "N/A") -> str:
    """
...
def formatar_dados_basicos(dados: dict | None) -> dict:
    ...
    nomes_amigaveis = {
        'tipo_transformador': 'Tipo', 'potencia_mva': 'Potência (MVA)',
        'frequencia': 'Frequência (Hz)',
        'tensao_at': 'Tensão Nom. AT (kV)', 'corrente_nominal_at': 'Corrente Nom. AT (A)', 'impedancia': 'Impedância Nom. (%)', 'conexao_at': 'Conexão AT', 'nbi_at': 'NBI AT (kV)',
        'tensao_at_tap_maior': 'Tensão Tap+ AT (kV)', 'corrente_nominal_at_tap_maior': 'Corrente Tap+ AT (A)', 'impedancia_tap_maior': 'Impedância Tap+ (%)',
        'tensao_at_tap_menor': 'Tensão Tap- AT (kV)', 'corrente_nominal_at_tap_menor': 'Corrente Tap- AT (A)', 'impedancia_tap_menor': 'Impedância Tap- (%)',
        'tensao_bt': 'Tensão Nom. BT (kV)', 'corrente_nominal_bt': 'Corrente Nom. BT (A)', 'conexao_bt': 'Conexão BT', 'nbi_bt': 'NBI BT (kV)',
        'tensao_terciario': 'Tensão Nom. Terciário (kV)', 'corrente_nominal_terciario': 'Corrente Nom. Terciário (A)', 'conexao_terciario': 'Conexão Terciário', 'nbi_terciario': 'NBI Terciário (kV)',
    ...
    ordem_campos = [
        'tipo_transformador', 'potencia_mva', 'frequencia',
        'tensao_at', 'corrente_nominal_at', 'impedancia', 'conexao_at', 'nbi_at', 'tensao_bucha_neutro_at',
        'tensao_at_tap_maior', 'corrente_nominal_at_tap_maior', 'impedancia_tap_maior',
        'tensao_at_tap_menor', 'corrente_nominal_at_tap_menor', 'impedancia_tap_menor',
        'tensao_bt', 'corrente_nominal_bt', 'conexao_bt', 'nbi_bt', 'tensao_bucha_neutro_bt',
        'tensao_terciario', 'corrente_nominal_terciario', 'conexao_terciario', 'nbi_terciario', 'tensao_bucha_neutro_terciario',
        'peso_total', 'peso_parte_ativa', 'peso_oleo',
    ]
    ...
    for k in ordem_campos:
        ...
        if valor is not None and str(valor).strip() != '':
            ...
            precision = 0 if 'peso' in k else (1 if 'mva' in k or 'tensao' in k or 'nbi' in k else 2)
            unit = 'kV' if 'tensao' in k or 'nbi' in k else ('%' if 'impedancia' in k else ('kg' if 'peso' in k else ('Hz' if 'freq' in k else ('A' if 'corrente' in k else ('MVA' if 'potencia' in k else '')))))
...
def formatar_perdas_vazio(dados_vazio: dict | None) -> dict:
    ...
    formatting_rules = {
        ...
        "Tensão nominal teste 1.0 pu (kV)": {'prec': 3, 'unit': 'kV'},
        "Corrente de excitação (A)": {'prec': 3, 'unit': 'A'},
        "Corrente de excitação calculada (A)": {'prec': 3, 'unit': 'A'},
        "Tensão de teste 1.1 pu (kV)": {'prec': 3, 'unit': 'kV'},
        "Corrente de excitação 1.1 pu (A)": {'prec': 3, 'unit': 'A'},
        "Tensão de teste 1.2 pu (kV)": {'prec': 3, 'unit': 'kV'},
        "Corrente de excitação 1.2 pu (A)": {'prec': 3, 'unit': 'A'},
        "Frequência (Hz)": {'prec': 1, 'unit': 'Hz'},
        "Potência Mag. (kVAr)": {'prec': 2, 'unit': 'kVAr'},
        "Fator de perdas Mag. (VAr/kg)": {'prec': 3, 'unit': 'VAr/kg'},
        "Fator de perdas (W/kg)": {'prec': 3, 'unit': 'W/kg'},
        "Peso do núcleo de projeto (Ton)": {'prec': 3, 'unit': 'Ton'},
        "Peso do núcleo Calculado(Ton)": {'prec': 3, 'unit': 'Ton'},
        "Potência de Ensaio (1 pu) (kVA)": {'prec': 1, 'unit': 'kVA'},
        "Potência de Ensaio (1.1 pu) (kVA)": {'prec': 1, 'unit': 'kVA'},
        "Potência de Ensaio (1.2 pu) (kVA)": {'prec': 1, 'unit': 'kVA'}
    }
    ...
    def format_section(data_dict):
        ...
        for key_orig, value in data_dict.items():
            if value is not None:
                 rules = formatting_rules.get(key_orig, {'prec': 2, 'unit': ''}) # Default formatting
...
def formatar_perdas_carga(dados_carga: dict | None) -> dict:
    ...
    map_nominal = {
        "tensao": ("Tensão", 2, "kV"), "corrente": ("Corrente", 2, "A"), "imp": ("Vcc (%)", 2, "%"),
        "vcc_kv": ("Vcc (kV)", 2, "kV"), "ptot_kw": ("Perdas Totais", 2, "kW"),
        "pc_frio_kw": ("Perdas CC Frio", 2, "kW")
    }
    map_frio = {
        "vt_frio_kv": ("Tensão", 2, "kV"), "it_frio_a": ("Corrente", 2, "A"), "pteste_frio_mva": ("Pteste", 2, "MVA"),
        "vbanco_frio_kv": ("Cap Bank Tensão", 1, "kV"), "pbanco_frio_mvar": ("Cap Bank Pot.", 2, "MVAr"),
        "pa_eps_frio_kw": ("Pot. Ativa EPS", 1, "kW")
    }
    map_quente = {
        "vt_quente_kv": ("Tensão", 2, "kV"), "it_quente_a": ("Corrente", 2, "A"), "pteste_quente_mva": ("Pteste", 2, "MVA"),
        "vbanco_quente_kv": ("Cap Bank Tensão", 1, "kV"), "pbanco_quente_mvar": ("Cap Bank Pot.", 2, "MVAr"),
        "pa_eps_quente_kw": ("Pot. Ativa EPS", 1, "kW")
    }
    map_1_2 = {
        "vt_1.2_kv": ("Tensão", 2, "kV"), "it_1.2_a": ("Corrente", 2, "A"), "pteste_1.2_mva": ("Pteste", 2, "MVA"),
        "pc_1.2_kw": ("Perdas", 1, "kW"), "vbanco_1.2_kv": ("Cap Bank Tensão", 1, "kV"),
        "pbanco_1.2_mvar": ("Cap Bank Pot.", 2, "MVAr"), "pa_eps_1.2_kw": ("Pot. Ativa EPS", 1, "kW")
    }
    map_1_4 = {
        "vt_1.4_kv": ("Tensão", 2, "kV"), "it_1.4_a": ("Corrente", 2, "A"), "pteste_1.4_mva": ("Pteste", 2, "MVA"),
        "pc_1.4_kw": ("Perdas", 1, "kW"), "vbanco_1.4_kv": ("Cap Bank Tensão", 1, "kV"),
        "pbanco_1.4_mvar": ("Cap Bank Pot.", 2, "MVAr"), "pa_eps_1.4_kw": ("Pot. Ativa EPS", 1, "kW")
    }
...
def formatar_elevacao_temperatura(dados: dict | None) -> dict:
    ...
    input_map = {
        'input_ta': ('Temp. Ambiente (Θa)', 1, '°C'), 'input_material': ('Material Enrolamento', 0, ''),
        'input_rc': ('Resistência Fria (Rc)', 3, 'Ohm'), 'input_tc': ('Temp. Ref. Fria (Θc)', 1, '°C'),
        'input_rw': ('Resistência Quente (Rw)', 3, 'Ohm'), 'input_t_oil': ('Temp. Topo Óleo Final (Θoil)', 1, '°C'),
...
def formatar_curto_circuito(dados: dict | None) -> dict:
    ...
    for key, (label, prec, unit) in result_map.items():
        ...
        if value is not None:
            ...
            val_str = f"±{format_parameter_value(value, prec, unit)}" if key == 'limit_used' else format_parameter_value(value, prec, unit)
...
def formatar_analise_dieletrica(dados: dict | None) -> dict:
    ...
    general_params = {
        "Tipo Transformador": format_parameter_value(params.get('tipo_transformador'), none_value='N/A'),
    ...
    input_headers = ["Enrolamento", "Um (kV)", "Conexão", "IA (kV)", "IM (kV)", "T.Aplicada (kV)", "Um Neutro (kV)", "IA Neutro (kV)"]
    ...
    for enrol in enrolamentos_params:
        if enrol.get('um'): # Inclui apenas se Um foi definido
            input_table_data.append([
                ...
                format_parameter_value(enrol.get('ia'), 0, "kV", "-"), # Precisão 0 para IA/IM/Aplicada
                format_parameter_value(enrol.get('im'), 0, "kV", "-"),
                format_parameter_value(enrol.get('tensao_curta'), 0, "kV", "-"),
                format_parameter_value(enrol.get('neutro_um'), 1, "kV", "-") if enrol.get('conexao') == 'YN' else '-',
                format_parameter_value(enrol.get('ia_neutro'), 0, "kV", "-") if enrol.get('conexao') == 'YN' else '-',
            ])
            has_input_data = True
    if has_input_data:
    ...
    results_headers = ["Enrolamento", "IAC (kV)", "Esp. F-T NBR (mm)", "Esp. F-F NBR (mm)", "Esp. Outro NBR (mm)", "Clearance IEEE (mm)"]
    ...
    if sequencias:
        def format_seq_table_pdf(seq_data):
            if not seq_data or not seq_data.get('patamares'): return None
        ...
        if seq_mono: seq_content["Ensaio Monofásico"] = seq_mono
        if seq_tri_curta: seq_content["Ensaio Trifásico Curta Duração"] = seq_tri_curta
        if seq_tri_longa: seq_content["Ensaio Trifásico Longa Duração"] = seq_tri_longa

    if seq_content:
        report_content["Sequências de Ensaio Induzido (NBR)"] = seq_content # Cria seção com subseções
...
def formatar_tensao_aplicada(dados: dict | None) -> dict:
    ...
    if has_viab_data:
        ...
        recommendations_text = "\n".join([f"- {enr.get('enrolamento')}: {enr.get('recommendation', '')}" for enr in analise_enrolamentos])
...
def formatar_tensao_induzida(dados: dict | None) -> dict:
    ...
    dados_calculo = resultados.get('dados_calculo', {})
    tipo_transformador = dados.get('inputs_tensao_induzida', {}).get('tipo_transformador', 'Desconhecido')
    ...
    map_mono = {
        'tensao_aplicada_bt': ('Tensão Aplicada BT', 2, 'kV'), 'up_un': ('Up/Un', 3, ''),
        'fp_fn': ('Fp/fn', 3, ''), 'beta_teste': ('βteste', 3, 'T'),
        'pot_ativa': ('Potência Ativa Pw', 2, 'kW'), 'pot_magnetica': ('Pot. Aparente Magnética Sm', 2, 'kVA'),
        'pot_induzida': ('Comp. Indutiva Qm', 2, 'kVAr ind'), 'u_dif': ('U p/ cálculo Scap', 2, 'kV'),
        'pcap': ('Pot. Capacitiva Scap', 2, 'kVAr cap'), 'fator_potencia_mag': ('Fator Pot. Mag.', 3, 'VAr/kg'),
...
def formatar_impulso(dados: dict | None) -> dict:
    ...
    # {key: (Label, Precision, Unit)}
    input_map = {
        'gen': ('Config. Gerador', 0, ''), 'v_test_target': ('V Teste Alvo', 1, 'kV'),
        'c_dut': ('Carga DUT', 0, 'pF'), 'c_stray': ('Carga Parasita', 0, 'pF'),
        'l_ext_h': ('L Externa Total', 1, 'µH'), # Converte H para µH
        'l_trafo_h': ('L Trafo', 4, 'H'), 'l_add_h': ('L Adicional', 1, 'µH'), # Converte H para µH
    ...
    for key_orig, (label, precision, unit) in input_map.items():
        ...
        if key_orig in ['l_ext_h', 'l_add_h'] and value is not None: value *= 1e6
        if value is not None:
    ...
    analysis_map = {}
    if sim_type == 'Lightning': analysis_map = analysis_map_li
    elif sim_type == 'Switching': analysis_map = analysis_map_si
    elif sim_type == 'Chopped': analysis_map = analysis_map_lic
    ...
    # {key: (Label, Precision, Unit)}
    circuit_map = {
        'cg_nf': ('Cg Efetiva', 3, 'nF'), 'cl_pf': ('Cl Total', 0, 'pF'), 'ceq_nf': ('Ceq', 3, 'nF'),
        'rf_total_ohm': ('Rf Efetiva', 2, 'Ω'), 'rt_total_ohm': ('Rt Efetiva', 2, 'Ω'),
        'l_total_h': ('L Total', 3, 'mH'), # Converte para mH
        'alpha': ('α Calculado', 2, 's⁻¹'), 'beta': ('β Calculado', 2, 's⁻¹'),
        'zeta': ('ζ Calculado', 3, ''),
    ...
    for key_orig, (label, precision, unit) in circuit_map.items():
        ...
        if key_orig == 'l_total_h' and value is not None: value *= 1000
        if value is not None:
    ...
    # --- Eficiência ---
    eff_map = {'circuit': ('Eficiência Circuito', 1, '%'), 'shape': ('Eficiência Forma', 1, '%'), 'total': ('Eficiência TOTAL', 1, '%')}
    sim_section_eff = {}
    for key, (label, prec, unit) in eff_map.items():
        value = sim_efficiency.get(key)
        if value is not None:
             sim_section_eff[label] = format_parameter_value(value * 100, prec, unit) # Multiplica por 100
    if sim_section_eff:
         sub_sections["Eficiência"] = sim_section_eff

    # --- Energia ---
    energy_map = {'req_kj': ('Energia Requerida', 2, 'kJ'), 'avail_kj': ('Energia Disponível', 1, 'kJ'), 'ok': ('Energia Suficiente?', 0, '')}
    sim_section_energy = {}
