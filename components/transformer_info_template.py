...
def _create_info_item(label, value, unit="", label_style_override=None, value_style_override=None):
    ...
    base_label_style = {"fontSize": "0.65rem", "fontWeight": "normal", "color": COLORS['text_muted'],
                        "whiteSpace": "nowrap", "lineHeight": "1.1", "display": "inline-block"}
    base_value_style = {"fontSize": "0.65rem", "fontWeight": "600", "color": COLORS['text_light'],
                        "lineHeight": "1.1", "display": "inline-block", "letterSpacing": "0.01rem"}
    ...
    # Tratar especificamente conexões para exibir D, Yn, Y em vez de triangulo, estrela...
        ...
        if value == 'triangulo': display_value = 'D'
        elif value == 'estrela': display_value = 'Yn'
        elif value == 'estrela_sem_neutro': display_value = 'Y'
        elif value == ' ': display_value = '-' # Terciário vazio
        else: display_value = value # Caso inesperado
    else:
        display_value = value

    formatted_label = label[0].upper() + label[1:] if label and len(label) > 0 else label
    content = [html.Span(f"{formatted_label} ", style=final_label_style), # Removido ':'
               html.Span(f" {display_value} {unit}".strip(), style=final_value_style)]
    div_style = {"marginBottom": "3px", "lineHeight": "1.2", "padding": "0 2px",
                 "borderBottom": f"1px solid {COLORS['border']}",
                 "textAlign": "left", "width": "100%"} # Mudado para textAlign left
...
def create_transformer_info_panel(transformer_data):
    ...
    if not isinstance(transformer_data, dict) or not transformer_data:
        ...
        return html.Div("Dados do transformador não disponíveis ou inválidos.",
                       style={"fontSize": "0.7rem", "color": COLORS['danger'], "padding": "10px",
                              "border": f"1px solid {COLORS['danger']}", "borderRadius": "4px",
                              "backgroundColor": COLORS['background_card']})

    col_base_style = {"padding": "4px 6px", "overflow": "hidden",
                      "backgroundColor": COLORS['background_card']}
    col_style_with_border = {**col_base_style, "borderRight": f"1px solid {COLORS['border']}",
                            "boxShadow": "inset -1px 0 0 rgba(0,0,0,0.1)"}
    col_style_last = col_base_style
    title_style_base = {"fontSize": "0.68rem", "fontWeight": "bold", "color": COLORS['text_light'],
                        "padding": "4px 4px", "borderRadius": "3px", "textAlign": "center",
                        "marginBottom": "6px", "lineHeight": "1.2", "width": "100%",
                        "boxShadow": "0 1px 2px rgba(0,0,0,0.1)",
                        "borderBottom": f"1px solid {COLORS['border']}",
                        "letterSpacing": "0.02rem", "textTransform": "uppercase",
                        "textShadow": "0px 1px 1px rgba(0,0,0,0.1)"}
    subtitle_style = {**title_style_base, "fontSize": "0.63rem", "fontWeight": "normal",
                     "backgroundColor": COLORS['background_card_header'], "color": COLORS['text_muted'],
                     "marginTop": "6px", "marginBottom": "4px", "boxShadow": "none",
                     "borderTop": f"1px solid {COLORS['border']}",
                     "borderBottom": f"1px solid {COLORS['border']}", "textTransform": "none", "textAlign":"center"} # Centralizado subtitle
    ...
    try:
        ...
        col_width = 12 // 5
        top_col_style = {"display": "flex", "justifyContent": "center", "alignItems": "center",
                         "padding": "0.25rem 0.5rem", "height": "100%"}
        top_row = dbc.Row([
            dbc.Col(
                _create_info_item(label, transformer_data.get(key), unit),
                width=col_width, className="text-center px-0",
                style={**top_col_style,
                       "borderRight": f"1px solid {COLORS['border']}" if i < len(top_items)-1 else None,
        ...
        ], className="g-0 py-1 align-items-stretch justify-content-between", style=top_row_style) # align-items-stretch

        # --- Linha Principal: Detalhes Técnicos (7 Colunas) ---
        main_row_style = {"borderBottom": f"1px solid {COLORS['border']}", "width": "100%",
                          "backgroundColor": COLORS['background_card'], "color": COLORS['text_light']}
        main_columns = [
            {"title": "AT Tap-", "bg_color": bg_color_tap, "items": [("Tensão:", 'tensao_at_tap_menor', "kV"), ("Corrente:", 'corrente_nominal_at_tap_menor', "A"), ("Z:", 'impedancia_tap_menor', "%")], "style": col_style_with_border},
            {"title": "AT Nominal", "bg_color": bg_color_nominal, "items": [
                ("Tensão:", 'tensao_at', "kV"),
            ...
            {"title": "Terciário", "bg_color": bg_color_default, "items": [
            ...
            {"title": "Tensões de Ensaio", "bg_color": bg_color_default, "items": [(None, None, None, "Aplicada (kV)"), ("AT:", 'teste_tensao_aplicada_at', ""), ("BT:", 'teste_tensao_aplicada_bt', ""), ("Ter.:", 'teste_tensao_aplicada_terciario', ""), (None, None, None, "Induzida (kV)"), ("AT:", 'teste_tensao_induzida', "")], "style": col_style_with_border},
            {"title": "Pesos (ton)", "bg_color": bg_color_default, "items": [("Total:", 'peso_total', ""), ("Parte Ativa:", 'peso_parte_ativa', ""), ("Óleo:", 'peso_oleo', ""), ("Tanque/Acess.:", 'peso_tanque_acessorios', ""), (None, None, None, "Líq. Isolante"), ("Tipo:", 'liquido_isolante', "")], "style": col_style_last}
        ]
        main_row = dbc.Row([
            dbc.Col([
                html.Div(col["title"], style={**title_style_base, "backgroundColor": col["bg_color"]}), # Corrigido para backgroundColor
                *[
                    html.Div(extra[0], style=subtitle_style) if len(extra) > 0 and label is None
                    else _create_info_item(
                        label,
                        transformer_data.get(key),
                        unit
                    )
                    for label, key, unit, *extra in col["items"]
                ],
            ], style=col["style"], className="d-flex flex-column") # Added flex column
            for col in main_columns
        ], className="row-cols-7 g-0 py-1", style=main_row_style)
        ...
        return html.Div(
            ...
            style={
                "backgroundColor": COLORS['background_card'], "borderColor": COLORS['border'],
                "color": COLORS['text_light'], "width": "100%",
        ...
        error_style = {"fontSize": "0.7rem", "color": COLORS['danger'], "padding": "10px",
                      "border": f"1px solid {COLORS['danger']}", "borderRadius": "4px",
                      "backgroundColor": COLORS['background_card']}
