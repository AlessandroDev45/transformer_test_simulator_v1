...
def create_labeled_input(
    ...
    **kwargs # Outros props para dbc.Input ou dcc.Dropdown
    ...
    from layouts import TYPOGRAPHY, COMPONENTS
    ...
    if input_type == "dropdown":
        ...
        if 'style' in kwargs:
            log.warning(f"Style fornecido tanto via input_style quanto via kwargs para {input_id}. Usando input_style.")
            kwargs.pop('style')
        ...
        input_component = dcc.Dropdown(
            id=input_id, options=options or [], value=str_value, placeholder=placeholder,
            disabled=disabled, clearable=kwargs.pop('clearable', False),
            persistence=persistence, persistence_type=persistence_type,
            style=final_input_style, className=class_name, **kwargs # Usa a classe fornecida ou dark-dropdown por padrão
        )
    else:
        # Remove style dos kwargs se estiver presente para evitar conflito
        if 'style' in kwargs:
        ...
        input_component = dbc.Input(
            ...
            step=step if input_type == "number" else None, value=value,
            disabled=disabled, readonly=readonly,
            persistence=persistence, persistence_type=persistence_type,
            style=final_input_style, className=kwargs.pop('className', ''), **kwargs
        )

    return dbc.Row(
        [
            dbc.Col(label_component, width=label_width, className="d-flex align-items-center justify-content-end"),
...
def create_dielectric_input_column(verificador_instance, label: str, identifier: str) -> dbc.Card:
    ...
    # Importar estilos padronizados
    from layouts import COLORS, TYPOGRAPHY, COMPONENTS
    ...
    return dbc.Card([
        dbc.CardHeader(html.Div(label, className="text-center fw-bold"), style=card_header_style),
        dbc.CardBody([
            # Conexão (Campo de texto somente leitura, como os demais)
            create_labeled_input(
                "Conexão:", conexao_id, input_type='text',
                placeholder="-", label_width=label_col_width, input_width=input_col_width,
                readonly=True, className="read-only-input"
            ),
            # Div Neutro - Estilo consistente com os demais campos
            html.Div(id=neutro_div_id, style={
                'display': 'block',
                'marginBottom': '0.5rem'
            }, children=[
                create_labeled_input(
                    "Um Neutro (kV):", neutro_um_id, input_type='text',
                    placeholder="-", label_width=label_col_width, input_width=input_col_width,
                    readonly=True, className="read-only-input"
                ),
                create_labeled_input(
                    "IA / BIL Neutro (kV):", ia_neutro_id, input_type='text',
                    placeholder="-", label_width=label_col_width, input_width=input_col_width,
                    readonly=True, className="read-only-input"
                ),
            ]),
            # Um
            create_labeled_input(
                "Um (kV):", um_id, input_type='text',
                placeholder=f"-", label_width=label_col_width, input_width=input_col_width,
                readonly=True, className="read-only-input"
            ),
            # IA / BIL
            create_labeled_input(
                "IA / BIL (kV):", ia_id, input_type='text',
                placeholder="-", label_width=label_col_width, input_width=input_col_width,
                readonly=True, className="read-only-input"
            ),
             # IAC
            create_labeled_input(
                "IAC (kV):", iac_id, input_type='text',
                placeholder="", readonly=True, label_width=label_col_width, input_width=input_col_width,
                className="read-only-input"
            ),
            # IM / SIL
            create_labeled_input(
                "IM / SIL (kV):", im_id, input_type='text',
                placeholder="-", label_width=label_col_width, input_width=input_col_width,
                readonly=True, className="read-only-input"
            ),
            # T.Aplicada
            create_labeled_input(
                "T.Aplicada (kV):", tc_id, input_type='text',
                placeholder="-", label_width=label_col_width, input_width=input_col_width,
                readonly=True, className="read-only-input"
            ),
            # T.Induzida (agora como input text em vez de dropdown)
            create_labeled_input(
                "T.Induzida (kV):", ti_id, input_type='text',
                placeholder="-", label_width=label_col_width, input_width=input_col_width,
                readonly=True, className="read-only-input"
            ),
            # Espaçamentos
            html.Div([
                ...
                html.Div("Distâncias Dielétricas:", style={**TYPOGRAPHY['section_title'], "marginBottom": "4px", "textAlign": "center"}),
                dcc.Loading(
                    html.Div([
...
def create_test_sequence_table(title: str, data: dict | None, tipo_isolamento: str = None) -> html.Div:
    ...
    title_style = {"fontSize": "0.7rem", "fontWeight": "bold", "backgroundColor": bg_medium, "padding": "2px", "marginTop": "4px", "marginBottom": "4px", "textAlign": "center"}
    info_style = {"fontSize": "0.7rem", "fontStyle": "italic", "marginBottom": "5px", "color": config.colors.get('text_muted', '#6c757d')}

    if not data or not isinstance(data, dict) or not data.get('patamares'):
        return html.Div([
             html.Div(title, style=title_style),
             html.P("Dados da sequência não disponíveis.", style={**base_style, "fontStyle": "italic"})
        ], className="mb-2")
    ...
    table_body = [html.Tbody([
        html.Tr([
            html.Td(format_parameter_value(p.get('nivel'), none_value='-'), style=base_style),
            html.Td(format_parameter_value(p.get('tensao'), 1, "kV", "-"), style=base_style),
            html.Td(format_parameter_value(p.get('tempo'), none_value='-'), style=base_style)
        ]) for p in patamares if isinstance(p, dict) # Verifica se p é dict
    ])]
    ...
    if medicao_dp and isinstance(medicao_dp, dict): # Verifica se medicao_dp é dict
        ...
        dp_items = [html.Li(f"{k.replace('_',' ').title()}:   {format_parameter_value(v, 1, 'kV', '-')}", style={"fontSize": "0.7rem"})
                    for k, v in medicao_dp.items() if v is not None]
        if dp_items:
             dp_info.append(html.Ul(dp_items, style={"marginBottom": "0.2rem", "paddingLeft": "1rem"}))
    ...
    if tipo_isolamento and "Trifásico Longa" in title:
        if tipo_isolamento == 'progressivo':
            isolamento_info.append(html.Div(
...
def create_comparison_table(title: str, nbr_data: dict | None, ieee_data: dict | None, tipo_isolamento: str = None) -> html.Div:
    ...
    if is_required_tests:
        for label, (nbr_key, ieee_key) in mapping_req.items():
            ...
            ieee_value = ieee_data.get(ieee_key, "-") if isinstance(ieee_data, dict) and ieee_key else "-"
            rows.append(html.Tr([
        ...
        all_keys = set()
        if isinstance(nbr_data, dict): all_keys.update(nbr_data.keys())
        if isinstance(ieee_data, dict): all_keys.update(ieee_data.keys())
    ...
    if not rows:
         return html.Div([html.Div(title, style=title_style), html.P("Dados não disponíveis.", style=base_style)])
...
def create_circuit_parameters_display(params_data: dict | None) -> dbc.Table | dbc.Alert:
    ...
    if "error" in params_data:
        return dbc.Alert(f"Erro nos parâmetros: {params_data['error']}", color="danger", style={"fontSize": "0.7rem"})

    header = html.Thead(html.Tr([html.Th("Parâmetro", style={"fontSize":"0.7rem"}), html.Th("Valor", style={"fontSize":"0.7rem"})]))
    rows = []
    ...
    def add_row(label, value, unit="", precision=2):
        ...
        rows.append(html.Tr([html.Td(label, style=label_style), html.Td(formatted_value, style=cell_style)]))
    ...
    try:
        ...
        return dbc.Alert(f"Erro ao exibir parâmetros: {str(e)}", color="danger", style={"fontSize": "0.7rem"})

    return dbc.Table([header, html.Tbody(rows)], bordered=True, hover=True, striped=True, size="sm", className="mb-0")

def create_energy_details_table(energy_required_kj: float | None, max_energy_kj: float | None, generator_config_label: str | None) -> dbc.Table | dbc.Alert:
    ...
    try:
        ...
        rows.append(html.Tr([html.Td("Margem de Energia:"), html.Td(margin_text, className=margin_class)]))
        cell_style = {"fontSize": "0.7rem", "padding": "0.2rem"}
        for row in rows:
            for cell in row.children:
                 if isinstance(cell, html.Td): cell.style = cell_style
        ...
        return dbc.Alert(f"Erro ao exibir energia: {str(e)}", color="danger", style={"fontSize": "0.7rem"})

def create_waveform_analysis_table(analysis_results: dict | None, impulse_type: str) -> dbc.Table | dbc.Alert:
    ...
    if constants is None:
        return dbc.Alert("Erro interno: Não foi possível carregar constantes.", color="danger", style={"fontSize": "0.7rem"})
    ...
    def add_analysis_row(label, param_key, unit, req_min, req_max, conforme_key, precision=2, is_max_limit=False):
        ...
        req_str = ""
        if is_max_limit and req_max is not None: req_str = f"≤ {format_parameter_value(req_max, precision, unit)}"
        elif req_min is not None and req_max is None: req_str = f"≥ {format_parameter_value(req_min, precision, unit)}"
        elif req_min is not None and req_max is not None: req_str = f"{format_parameter_value(req_min, precision, '')} - {format_parameter_value(req_max, precision, unit)}"
        else: req_str = "-"

        status_symbol = "❓"
        current_row_style = warn_style
        if conforme is True:
            status_symbol = "✔️"; current_row_style = pass_style
        elif conforme is False:
            status_symbol = "❌"; current_row_style = fail_style
    ...
    try:
        if impulse_type == "lightning":
            req = {'T1_min': constants.LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1 - constants.LIGHTNING_FRONT_TOLERANCE),
                   'T1_max': constants.LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1 + constants.LIGHTNING_FRONT_TOLERANCE),
                   'T2_min': constants.LIGHTNING_IMPULSE_TAIL_TIME_NOM * (1 - constants.LIGHTNING_TAIL_TOLERANCE),
                   'T2_max': constants.LIGHTNING_IMPULSE_TAIL_TIME_NOM * (1 + constants.LIGHTNING_TAIL_TOLERANCE),
                   'Beta_max': constants.LIGHTNING_OVERSHOOT_MAX * 100.0 }
            add_analysis_row("Vt (Pico Ensaio)", 'peak_value_test', "kV", None, None, 'conforme_pico', precision=1)
            add_analysis_row("T₁ (Frente)", 't_front_us', "µs", req['T1_min'], req['T1_max'], 'conforme_frente')
            add_analysis_row("T₂ (Cauda)", 't_tail_us', "µs", req['T2_min'], req['T2_max'], 'conforme_cauda', precision=1)
            add_analysis_row("β (Overshoot)", 'overshoot_percent', "%", None, req['Beta_max'], 'conforme_overshoot', precision=1, is_max_limit=True)
        elif impulse_type == "switching":
            req = {'Tp_min': constants.SWITCHING_IMPULSE_PEAK_TIME_NOM * (1 - constants.SWITCHING_PEAK_TIME_TOLERANCE),
                   'Tp_max': constants.SWITCHING_IMPULSE_PEAK_TIME_NOM * (1 + constants.SWITCHING_PEAK_TIME_TOLERANCE),
                   'T2_min': constants.SWITCHING_IMPULSE_TAIL_TIME_NOM * (1 - constants.SWITCHING_TAIL_TOLERANCE),
                   'T2_max': constants.SWITCHING_IMPULSE_TAIL_TIME_NOM * (1 + constants.SWITCHING_TAIL_TOLERANCE),
                   'Td_min': constants.SWITCHING_TIME_ABOVE_90_MIN,
                   'Tz_min': constants.SWITCHING_TIME_TO_ZERO_MIN }
            add_analysis_row("Vp (Pico Medido)", 'peak_value_measured', "kV", None, None, 'conforme_pico', precision=1)
            add_analysis_row("Tp (Tempo Pico Norma)", 't_p_us', "µs", req['Tp_min'], req['Tp_max'], 'conforme_tp', precision=0)
            add_analysis_row("T₂ (Meio Valor)", 't_2_us', "µs", req['T2_min'], req['T2_max'], 'conforme_t2', precision=0)
            add_analysis_row("Td (>90%)", 'td_us', "µs", req['Td_min'], None, 'conforme_td', precision=0)
            add_analysis_row("Tz (Zero)", 't_zero_us', "µs", req['Tz_min'], None, 'conforme_tzero', precision=0)
        elif impulse_type == "chopped":
            req = {'T1_min': constants.LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1 - constants.CHOPPED_FRONT_TOLERANCE),
                   'T1_max': constants.LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1 + constants.CHOPPED_FRONT_TOLERANCE),
            ...
            add_analysis_row("Vc (Tensão Corte Teste)", 'chop_voltage_test_kv', "kV", None, None, 'conforme_pico_corte', precision=1)
            add_analysis_row("T₁ (Frente Subj.)", 't_front_us', "µs", req['T1_min'], req['T1_max'], 'conforme_frente')
            add_analysis_row("Tc (Tempo Corte)", 'chop_time_us', "µs", req['Tc_min'], req['Tc_max'], 'conforme_corte')
            add_analysis_row("Undershoot", 'undershoot_percent', "%", None, req['Undershoot_max'], 'conforme_undershoot', precision=1, is_max_limit=True)
        else:
