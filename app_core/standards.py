...
import numpy as np
from typing import Dict, List, Any, Optional, Union # Adicionar Optional e Union
        ...
        try:
            ...
            if '/' in value_cleaned:
                try:
                    ...
                    log.error(f"Erro ao converter parte antes de '/' em '{value_cleaned}' para float.")
                    return None
            else:
...
class NormaBase:
    ...
    def __init__(self, nome_norma: str, arquivo: str = None, arquivo_json: str = None):
        ...
        log.debug(f"Inicializando base para norma {self.nome_norma}" +
    ...
    def _carregar_dados(self):
        ...
        if not json_loaded:
            log.info(f"Falha ao carregar JSON para {self.nome_norma}. Tentando carregar Excel (se definido)...")
            import os
            if self.arquivo and os.path.exists(self.arquivo):
                self._carregar_excel()
            else:
                log.warning(f"Nem JSON nem arquivo Excel ({self.arquivo}) encontrados ou definidos para {self.nome_norma}.")
        ...
        try:
            ...
            for sheet_name in excel_data.sheet_names:
                try:
                    ...
                    log.error(f"Erro ao carregar planilha '{sheet_name}' do Excel ({self.arquivo}): {e_sheet}")
    ...
    def _carregar_json(self) -> bool:
        ...
        try:
            ...
            if not os.path.isabs(self.arquivo_json):
                 ...
                 json_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), self.arquivo_json)
            else:
            ...
            self.tabelas["json_loaded"] = True # Indica que JSON foi carregado (para compatibilidade)
            return True
            ...
            log.exception(f"Erro inesperado ao carregar dados JSON para {self.nome_norma} de {self.arquivo_json}: {e}")
            self.dados_json = None
            return False
        ...
        try:
            ...
            for um_valor_str, level_data in levels.items():
                ...
                if um_float < 100:
                    ...
                    valores_um = sorted([safe_float_convert(k) for k in levels.keys() if safe_float_convert(k) is not None])
                    idx = valores_um.index(um_float)

                    if idx == 0:  # Primeiro valor
                        um_min = 0
                        um_max = (um_float + valores_um[idx + 1]) / 2 if idx + 1 < len(valores_um) else um_float * 1.1
                ...
                for teste_key, teste_data in level_data.items():
                    if teste_key in ['li', 'lic', 'si', 'acsd', 'acld', 'applied']:
                        ...
                        teste_obj = {
                            ...
                            'classificacao_ieee': 'Rotina' if teste_data.get('sel_req', False) else 'Design/Other',
                            'nbr_ref': 'NBR 5356-3',
                        ...
                        if teste_nome in ['LI', 'LIC', 'SI']:
                            ...
                            for valor in teste_data.get('nbr', []):
                                teste_obj['ocr_kv_crista'][str(um_float)] = teste_data.get('nbr', [])
                        else:
                            teste_obj['ocr_kv_eficaz'] = {}
                            # Adiciona valores NBR
                            for valor in teste_data.get('nbr', []):
                                teste_obj['ocr_kv_eficaz'][str(um_float)] = teste_data.get('nbr', [])
    ...
    def _encontrar_nivel_isolamento(self, um_valor: float) -> List[Dict]:
        ...
        if not self.dados_json or 'insulation_levels' not in self.dados_json:
            return []
        try:
             return [nivel for nivel in self.dados_json['insulation_levels']
                     if nivel.get('um_kv') is not None and
                        math.isclose(safe_float_convert(nivel['um_kv']), um_valor)]
        except Exception as e:
    ...
    def get_tensao_aplicada_values(self, classe_tensao: Union[str, float]) -> List[Union[int, float]]:
    ...
    def get_tensao_induzida_values(self, classe_tensao: Union[str, float]) -> List[Union[int, float]]:
...
class TabelaTransformadorIEEE(NormaBase):
    ...
    def encontrar_tensao_proxima(self, voltage: float) -> Optional[float]:
        ...
        if voltage_float is None: return None
        ...
        for nivel in self.dados_json['insulation_levels']:
            if nivel.get('standard') == 'IEEE' and nivel.get('um_kv') is not None:
                ...
                if um_f is not None: all_ieee_voltages.add(um_f)
            ...
            if nom_v is not None:
                 ...
                 if nom_f is not None: all_ieee_voltages.add(nom_f)


        if not all_ieee_voltages: return None
    ...
    def get_bil_values(self, voltage: float) -> List[Union[int, float]]:
        ...
        if tensao_proxima is None: return []
        bil_values = set()
        for nivel in self.dados_json.get('insulation_levels', []):
             um_f = safe_float_convert(nivel.get('um_kv'))
             if nivel.get('standard') == 'IEEE' and um_f is not None and math.isclose(um_f, tensao_proxima):
                 bil = nivel.get('bil_kvp')
                 if bil is not None: bil_values.add(bil)
    ...
    def get_test_levels(self, voltage: float) -> Optional[Dict]:
        ...
        tensao_proxima = self.encontrar_tensao_proxima(voltage)
        if tensao_proxima is None: return None
        ...
        if not ieee_niveis: return None
...
class VerificadorTransformador:
    ...
    def __init__(self):
        ...
        try:
            ...
            log.info("Instâncias das normas NBR e IEEE criadas e dados JSON carregados com sucesso.")
        except Exception as e:
    ...
    def is_valid(self) -> bool:
        ...
        return self.nbr is not None and self.ieee is not None \
               and self.nbr.dados_json is not None and self.ieee.dados_json is not None
    ...
    def get_nearest_um_value(self, tensao: float) -> Optional[str]:
        ...
        if um_valor is None: return None

        all_um_values = set()
        for nivel in self.nbr.dados_json.get('insulation_levels', []):
             um_f = safe_float_convert(nivel.get('um_kv'))
             if um_f is not None: all_um_values.add(um_f)

        if not all_um_values: return str(tensao) # Fallback
        ...
        return str(int(closest_um_float)) if math.isclose(closest_um_float, round(closest_um_float)) else str(closest_um_float)
    ...
    def get_all_insulation_options(self, voltage_class: Union[str, float]) -> List[Dict[str, Any]]:
        ...
        all_options = []
        if not self.is_valid(): return all_options
        um_valor = safe_float_convert(voltage_class)
        if um_valor is None: return all_options

        niveis_encontrados = self._encontrar_nivel_isolamento(um_valor)
        if not niveis_encontrados: return all_options
    ...
    def get_clearances(self, voltage: Union[str, float], transformer_type: str = "distribution", ia: Union[str, float] = None, im: Union[str, float] = None) -> dict:
        """ Obtém espaçamentos comparativos de ambas as normas (usando JSON). """
        results = {"NBR": None, "IEEE": None}
        if not self.is_valid(): return results
        um_valor = safe_float_convert(voltage)
        if um_valor is None: return results
        ...
        for nivel in niveis_um:
            if nivel.get('standard') == 'IEC/NBR':
                 ...
                 match_bil = ia_valor is not None and bil_nivel is not None and math.isclose(bil_nivel, ia_valor)
                 match_sil = (im_valor is None and sil_nivel is None) or \
                             (im_valor is not None and sil_nivel is not None and math.isclose(sil_nivel, im_valor))
    ...
    def get_test_levels_comparison(self, voltage: Union[str, float], conexao=None, neutro_um=None) -> dict:
        """ Obtém níveis de teste comparativos (Tensão Aplicada/ACSD) usando JSON. """
        results = {"NBR": {"valores": [], "norma": "NBR 5356-3"}, "IEEE": {"valores": [], "norma": "IEEE C57.12.00"}}
        if not self.is_valid(): return results

        # Determina qual Um usar (principal ou neutro)
        um_busca = safe_float_convert(neutro_um) if conexao == 'YN' and neutro_um else safe_float_convert(voltage)
        if um_busca is None: return results
        ...
        for nivel in niveis_um:
            ...
            if acsd is not None:
                 if nivel.get('standard') == 'IEC/NBR':
                      if acsd not in results['NBR']['valores']: results['NBR']['valores'].append(acsd)
                 elif nivel.get('standard') == 'IEEE':
                      if acsd not in results['IEEE']['valores']: results['IEEE']['valores'].append(acsd)
    ...
    def get_required_tests_comparison(self, voltage: Union[str, float], tipo_isolamento: str = 'uniforme') -> dict:
        ...
        niveis_um = self._encontrar_nivel_isolamento(um_valor)
        if not niveis_um: return results
        ...
        def get_requirements(nivel_data):
            ...
            if nivel_data:
                ...
                req['impulso_atmosferico_onda_cortada'] = 'Requerido' if nivel_data.get('lic_kvp') else 'Não Aplicável'
                req['impulso_manobra'] = 'Requerido' if nivel_data.get('sil_kvp') or nivel_data.get('bsl_kvp') else 'Não Aplicável'
                req['tensao_aplicada'] = 'Requerido' if nivel_data.get('acsd_kv_rms') else 'Não Aplicável' # Mapeia para ACSD
                req['tensao_induzida_curta'] = 'Requerido' if nivel_data.get('acsd_kv_rms') else 'Não Aplicável'
                req['tensao_induzida_longa'] = 'Requerido' if nivel_data.get('acld_kv_rms') else 'Não Aplicável'
                # DP requerido pode ser lido diretamente
                req['descargas_parciais'] = 'Requerido' if nivel_data.get('pd_required') else 'Não Aplicável'
            return req
    ...
    def get_chopped_impulse_comparison(self, voltage: Union[str, float], ia: Union[str, float]) -> dict:
        """ Calcula e compara valores de impulso cortado (LIC) usando JSON. """
        results = {"NBR": None, "IEEE": None}
        if not self.is_valid(): return results
        um_valor = safe_float_convert(voltage)
        ia_valor = safe_float_convert(ia)
        if um_valor is None or ia_valor is None: return results

        niveis_um = self._encontrar_nivel_isolamento(um_valor)

        # --- NBR ---
        nbr_nivel = next((n for n in niveis_um if n.get('standard') == 'IEC/NBR' and n.get('bil_kvp') is not None and math.isclose(n['bil_kvp'], ia_valor)), None)
        if nbr_nivel:
        ...
        # --- IEEE ---
        ieee_nivel = next((n for n in niveis_um if n.get('standard') == 'IEEE' and n.get('bil_kvp') is not None and math.isclose(n['bil_kvp'], ia_valor)), None)
        if ieee_nivel:
    ...
    def get_test_sequences_from_profiles(self, id_combinacao: str) -> Optional[Dict[str, Any]]:
        ...
        if not perfis_aplicaveis: return None
