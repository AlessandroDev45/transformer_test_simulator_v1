...
def parse_resistor_expression(expression: str | None) -> tuple[float, dict]:
    ...
    if cap_match:
        try:
            ...
            log.info(f"Capacitor de acoplamento {cap_value*1e12:.1f} pF detectado e removido da expressão Rf/Rt.")
    ...
    try:
        ...
        expression_safe = re.sub(r'(\d+)\s*k\s*(\d+)', r'(\1 * 1000 + \2 * 100)', expression_safe) # 1k5 -> 1500
        expression_safe = re.sub(r'(\d+(?:\.\d+)?)\s*m', r'(\1 * 1000000)', expression_safe) # Para Mega Ohms (raro)
        ...
        def calculate_parallel(*args):
            ...
            for r in args:
                try:
                    r_float = float(r)
                    if r_float > 1e-9: # Considera resistências muito pequenas como circuito aberto no paralelo
                        inv_sum += 1.0 / r_float
                        valid_resistors_found = True
                except (ValueError, TypeError, ZeroDivisionError):
                    continue # Ignora valores inválidos
            if not valid_resistors_found: return float('inf') # Nenhum resistor válido no paralelo
        ...
        if not series_parts or all(not part.strip() for part in series_parts):
            ...
            if special_components: return 0.0, special_components # Retorna 0 Ohm se só tinha capacitor
            log.warning(f"Expressão de resistor '{expression}' sem partes válidas após processamento.")
        ...
        for part in series_parts:
            ...
            if not re.match(allowed_chars, part_no_space.replace('__parallel__','')):
                 ...
                 if not re.match(allowed_chars, ''.join(temp_part_for_check.split())):
                      log.error(f"Parte da expressão de resistor contém caracteres inválidos: '{part}'")
                      return float('inf'), special_components

            try:
                # Avalia a parte no ambiente seguro
                part_value = eval(part, {"__builtins__": {}}, safe_dict)
                if not isinstance(part_value, (int, float)) or part_value < 0 or math.isinf(part_value) or math.isnan(part_value):
                    log.warning(f"Parte da expressão resultou em valor inválido: {part} -> {part_value}")
                    return float('inf'), special_components
        ...
        log.debug(f"Expressão '{expression}' -> Resistência: {total_resistance:.3f} Ω, Especiais: {special_components}")
...
def get_generator_params(config_value: str) -> tuple[int, int, float, float]:
    ...
    config = next((item for item in constants.GENERATOR_CONFIGURATIONS if item['value'] == config_value), None)
    if config:
        try:
            ...
            return stages, parallel, max_voltage_kv, energy_kj
             ...
             log.error(f"Erro ao ler parâmetros da configuração '{config_value}': {e}. Usando padrão 12S-1P.")
    else:
        log.warning(f"Configuração do gerador '{config_value}' não encontrada. Usando padrão 12S-1P.")

    # Fallback para padrão 12S-1P
    default_config = next((item for item in constants.GENERATOR_CONFIGURATIONS if item['value'] == "12S-1P"), {})
    return default_config.get("stages", 12), default_config.get("parallel", 1), default_config.get("max_voltage_kv", 2400.0), default_config.get("energy_kj", 360.0)
...
def get_divider_capacitance(generator_max_voltage_kv: float) -> float:
    ...
    try:
        ...
        log.error(f"Erro ao obter capacitância do divisor para Vmax={generator_max_voltage_kv}: {e}. Usando padrão HIGH.")
...
def calculate_effective_gen_params(n_stages: int, n_parallel: int) -> tuple[float, float]:
    ...
    log.debug(f"Parâmetros Efetivos Gerador ({n_stages}S-{n_parallel}P): Cg={c_gen_effective*1e6:.3f} µF, Lg={l_gen_effective*1e6:.3f} µH")
    return c_gen_effective, l_gen_effective

# === Funções de Cálculo de Carga (de impulse.py) ===

def calculate_transformer_inductance(voltage_kv: float | None, power_mva: float | None, impedance_percent: float | None, freq_hz: float = constants.DEFAULT_FREQUENCY) -> float:
    ...
    try:
        ...
        log.info(f"Indutância do trafo calculada (Lcc): {l_cc:.4f} H (V={voltage_kv}kV, P={power_mva}MVA, Z={impedance_percent}%, f={freq_hz_f}Hz)")
        return l_cc
    except (ValueError, TypeError, ZeroDivisionError) as e:
        log.error(f"Erro ao calcular indutância do trafo: {e}. Usando padrão {default_inductance} H.")
        return default_inductance

def calculate_total_load_capacitance(c_dut_pf: float | None, c_stray_pf: float | None, impulse_type: str, generator_max_voltage_kv: float) -> float:
    ...
    log.debug(f"Carga Total: C_dut={c_dut_f*1e12:.0f}pF, C_div={c_divider_f*1e12:.0f}pF, C_stray={c_stray_f*1e12:.0f}pF, C_extra={c_load_extra_f*1e12:.0f}pF => C_load={c_load_total_f*1e9:.2f}nF")
    return c_load_total_f

# === Funções de Cálculo de Eficiência e Energia (de impulse.py) ===

def calculate_circuit_efficiency(c_gen_effective_f: float, c_load_total_f: float, impulse_type: str) -> tuple[float, float, float]:
    """Calcula a eficiência total, do circuito e de forma."""
    if c_gen_effective_f <= 0 or c_load_total_f < 0:
         log.warning(f"Capacitâncias inválidas para cálculo de eficiência: Cg_eff={c_gen_effective_f}, C_load={c_load_total_f}")
         return 0.0, 0.0, 0.0
    ...
    shape_efficiency = 0.95 if impulse_type in ["lightning", "chopped"] else 0.85 # Ajustado para SI
...
def calculate_energy_requirements(actual_test_voltage_kv: float | None, c_load_total_f: float | None) -> float:
    ...
    try:
        ...
        if test_voltage_v < 0: return 0.0

        energy_joules = 0.5 * c_load * (test_voltage_v**2)
        energy_kj = energy_joules / 1000
        log.debug(f"Energia requerida pela carga @ {actual_test_voltage_kv:.1f} kV (C={c_load*1e9:.2f} nF): {energy_kj:.2f} kJ")
        return energy_kj
...
def calculate_rlc_equivalent_params(n_stages: int, n_parallel: int,
                                   rf_per_column: float, rt_per_column: float,
                                   c_load_total_f: float,
                                   l_extra_h: float, l_transformer_h: float, l_inductor_h: float,
                                   inductance_factor: float = 1.0, tail_resistance_factor: float = 1.0
                                   ) -> dict | None:
    """
    ...
    """
    log.debug(f"Calculando RLC equiv. para {n_stages}S-{n_parallel}P, Rf={rf_per_column}, Rt={rt_per_column}")
    if n_stages <= 0 or n_parallel <= 0 or rf_per_column <= 0 or rt_per_column <= 0 or c_load_total_f < 0:
    ...
    try:
        ...
        if c_gen_effective <= 0: return None # Erro já logado em calculate_effective_gen_params

        c_eq = (c_gen_effective * c_load_total_f) / (c_gen_effective + c_load_total_f) if (c_gen_effective + c_load_total_f) > 1e-15 else 0.0
        ...
        log.debug(f"Componentes L inicial: Lgen={l_gen_effective:.3e}, Lext={l_extra_h:.3e}, Ltrafo={l_transformer_h:.3e}, Ladd={l_inductor_h:.3e} => Ltotal_init={l_total_initial:.3e} H")
        ...
        log.debug(f"Ajustes: L_factor={inductance_factor:.2f}, Rt_factor={tail_resistance_factor:.2f} => L_total={l_total:.3e} H, Rt_total={rt_total:.2f} Ohm")

        # Calcula constantes de tempo e parâmetros RLC
        # Alpha ~ 1 / (Rt * (Cg + Cl))
        alpha = 1.0 / (rt_total * (c_gen_effective + c_load_total_f)) if rt_total > 1e-9 and (c_gen_effective + c_load_total_f) > 1e-15 else 0.0
        # Beta ~ 1 / (Rf * Ceq)
        ...
        if beta <= alpha + 1e-9: # Adiciona pequena tolerância
            ...
            log.warning(f"Beta calculado ({beta:.2e}) <= Alpha ({alpha:.2e}). Ajustando Beta para {new_beta:.2e} para evitar singularidade.")
        ...
        log.debug(f"Parâmetros RLC Efetivos: Cg={c_gen_effective:.3e}F, Cl={c_load_total_f:.3e}F, Ceq={c_eq:.3e}F")
        log.debug(f"                       Rf={rf_total:.2f}Ω, Rt={rt_total:.2f}Ω, L={l_total:.3e}H")
        log.debug(f"Constantes: Alpha={alpha:.2e} s⁻¹, Beta={beta:.2e} s⁻¹")
        log.debug(f"Amortecimento: R_damp={r_damping:.2f}Ω, ω0={omega0:.2e} rad/s, ζ={zeta:.3f}, Oscilatório={is_oscillatory}")

        return {
            "c_gen_effective_f": c_gen_effective, "l_gen_effective_h": l_gen_effective,
            "c_load_total_f": c_load_total_f, "c_eq_f": c_eq,
            "rf_total_ohm": rf_total, "rt_total_ohm": rt_total,
            "l_total_h": l_total,
            "alpha": alpha, "beta": beta,
            "r_damping_ohm": r_damping, "omega0_rad_s": omega0, "zeta": zeta,
       ...
       if abs(delta / omega0_sq) < 1e-6: # Criticamente Amortecido (delta ≈ 0)
           ...
           if omega_d < 1e-6 * a: # Próximo ao crítico
            ...
            if s1 > 1e-9 or s2 > 1e-9:
                log.warning(f"Raízes instáveis ou zeradas na solução RLC sobreamortecida: s1={s1:.2e}, s2={s2:.2e}.")
...
def double_exp_func(t_sec: np.ndarray, V0_norm: float, alpha: float, beta: float) -> np.ndarray:
   ...
   try:
       ...
       if abs(val_at_peak) < 1e-12:
           log.warning("Valor no pico da dupla exponencial próximo de zero. Não é possível normalizar.")
           return v_out
...
def calculate_k_factor_transform(v_kv: np.ndarray, t_us: np.ndarray, return_params: bool = False) -> tuple:
   """
   ...
   # --- Validações Iniciais ---
   if not isinstance(v_kv, np.ndarray) or not isinstance(t_us, np.ndarray) or v_kv.ndim != 1 or t_us.ndim != 1 or len(v_kv) != len(t_us) or len(v_kv) < 10:
       log.warning("Dados de entrada inválidos para K-factor (tamanho/tipo).")
       return (v_test, v_base, v_residual_filtered, overshoot_rel, (alpha_fit, beta_fit)) if return_params else (v_test, v_base, v_residual_filtered, overshoot_rel)
   ...
   if abs(peak_value_orig) < 1e-3: # Check absolute value for negative pulses
       log.warning(f"Amplitude de pico ({peak_value_orig:.2e} kV) muito baixa para K-factor significativo.")
       return (v_test, v_base, v_residual_filtered, overshoot_rel, (alpha_fit, beta_fit)) if return_params else (v_test, v_base, v_residual_filtered, overshoot_rel)
   ...
   try:
       ...
       if len(crossed_indices) > 0:
           ...
           if idx1 < len(t_after_peak) and idx2 < len(t_after_peak):
               ...
               if abs(v2_tail - v1_tail) > 1e-9: # Avoid division by zero
                   t_half_sec = t1_tail + (v_half_target - v1_tail) * (t2_tail - t1_tail) / (v2_tail - v1_tail)
       ...
       def _double_exp_fit_func(t, A_norm, alpha, beta):
           try:
                ...
                t_peak = math.log(beta / alpha) / (beta - alpha)
                val_at_peak = math.exp(-alpha * t_peak) - math.exp(-beta * t_peak)
                if abs(val_at_peak) < 1e-12: return np.zeros_like(t)
       ...
       popt, pcov = curve_fit(
           _double_exp_fit_func, t_sec, v_kv, p0=p0,
           bounds=(lower_bounds, upper_bounds), maxfev=5000, method='trf'
       )

       A_fit, alpha_fit, beta_fit = popt
       log.debug(f"Ajuste Curva Base K-Factor: A={A_fit:.2f}, alpha={alpha_fit:.2e}, beta={beta_fit:.2e}")
       v_base = _double_exp_fit_func(t_sec, A_fit, alpha_fit, beta_fit)
       fit_success = True

   except (RuntimeError, ValueError, OptimizeWarning, Exception) as e:
       log.warning(f"Ajuste da curva base para K-Factor falhou: {e}. Usando onda original como base.")
   ...
   if fit_success and len(t_us) > 1:
        ...
        if dt_us > 1e-9:
            ...
            try:
                ...
                log.error(f"Erro durante FFT/Filtragem/IFFT no K-factor: {e_fft}. Resíduo filtrado será zero.")
                v_residual_filtered = np.zeros_like(v_residual)
        else:
             log.warning("Passo de tempo inválido ou ajuste base falhou. Resíduo filtrado K-factor será zero.")
             v_residual_filtered = np.zeros_like(v_residual)
   else: # If fit failed
        v_residual_filtered = np.zeros_like(v_residual)
   ...
   if peak_value_base > 1e-9:
       ...
       log.debug(f"Overshoot K-Factor: Vp_orig={peak_value_orig:.2f}, Vp_base={np.max(v_base):.2f}, MaxRes={max_residual:.2f} => Overshoot={overshoot_rel:.1f}%")
   else:
        overshoot_rel = 0.0
...
def analyze_lightning_impulse(t_us: np.ndarray, v_kv: np.ndarray) -> dict:
    ...
    results = {
        ...
        "conforme_overshoot": True, "conforme_pico": False, # Pico check needs target
    ...
    try:
        if v_kv is None or t_us is None or len(v_kv) < 10 or len(t_us) != len(v_kv) or np.std(v_kv) < 1e-9:
            results["error"] = "Dados de entrada inválidos ou insuficientes para análise LI."; log.error(results["error"]); results["status_geral"] = "Erro"; return results

        results["peak_value_measured"] = np.max(v_kv)

        # 1. Aplica K-Factor
        v_test_kv, v_base_kv, _, overshoot_rel, params_base = calculate_k_factor_transform(v_kv, t_us, return_params=True)
        results["overshoot_percent"] = overshoot_rel
        results["peak_value_base"] = np.max(v_base_kv) if v_base_kv is not None and len(v_base_kv) > 0 else 0.0
        if params_base: results["params_base_alpha"], results["params_base_beta"] = params_base

        if v_test_kv is None or len(v_test_kv) < 5 or np.std(v_test_kv) < 1e-6 * np.mean(np.abs(v_test_kv)):
            results["error"] = "Onda de ensaio (Vt) inválida após K-factor. Usando original."; log.warning(results["error"])
            v_test_kv = v_kv
        ...
        if results["peak_value_test"] > 1e-6:
            ...
            if len(v_before_kv) > 1:
                ...
                if v_max_b >= v_90_target and v_min_b <= v_30_target:
                    try:
                        t_30 = np.interp(v_30_target, v_before_kv, t_before_us, left=np.nan, right=np.nan)
                        t_90 = np.interp(v_90_target, v_before_kv, t_before_us, left=np.nan, right=np.nan)
                        results["t_30_us"] = t_30; results["t_90_us"] = t_90
                        if not np.isnan(t_30) and not np.isnan(t_90) and t_90 > t_30:
                            ...
                            if delta_t_front > 1e-9:
                                ...
                                if abs(slope) > 1e-9: t_0_virtual_us = t_30 - (v_30_target / slope)
                                else: t_0_virtual_us = t_30
                            results["t_0_virtual_us"] = t_0_virtual_us
                        else: log.warning("t30 ou t90 inválidos ou não crescentes.")
                    except Exception as e_interp_front: log.error(f"Erro interpolação frente LI: {e_interp_front}")
                else: log.warning(f"Níveis 30%/90% fora do range ({v_min_b:.2f}-{v_max_b:.2f}).")
            else: log.warning("Dados insuficientes antes do pico.")
        else: log.warning("Pico de Vt muito baixo.")
            ...
            try:
                ...
                if len(indices_below_50) > 0:
                    ...
                    if abs(v2_tail - v1_tail) > 1e-9:
                        t_50 = t1_tail + (v_50_target - v1_tail) * (t2_tail - t1_tail) / (v2_tail - v1_tail)
                    elif v1_tail >= v_50_target: t_50 = t1_tail
                    else: t_50 = t2_tail # Should ideally not happen if search is correct
                    if not np.isnan(t_50):
                        results["t_50_us"] = t_50
                        results["t_tail_us"] = t_50 - t_0_virtual_us
            except Exception as e_interp_tail: log.error(f"Erro interpolação cauda LI: {e_interp_tail}")
        else: log.warning("Dados insuficientes ou tensão não caiu para 50% na cauda.")

        # 5. Verificação de Conformidade
        t1 = results["t_front_us"]; t2 = results["t_tail_us"]
        vt = results["peak_value_test"]; beta = results["overshoot_percent"]
        if t1 is not None: results["conforme_frente"] = (constants.LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1 - constants.LIGHTNING_FRONT_TOLERANCE) <= t1 <= constants.LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1 + constants.LIGHTNING_FRONT_TOLERANCE))
        if t2 is not None: results["conforme_cauda"] = (constants.LIGHTNING_IMPULSE_TAIL_TIME_NOM * (1 - constants.LIGHTNING_TAIL_TOLERANCE) <= t2 <= constants.LIGHTNING_IMPULSE_TAIL_TIME_NOM * (1 + constants.LIGHTNING_TAIL_TOLERANCE))
        if beta is not None: results["conforme_overshoot"] = beta <= (constants.LIGHTNING_OVERSHOOT_MAX * 100.0)
        # Conformidade de pico é omitida por padrão, precisa de um alvo

        if results["conforme_frente"] and results["conforme_cauda"] and results["conforme_overshoot"]: results["status_geral"] = "Conforme"
        else: results["status_geral"] = "Não Conforme"

        log.info(f"Análise LI Concluída: Vt={vt:.2f}kV, T1={t1:.2f}µs, T2={t2:.1f}µs, β={beta:.1f}%, Status={results['status_geral']}")
        return results

    except Exception as e:
        log.exception(f"Erro geral em analyze_lightning_impulse: {e}")
        results["error"] = f"Erro inesperado: {str(e)}"; results["status_geral"] = "Erro"; return results
...
def analyze_switching_impulse(t_us: np.ndarray, v_kv: np.ndarray) -> dict:
    ...
    try:
        if v_kv is None or t_us is None or len(v_kv) < 10 or len(t_us) != len(v_kv) or np.std(v_kv) < 1e-9:
            results["error"] = "Dados de entrada inválidos ou insuficientes para análise SI."; log.error(results["error"]); results["status_geral"] = "Erro"; return results

        peak_index = np.argmax(v_kv); results["peak_time_us"] = t_us[peak_index]; results["peak_value_measured"] = v_kv[peak_index]
        t_origin_us = t_us[0]
        ...
        if len(indices_above_90) > 1:
            ...
            if 0 <= first_idx < len(t_us) and 0 <= last_idx < len(t_us): results["td_us"] = t_us[last_idx] - t_us[first_idx]
            else: log.warning("Índices Td fora dos limites.")
        else: log.warning("Não foi possível calcular Td (pontos >= 90%).")

        # --- Cálculo de T2 ---
        v_50_target = 0.5 * results["peak_value_measured"]
        mask_after_peak = t_us >= results["peak_time_us"]; t_after_us = t_us[mask_after_peak]; v_after_kv = v_kv[mask_after_peak]
        if len(v_after_kv) > 1 and np.min(v_after_kv) <= v_50_target + 1e-6:
            try:
                indices_below_50 = np.where(v_after_kv <= v_50_target)[0]
                if len(indices_below_50) > 0:
                    idx2 = indices_below_50[0]; idx1 = max(0, idx2 - 1)
                    t1_tail, v1_tail = t_after_us[idx1], v_after_kv[idx1]; t2_tail, v2_tail = t_after_us[idx2], v_after_kv[idx2]
                    if abs(v2_tail - v1_tail) > 1e-9: t_half = t1_tail + (v_50_target - v1_tail) * (t2_tail - t1_tail) / (v2_tail - v1_tail)
                    elif v1_tail >= v_50_target: t_half = t1_tail
                    else: t_half = t2_tail
                    if not np.isnan(t_half): results["t_half_us"] = t_half; results["t_2_us"] = t_half - t_origin_us
            except Exception as e: log.error(f"Erro interpolação T2 SI: {e}")
        else: log.warning("Tensão não caiu para 50% para cálculo T2.")
            ...
            try:
                ...
                if len(zero_cross_indices) > 0:
                    ...
                    if first_zero_idx_abs > 0:
                        ...
                        t1_z, v1_z = t_us[idx1_z], v_kv[idx1_z]; t2_z, v2_z = t_us[idx2_z], v_kv[idx2_z]
                        if v1_z > 0 and v2_z <= 0 and abs(v2_z - v1_z) > 1e-9: t_z = t1_z - v1_z * (t2_z - t1_z) / (v2_z - v1_z)
                        elif v2_z <= 0: t_z = t2_z
                        else: t_z = np.nan # Should not happen if logic is correct
                        if not np.isnan(t_z): results["t_z_us"] = t_z; results["t_zero_us"] = t_z - t_origin_us
            except Exception as e: log.error(f"Erro interpolação Tz SI: {e}")
        else: log.warning("Tensão não cruzou zero para cálculo Tz.")

        # --- Cálculo de Tp ---
        v_30_target = 0.3 * results["peak_value_measured"]; v_90_target_tp = 0.9 * results["peak_value_measured"]
        mask_before_peak = t_us <= results["peak_time_us"]; t_before_us = t_us[mask_before_peak]; v_before_kv = v_kv[mask_before_peak]
        ...
        if len(v_before_kv) > 1:
             ...
             if v_max_b >= v_90_target_tp and v_min_b <= v_30_target:
                 try:
                     t_30 = np.interp(v_30_target, v_before_kv, t_before_us, left=np.nan, right=np.nan)
                     t_90 = np.interp(v_90_target_tp, v_before_kv, t_before_us, left=np.nan, right=np.nan)
                     results["t_30_us"] = t_30; results["t_90_us"] = t_90
                     if not np.isnan(t_30) and not np.isnan(t_90) and t_90 > t_30: results["t_ab_us"] = t_90 - t_30
                 except Exception as e: log.error(f"Erro interpolação T_AB SI: {e}")
             else: log.warning("Níveis 30%/90% fora do range T_AB.")
        ...
        if results.get("t_ab_us") is not None and results.get("t_2_us") is not None:
            ...
            except Exception as e: log.error(f"Erro cálculo Tp fórmula K: {e}"); results["t_p_us"] = results["peak_time_us"] - t_origin_us
        else: results["t_p_us"] = results["peak_time_us"] - t_origin_us

        # --- Verificação de Conformidade ---
        tp, t2, td, tz = results["t_p_us"], results["t_2_us"], results.get("td_us"), results.get("t_zero_us")
        if tp is not None: results["conforme_tp"] = (constants.SWITCHING_IMPULSE_PEAK_TIME_NOM * (1 - constants.SWITCHING_PEAK_TIME_TOLERANCE) <= tp <= constants.SWITCHING_IMPULSE_PEAK_TIME_NOM * (1 + constants.SWITCHING_PEAK_TIME_TOLERANCE))
        if t2 is not None: results["conforme_t2"] = (constants.SWITCHING_IMPULSE_TAIL_TIME_NOM * (1 - constants.SWITCHING_TAIL_TOLERANCE) <= t2 <= constants.SWITCHING_IMPULSE_TAIL_TIME_NOM * (1 + constants.SWITCHING_TAIL_TOLERANCE))
        if td is not None: results["conforme_td"] = td >= constants.SWITCHING_TIME_ABOVE_90_MIN
        if tz is not None: results["conforme_tzero"] = tz >= constants.SWITCHING_TIME_TO_ZERO_MIN

        if results["conforme_tp"] and results["conforme_t2"] and results["conforme_td"] and results["conforme_tzero"]: results["status_geral"] = "Conforme"
        else: results["status_geral"] = "Não Conforme"

        log.info(f"Análise SI Concluída: Vp={results['peak_value_measured']:.1f}kV, Tp={tp:.1f}µs, T2={t2:.1f}µs, Td={td:.1f}µs, Tz={tz:.1f}µs, Status={results['status_geral']}")
        return results
...
def analyze_chopped_impulse(t_us: np.ndarray, v_kv: np.ndarray, chop_time_actual_us: float | None) -> dict:
    ...
    results = {
        "waveform_type": "LIC", "peak_value_full_wave_test": None, "peak_time_full_wave_us": None,
        "t_front_us": None, "t_0_virtual_us": None, "chop_time_us": None,
        "chop_time_absolute_us": None, "chop_voltage_test_kv": None,
        "chop_voltage_measured_kv": None, "undershoot_percent": 0.0,
        "t_30_us": None, "t_90_us": None,
        "conforme_frente": False, "conforme_corte": False, "conforme_undershoot": True, # Conformidade Vc omitida
        "status_geral": "Indeterminado", "error": None
    }

    try:
        if v_kv is None or t_us is None or len(v_kv) < 10 or len(t_us) != len(v_kv) or np.std(v_kv) < 1e-9:
            results["error"] = "Dados de entrada inválidos/insuficientes para análise LIC."; log.error(results["error"]); results["status_geral"] = "Erro"; return results
        if chop_time_actual_us is None or chop_time_actual_us <= t_us[0] or chop_time_actual_us >= t_us[-1]:
            results["error"] = f"Tempo de corte ({chop_time_actual_us}) inválido ou fora do range."; log.error(results["error"]); results["status_geral"] = "Erro"; return results
        ...
        if chop_start_index < 10:
             results["error"] = f"Tempo de corte ({chop_time_actual_us}µs) muito próximo do início."; log.error(results["error"]); results["status_geral"] = "Erro"; return results

        t_before_chop_us = t_us[:chop_start_index+1]; v_before_chop_kv = v_kv[:chop_start_index+1]
        v_test_before_chop, _, _, _, _ = calculate_k_factor_transform(v_before_chop_kv, t_before_chop_us, return_params=False)
        if v_test_before_chop is None or len(v_test_before_chop) < 5:
            results["error"] = "Falha K-factor na frente cortada."; log.error(results["error"]); results["status_geral"] = "Erro"; return results
        ...
        if results["peak_value_full_wave_test"] > 1e-6:
            v_30_t = 0.3 * results["peak_value_full_wave_test"]; v_90_t = 0.9 * results["peak_value_full_wave_test"]
            mask_f = t_before_chop_us <= results["peak_time_full_wave_us"]; t_f_us = t_before_chop_us[mask_f]; v_f_kv = v_test_before_chop[mask_f]
            if len(v_f_kv) > 1:
                ...
                if v_max_s >= v_90_t and v_min_s <= v_30_t:
                    try:
                        ...
                        results["t_30_us"] = t_30; results["t_90_us"] = t_90
                        if not np.isnan(t_30) and not np.isnan(t_90) and t_90 > t_30:
                            delta_t = t_90 - t_30; results["t_front_us"] = 1.67 * delta_t
                            if delta_t > 1e-9:
                                slope = (v_90_t - v_30_t) / delta_t
                                if abs(slope) > 1e-9: t_0_virtual_us_subj = t_30 - (v_30_t / slope)
                                else: t_0_virtual_us_subj = t_30
                            results["t_0_virtual_us"] = t_0_virtual_us_subj
                    except Exception as e: log.error(f"Erro interpolação frente LIC: {e}")
                else: log.warning("Níveis 30/90 não encontrados na frente subj.")
            else: log.warning("Dados insuficientes na frente subj.")
        else: log.warning("Pico da onda plena subj. muito baixo.")

        # --- Parâmetros do Corte ---
        if results["t_0_virtual_us"] is not None: results["chop_time_us"] = results["chop_time_absolute_us"] - results["t_0_virtual_us"]
        results["chop_voltage_test_kv"] = np.interp(results["chop_time_absolute_us"], t_before_chop_us, v_test_before_chop, left=np.nan, right=np.nan)
        ...
        if len(v_after_chop_kv) > 0:
            ...
            if min_after_chop < 0 and vc_ref is not None and abs(vc_ref) > 1e-6:
                ...
                log.debug(f"Undershoot LIC: Vmin={min_after_chop:.2f}, Vc={vc_ref:.2f} => Undershoot={results['undershoot_percent']:.1f}%")
            elif vc_ref is None or abs(vc_ref) < 1e-6: log.warning("Vc inválido para undershoot.")

        # --- Conformidade ---
        t1, tc, undershoot = results["t_front_us"], results["chop_time_us"], results["undershoot_percent"]
        if t1 is not None: results["conforme_frente"] = (constants.LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1 - constants.CHOPPED_FRONT_TOLERANCE) <= t1 <= constants.LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1 + constants.CHOPPED_FRONT_TOLERANCE))
        if tc is not None: results["conforme_corte"] = constants.CHOPPED_IMPULSE_CHOP_TIME_MIN <= tc <= constants.CHOPPED_IMPULSE_CHOP_TIME_MAX
        if undershoot is not None: results["conforme_undershoot"] = undershoot <= (constants.CHOPPED_UNDERSHOOT_MAX * 100.0)

        if results["conforme_frente"] and results["conforme_corte"] and results["conforme_undershoot"]: results["status_geral"] = "Conforme"
        else: results["status_geral"] = "Não Conforme"

        log.info(f"Análise LIC Concluída: Vc={results['chop_voltage_test_kv']:.1f}kV, T1_subj={t1:.2f}µs, Tc={tc:.2f}µs, Undershoot={undershoot:.1f}%, Status={results['status_geral']}")
        return results
...
def calculate_winding_temps(rc: float, tc: float, rw: float, ta: float, material: str) -> tuple[float | None, float | None]:
    ...
    try:
        ...
        log.debug(f"Calc Temps: Rc={rc:.3f}Ω, Tc={tc:.1f}°C, Rw={rw:.3f}Ω, Ta={ta:.1f}°C, C={C} => Θw={theta_w:.1f}°C, ΔΘw={delta_theta_w:.1f}K")
...
def calculate_top_oil_rise(t_oil: float, ta: float) -> float | None:
     ...
     try:
          ...
          log.debug(f"Calc Oil Rise: Toil={t_oil:.1f}°C, Ta={ta:.1f}°C => ΔΘoil={delta_theta_oil:.1f}K")
          return delta_theta_oil
     except Exception as e:
          log.exception(f"Erro ao calcular elevação do óleo: {e}")
          return None

def calculate_thermal_time_constant(ptot_kw: float | None, delta_theta_oil_max_k: float | None,
                                    mass_total_kg: float | None, mass_oil_kg: float | None) -> float | None:
    """
    ...
    try:
        ...
        if ptot <= 0 or delta_max <= 0 or mt <= 0 or mo < 0:
            log.error(f"Valores inválidos para cálculo de τ₀: Ptot={ptot}, ΔΘmax={delta_max}, mT={mt}, mO={mo}")
            return None

        # Verificação adicional para garantir que a massa total seja maior que a massa do óleo
        if mt < mo:
            log.warning(f"Massa total ({mt}t) menor que massa do óleo ({mo}t). Ajustando para cálculo.")
        ...
        log.info(f"Constante Térmica Óleo (τ₀) calculada: {tau0_h:.2f} h (Ptot={ptot:.1f}kW, ΔΘmax={delta_max:.1f}K, mT={mt:.1f}t, mO={mo:.1f}t)")
        return tau0_h
...
def calculate_short_circuit_params(corrente_nominal_a: float | None, impedancia_pu: float | None, k_peak_factor: float | None) -> tuple[float | None, float | None]:
    ...
    try:
        ...
        log.info(f"Cálculo Curto: In={in_a:.1f}A, Zpu={z_pu:.4f}, k√2={k_sqrt2:.2f} => Isc={isc_sym_ka:.2f}kA, ip={isc_peak_ka:.2f}kA")
        return isc_sym_ka, isc_peak_ka
...
def calculate_capacitive_load(capacitance_pf: float | None, voltage_v: float | None, frequency_hz: float | None) -> tuple[float | None, float | None, float | None]:
    ...
    try:
        ...
        if cap_f <= 0 or v_v < 0 or f_hz <= 0:
            log.error(f"Valores inválidos para cálculo capacitivo: C={cap_f}F, V={v_v}V, f={f_hz}Hz")
        ...
        log.debug(f"Carga Capacitiva: C={capacitance_pf:.1f}pF, V={v_v/1000:.2f}kV, f={f_hz:.1f}Hz => Zc={zc_ohm:.1f}Ω, I={current_ma:.2f}mA, Q={power_kvar:.3f}kVAr")
        return zc_ohm, current_ma, power_kvar
...
def interpolate_resistor_data(c_load_nf: float, reference_data: list[tuple[float, float]]) -> float | None:
    ...
    try:
        ...
        if c_load_nf <= c_loads[0]: return r_values[0]
        if c_load_nf >= c_loads[-1]: return r_values[-1]
...
def calculate_cap_bank_parameters(voltage_kv: float | None, power_mvar: float | None) -> tuple[float | None, float | None]:
    """
    ...
    try:
        ...
        if cap_bank_voltage_kv <= 23.9: factor = 0.25
        elif cap_bank_voltage_kv <= 71.7: factor = 0.75
        else: factor = 1.0

        if cap_bank_voltage_kv > 1e-6:
            voltage_ratio_sq = (v_kv / cap_bank_voltage_kv)**2
            denominator = voltage_ratio_sq * factor
            if denominator > 1e-9: pot_cap_bank_mvar = p_mvar / denominator
            else: log.warning("Denominador zero no cálculo da potência do Cap Bank."); pot_cap_bank_mvar = float('inf')
        else: log.error("Tensão nominal do Cap Bank calculada é zero."); pot_cap_bank_mvar = float('inf')

        log.debug(f"Cap Bank Calc: Vtest={v_kv:.2f}kV, Ptest={p_mvar:.2f}MVAr -> Vbank={cap_bank_voltage_kv:.1f}kV, Pbank={pot_cap_bank_mvar:.2f}MVAr")
        return cap_bank_voltage_kv, pot_cap_bank_mvar
...
def evaluate_eps_limits(voltage_kv: float | None, current_a: float | None,
                        active_power_kw: float | None, reactive_power_mvar: float | None,
                        cap_bank_voltage_kv: float | None) -> dict:
    """
    Avalia os parâmetros de ensaio contra os limites da fonte EPS e do banco.
    """
    status = "OK"; messages = []; event_count = 0
    params = {
        "Tensão": voltage_kv, "Corrente": current_a, "Pot. Ativa": active_power_kw,
        "Pot. Reativa Banco": reactive_power_mvar, "Tensão Banco": cap_bank_voltage_kv
    }
    valid_inputs = True
    for name, val in params.items():
        if val is None or (isinstance(val, float) and (math.isnan(val) or math.isinf(val))):
            ...
            valid_inputs = False
    if not valid_inputs: return {'status': 'Erro', 'messages': messages}

    if voltage_kv > constants.EPS_VOLTAGE_LIMIT_KV: messages.append(f"Tensão ({voltage_kv:.1f} kV) > Limite EPS ({constants.EPS_VOLTAGE_LIMIT_KV} kV)."); status = "Alerta"; event_count += 1
    if current_a > constants.EPS_CURRENT_LIMIT_A: messages.append(f"Corrente ({current_a:.1f} A) > Limite EPS ({constants.EPS_CURRENT_LIMIT_A} A)."); status = "Alerta"; event_count += 1
    if active_power_kw > constants.EPS_ACTIVE_POWER_LIMIT_KW: messages.append(f"Pot. Ativa ({active_power_kw:.1f} kW) > Limite EPS ({constants.EPS_ACTIVE_POWER_LIMIT_KW} kW)."); status = "Alerta"; event_count += 1
    if reactive_power_mvar > constants.EPS_REACTIVE_POWER_LIMIT_MVAR_HIGH: messages.append(f"Pot. Reativa Banco ({reactive_power_mvar:.1f} MVAr) > Limite Máximo ({constants.EPS_REACTIVE_POWER_LIMIT_MVAR_HIGH} MVAr). Ensaio IMPOSSÍVEL."); status = "Erro"; event_count += 1
    elif reactive_power_mvar > constants.EPS_REACTIVE_POWER_LIMIT_MVAR_LOW: messages.append(f"Pot. Reativa Banco ({reactive_power_mvar:.1f} MVAr) > Limite Inferior ({constants.EPS_REACTIVE_POWER_LIMIT_MVAR_LOW} MVAr). Requer aumento do banco."); status = "Alerta" if status != "Erro" else status; event_count += 1
    if cap_bank_voltage_kv is not None and voltage_kv > cap_bank_voltage_kv * 1.1:
        percent_above = ((voltage_kv / cap_bank_voltage_kv) - 1) * 100 if cap_bank_voltage_kv > 0 else 0
        messages.append(f"Tensão ({voltage_kv:.1f} kV) > 110% Vbanco ({cap_bank_voltage_kv:.1f} kV) - {percent_above:.1f}% acima."); status = "Alerta" if status != "Erro" else status; event_count += 1
    if event_count > 1: messages.append(f"Total de {event_count} alertas/erros.")
    if status == "OK": messages.append("Parâmetros dentro dos limites esperados.")

    log.debug(f"Avaliação Limites EPS: Status={status}, Mensagens={messages}")
    return {'status': status, 'messages': messages}

# --- Function for Bilinear Interpolation (Moved here) ---
def buscar_valores_tabela(inducao_teste: float | None, frequencia_teste: float | None, df: pd.DataFrame) -> float | None:
    ...
    if df.empty or inducao_teste is None or frequencia_teste is None:
        log.warning(f"DataFrame vazio ou inputs inválidos ({inducao_teste=}, {frequencia_teste=}) para buscar_valores_tabela.")
        return None
    if not isinstance(inducao_teste, (int, float)) or not isinstance(frequencia_teste, (int, float)):
         log.warning(f"Tipos inválidos para inducao/frequencia ({type(inducao_teste)}, {type(frequencia_teste)}).")
         return None
    ...
    try:
        ...
        try:
            ...
            q11 = df.loc[(ind_low, freq_low)].iloc[0] if isinstance(df.loc[(ind_low, freq_low)], pd.Series) else df.loc[(ind_low, freq_low)].iloc[0,0]
            q12 = df.loc[(ind_low, freq_high)].iloc[0] if isinstance(df.loc[(ind_low, freq_high)], pd.Series) else df.loc[(ind_low, freq_high)].iloc[0,0]
            q21 = df.loc[(ind_high, freq_low)].iloc[0] if isinstance(df.loc[(ind_high, freq_low)], pd.Series) else df.loc[(ind_high, freq_low)].iloc[0,0]
            q22 = df.loc[(ind_high, freq_high)].iloc[0] if isinstance(df.loc[(ind_high, freq_high)], pd.Series) else df.loc[(ind_high, freq_high)].iloc[0,0]
        except (KeyError, IndexError) as e:
             log.error(f"Erro ao acessar DataFrame em buscar_valores_tabela: {e}. Índices: ({ind_low},{freq_low}), etc.")
             return None
        ...
        log.debug(f"Interpolação ({col_name}): B={inducao_teste_clamped:.2f}T, f={frequencia_teste_clamped:.1f}Hz -> {valor_interpolado:.3f}")
        return valor_interpolado

    except Exception as e:
        log.exception(f"Erro durante interpolação bilinear para B={inducao_teste}, f={frequencia_teste}: {e}")
        return None


# === Funções de Simulação de Impulso (Híbrida) ===

def simulate_hybrid_impulse(t_sec: np.ndarray, v0_charge: float, rf: float, rt: float, l_total: float, c_gen: float, c_load: float, impulse_type: str, gap_distance_cm: float = None) -> tuple:
    """
