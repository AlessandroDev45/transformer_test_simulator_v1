...
from app_core.calculations import calculate_short_circuit_params, calculate_impedance_variation
from components.validators import validate_dict_inputs # Para validação
from components.formatters import formatar_curto_circuito, format_parameter_value # Formatadores
...
from dash import Input, Output # Adicionar Input, Output se não estiverem já importados globalmente
from dash.exceptions import PreventUpdate # Adicionar PreventUpdate
...
# O callback para atualizar o cache de dados do transformador está implementado
# na função short_circuit_update_transformer_info abaixo

# --- Define Styles Locally Using Imported Colors ---
# Define base styles first
BASE_STATUS_STYLE = {"fontWeight": "bold", "fontSize": "0.8rem", "padding": "2px 5px", "borderRadius": "3px"}

# Define specific status styles by merging base with colors
PASS_STYLE = {**BASE_STATUS_STYLE, "color": colors.get('pass', '#198754'), "backgroundColor": "#d1e7dd"} # Green text, light green bg
FAIL_STYLE = {**BASE_STATUS_STYLE, "color": colors.get('fail', '#dc3545'), "backgroundColor": "#f8d7da"} # Red text, light red bg
...
@app.callback(
    ...
    [Input("short-circuit-store", "data"),
     Input("url", "pathname")], # Triggered when store data is loaded/changed or URL changes
...
def short_circuit_load_data(stored_data, pathname):
    ...
    if status_text == "APROVADO":
        ...
        status_children = html.Span(f"ΔZ={delta_z:.2f}% | {status_text}", style=status_style_to_use)
    ...
    print(f"[LOAD ShortCircuit] Returning values: z_before={z_before}, z_after={z_after}, delta_z={delta_z}")
    print(f"[LOAD ShortCircuit] Returning figure: {fig}")

    return (z_before, z_after, peak_factor, isc_side, category,
            isc_sym, isc_peak, f"{delta_z:.2f}" if delta_z is not None else "", status_children, fig, "")
...
def short_circuit_calculate_and_verify(
    ...
    if pathname != "/curto-circuito":
        log.info(f"Ignorando callback short_circuit_calculate_and_verify em página diferente: {pathname}")
        raise PreventUpdate
    ...
    try:
        ...
        if corrente_nominal_a is None:
            ...
            if patch_mcp('transformer-inputs-store', serializable_data):
        ...
        return (round(isc_sym_ka, 2) if isc_sym_ka else None,
