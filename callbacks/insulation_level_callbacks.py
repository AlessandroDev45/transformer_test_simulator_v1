...
def register_insulation_level_callbacks(app_instance):
    ...
    def create_main_winding_insulation_callback(winding_prefix):
        ...
        def update_winding_insulation_options(pathname, norma_selecionada, um_kv_str_input):
            triggered_id = ctx.triggered_id if ctx.triggered else "initial_load_or_direct_call"
            log.info(f"[OPTIONS CB - {winding_prefix.upper()}] Acionado por: {triggered_id}. Norma: {norma_selecionada}, Um_kv: {um_kv_str_input}, Path: {pathname}")
    ...
    def create_neutral_insulation_callbacks(winding_prefix):
        ...
        def update_neutral_insulation_options(pathname, norma_selecionada, um_kv_neutral_str, connection_type):
            triggered_id = ctx.triggered_id if ctx.triggered else "initial_load_or_direct_call"
            log.info(f"[OPTIONS CB - {winding_prefix.upper()}-NEUTRO] Acionado por: {triggered_id}. Norma: {norma_selecionada}, Um_Neutro: {um_kv_neutral_str}, Conexão: {connection_type}")
