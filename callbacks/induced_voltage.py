...
def register_induced_voltage_callbacks(app_instance):
    ...
    def calculate_induced_voltage(
        ...
        try:
            ...
            log.debug(
                f"[Induced Voltage] Tensão de ensaio obtida de transformer-inputs-store.teste_tensao_induzida: {tensao_prova}"
            )
            if tensao_prova is None or tensao_prova <= 0:
                raise ValueError(
                    "A tensão de ensaio (Up) não foi encontrada nos dados do transformador ('teste_tensao_induzida'). Verifique os dados básicos."
                )
            ...
            if isinstance(current_store_data, dict):
                print(
                    f"  Chaves de current_store_data após update: {list(current_store_data.keys())}"
                )
            else:
                print(
                    f"  Conteúdo de current_store_data após update: {repr(current_store_data)[:100]}"
                )

            print("--- Fim [INDUCED VOLTAGE STORE SAVE DEBUG] ---")
