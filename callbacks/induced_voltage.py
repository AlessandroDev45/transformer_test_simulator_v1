...
potencia_magnet = {
    ...
    (1.4, 50): 0.95, (1.4, 60): 1.20, (1.4, 100): 2.50, (1.4, 120): 3.30, (1.4, 150): 4.50, (1.4, 200): 7.00, (1.4, 240): 9.00, # <-- Relevant for Mag interpolation
    (1.5, 50): 1.10, (1.5, 60): 1.40, (1.5, 100): 3.00, (1.5, 120): 4.00, (1.5, 150): 5.50, (1.5, 200): 9.00, (1.5, 240): 11.00, # <-- Relevant for Mag interpolation
...
perdas_nucleo = {
    ...
    (1.4, 50): 0.71, (1.4, 60): 0.95, (1.4, 100): 1.95, (1.4, 120): 2.80, (1.4, 150): 3.80, (1.4, 200): 6.00, (1.4, 240): 8.50,  # <-- Relevant for Perdas interpolation
    (1.5, 50): 0.85, (1.5, 60): 1.10, (1.5, 100): 2.30, (1.5, 120): 3.30, (1.5, 150): 4.50, (1.5, 200): 7.00, (1.5, 240): 10.00, # <-- Relevant for Perdas interpolation
...
def register_induced_voltage_callbacks(app_instance):
    ...
    @app_instance.callback(
        ...
        [
            ...
            # Add missing States for values used inside the callback but not passed as parameters
            # State("frequencia-nominal", "value"), # If this field exists in the layout
            # State("tensao-at", "value"),          # If this field exists in the layout
            # State("tensao-bt", "value"),          # If this field exists in the layout
            # State("inducao-nominal", "value")     # If this field exists in the layout
            # Assuming these values primarily come from the stores for now
        ],
        prevent_initial_call=True
    )
    def calculate_induced_voltage(n_clicks, transformer_data, losses_data, current_store_data, pathname, freq_teste_input, capacitancia_input, tipo_transformador_input):
        ...
        from utils.routes import normalize_pathname, ROUTE_INDUCED_VOLTAGE
        ...
        if clean_path != ROUTE_INDUCED_VOLTAGE:
            log.debug(f"[Induced Voltage] Não estamos na página de tensão induzida (pathname={pathname}, clean_path={clean_path}), prevenindo atualização")
            raise PreventUpdate
        ...
        try:
            # Obter valores dos campos de entrada usando callback_context
            # ctx = callback_context # Already defined above if needed
            # states = ctx.states if ctx else {} # Already defined above if needed
            ...
            freq_nominal = safe_float(transformer_data.get('frequencia'), 60) # Default 60 Hz

            # Frequência de teste (fp) - Directly from input parameter
            log.debug(f"[Induced Voltage] VALOR OBTIDO DO PARÂMETRO freq_teste_input: {freq_teste_input}, tipo: {type(freq_teste_input)}")
            try:
                ...
                else: freq_teste = None
            except (ValueError, TypeError): freq_teste = None
            ...
            tensao_prova = safe_float(transformer_data.get('teste_tensao_induzida'), None)
            log.debug(f"[Induced Voltage] Tensão de ensaio obtida de transformer-inputs-store.teste_tensao_induzida: {tensao_prova}")
            if tensao_prova is None or tensao_prova <= 0:
                raise ValueError("A tensão de ensaio (Up) não foi encontrada nos dados do transformador ('teste_tensao_induzida'). Verifique os dados básicos.")
            log.debug(f"[Induced Voltage] Tensão de ensaio (Up): {tensao_prova} kV")

            # Capacitância AT-GND (C) - Directly from input parameter
            log.debug(f"[Induced Voltage] Valor bruto da capacitância: {capacitancia_input}, tipo: {type(capacitancia_input)}")
            try:
                ...
                else: capacitancia = None
            except (ValueError, TypeError): capacitancia = None
            ...
            inducao_from_losses = safe_float(losses_data['resultados_perdas_vazio'].get('inducao'), None)
            if inducao_from_losses is None or inducao_from_losses <= 0:
                raise ValueError("Indução nominal não encontrada nos dados de perdas em vazio. Por favor, complete a seção de perdas em vazio.")
            inducao_nominal = inducao_from_losses
            log.debug(f"[Induced Voltage] Indução nominal obtida do losses-store: {inducao_nominal} T")

            # Peso do núcleo (m_core) - obrigatoriamente do losses_data
            peso_nucleo = safe_float(losses_data['resultados_perdas_vazio'].get('peso_nucleo'), None)
            if peso_nucleo is None or peso_nucleo <= 0:
                raise ValueError("Peso do núcleo não encontrado nos dados de perdas em vazio. Por favor, complete a seção de perdas em vazio.")
            ...
            perdas_vazio = safe_float(losses_data['resultados_perdas_vazio'].get('perdas_vazio_kw'), None)
            if perdas_vazio is None or perdas_vazio <= 0:
                raise ValueError("Perdas em vazio não encontradas nos dados de perdas em vazio. Por favor, complete a seção de perdas em vazio.")
            log.debug(f"[Induced Voltage] Perdas em vazio obtidas do losses-store: {perdas_vazio} kW")

            # Tipo do transformador - Directly from input parameter or transformer_data
            tipo_transformador = tipo_transformador_input if tipo_transformador_input else transformer_data.get('tipo_transformador', 'Trifásico')
            ...
            log.debug(f"[Induced Voltage] Tensão induzida: {tensao_induzida} kV (igual à tensão de ensaio)")
            ...
            if not all([tensao_at, freq_teste, freq_nominal, tensao_induzida, inducao_nominal]):
                 raise ValueError("Valores inválidos ou ausentes para cálculo da indução de teste (tensão AT, frequências, tensão de ensaio, indução nominal).")
            ...
            log.debug(f"[Induced Voltage] Indução no teste (β) calculada: {inducao_teste:.4f} T (Bn={inducao_nominal}, Up={tensao_induzida}, Un_AT={tensao_at}, fn={freq_nominal}, fp={freq_teste})")
            ...
            if tipo_transformador == 'Monofásico':
                ...
                log.debug(f"[Induced Voltage] Up/Un (Monofásico): {up_un:.4f} (Up={tensao_prova}, Un_ref=Un_AT={un_ref})")
            else: # Trifásico
                ...
                up_un = tensao_prova / un_ref if un_ref else 0
                log.debug(f"[Induced Voltage] Up/Un (Trifásico): {up_un:.4f} (Up={tensao_prova}, Un_ref=Un_AT_phase={un_ref:.2f})")
            ...
            if tipo_transformador == 'Monofásico':
                 ...
                 tensao_aplicada_bt = (tensao_bt / tensao_at) * tensao_prova if tensao_at else 0
            log.debug(f"[Induced Voltage] Tensão aplicada BT calculada: {tensao_aplicada_bt:.2f} kV")
            ...
            def buscar_valores_tabela(inducao_teste, frequencia_teste, df):
                ...
                frequencia_teste_clipped = max(min(frequencia_teste, max(frequencias)), min(frequencias))
                ...
                if ind_high == ind_low: x = 0.0
                else: x = (inducao_teste - ind_low) / (ind_high - ind_low)

                if freq_high == freq_low: y = 0.0
                else: y = (frequencia_teste - freq_low) / (freq_high - freq_low)

                # Bilinear interpolation formula
                valor_interpolado = (1-x)*(1-y)*q11 + x*(1-y)*q21 + (1-x)*y*q12 + x*y*q22

                log.debug(f"Interpolação para B={inducao_teste:.3f}, f={frequencia_teste:.1f}: Pontos ({ind_low},{freq_low})={q11}, ({ind_low},{freq_high})={q12}, ({ind_high},{freq_low})={q21}, ({ind_high},{freq_high})={q22}. Pesos x={x:.3f}, y={y:.3f}. Resultado={valor_interpolado:.4f}")
                return valor_interpolado

            # --- Obtenção dos Fatores das Tabelas ---
            log.debug(f"[Induced Voltage] Buscando valores nas tabelas para beta_teste={beta_teste:.4f} T e freq_teste={freq_teste:.1f} Hz")
            fator_potencia_mag = buscar_valores_tabela(beta_teste, freq_teste, df_potencia_magnet) # VAr/kg
            fator_perdas = buscar_valores_tabela(beta_teste, freq_teste, df_perdas_nucleo)          # W/kg
            log.debug(f"[Induced Voltage] Valores interpolados: fator_potencia_mag={fator_potencia_mag:.2f} VAr/kg, fator_perdas={fator_perdas:.2f} W/kg")
            ...
            if tipo_transformador == 'Monofásico':
                ...
                log.debug(f"[Monofásico] Potência ativa (Pw): {pot_ativa:.2f} kW (fator_perdas={fator_perdas:.2f} W/kg, peso_kg={peso_nucleo_kg})")

                # Potência Magnética (Sm) = Fator Pot Mag [VAr/kg] * Peso Núcleo [kg] / 1000 [kVAr] -> should be kVA
                pot_magnetica = fator_potencia_mag * peso_nucleo_kg / 1000.0 # This is technically kVAr, but often referred to as kVA in this context
                log.debug(f"[Monofásico] Potência magnética (Sm): {pot_magnetica:.2f} kVA (fator_pot_mag={fator_potencia_mag:.2f} VAr/kg, peso_kg={peso_nucleo_kg})")
                ...
                log.debug(f"[Monofásico] Tensão para cálculo Scap (U_calc_scap): {u_calc_scap:.2f} kV (tensao_prova={tensao_prova}, up_un={up_un}, tensao_bt={tensao_bt})")
                ...
                pcap = - (((u_calc_scap * 1000)**2 * 2 * math.pi * freq_teste * capacitancia * 1e-12) / 3)/1000 # Convertendo para kVAr
                log.debug(f"[Monofásico] Potência capacitiva (Scap): {pcap:.2f} kVAr cap (U={u_calc_scap} kV, f={freq_teste} Hz, C={capacitancia} pF)")
                ...
                log.debug(f"[Trifásico] Potência magnética total (Sm): {pot_magnetica_total:.2f} kVA")
                ...
                log.debug(f"[Trifásico] Corrente de excitação (Iexc): {corrente_excitacao:.2f} A (Sm={pot_magnetica_total:.2f} kVA, U_aplicada_bt={tensao_aplicada_bt:.2f} kV)")
            ...
            param_cell_style = {"backgroundColor": "#34495e", "color": "#ecf0f1", "fontWeight": "500", "fontSize": "0.8rem", "padding": "0.3rem 0.5rem", "textAlign": "left", "borderRight": "1px solid #2c3e50", "fontFamily": "Arial, sans-serif"}
            value_cell_style = {"backgroundColor": "#34495e", "color": "#ecf0f1", "fontSize": "0.8rem", "padding": "0.3rem 0.5rem", "textAlign": "right", "fontFamily": "Consolas, monospace", "letterSpacing": "0.02rem"}
            unit_cell_style = {"backgroundColor": "#34495e", "color": "#bdc3c7", "fontSize": "0.75rem", "padding": "0.3rem 0.5rem", "textAlign": "left", "fontStyle": "italic", "width": "60px"}
            card_header_style = {"backgroundColor": COLORS.get('primary', '#3498db'), "color": COLORS['text_light'], "fontWeight": "bold", "fontSize": "0.9rem", "padding": "0.4rem 0.5rem", "borderBottom": "1px solid " + COLORS['border'], "borderRadius": "4px 4px 0 0"}
            card_style = {"backgroundColor": COLORS['background_card'], "border": "1px solid " + COLORS['border'], "borderRadius": "4px", "boxShadow": "0 2px 4px rgba(0,0,0,0.2)", "height": "100%"}
            section_title_style = {"fontSize": "0.85rem", "color": "#3498db", "fontWeight": "bold", "marginBottom": "0.5rem"} # Added margin
            table_style = {"fontSize": "0.8rem", "color": "#ecf0f1"}
            recommendation_card_style = {"backgroundColor": COLORS['background_card'], "border": "1px solid " + COLORS['border'], "borderRadius": "4px", "boxShadow": "0 2px 4px rgba(0,0,0,0.2)", "height": "100%"}
            recommendation_header_style = card_header_style
            recommendation_item_style = {"fontSize": "0.75rem", "padding": "0.2rem 0.4rem", "color": "#000000", "backgroundColor": "#f8f9fa", "borderBottom": "1px solid " + COLORS['border'], "lineHeight": "1.3"}
            ...
            parametros_entrada = dbc.Col([
                html.H6("Parâmetros de Entrada", className="text-center", style=section_title_style),
                dbc.Table(html.Tbody([
                    ...
                    html.Tr([html.Td("Perdas em Vazio", style=param_cell_style), html.Td(format_parameter_value(perdas_vazio, 1), style=value_cell_style), html.Td("kW", style=unit_cell_style)]),
                    html.Tr([html.Td("Peso do Núcleo", style=param_cell_style), html.Td(format_parameter_value(peso_nucleo, 1), style=value_cell_style), html.Td("Ton", style=unit_cell_style)])
                ]), bordered=True, hover=True, size="sm", className="table-dark mb-0", style=table_style)
            ], md=4)

            # Tabela: Parâmetros do Ensaio
            parametros_ensaio = dbc.Col([
                html.H6("Parâmetros do Ensaio", className="text-center", style=section_title_style),
                dbc.Table(html.Tbody([
            ...
            if tipo_transformador == 'Monofásico':
                resultados_calculados = dbc.Col([
                    html.H6("Resultados (Monofásico)", className="text-center", style=section_title_style),
                    dbc.Table(html.Tbody([
                        ...
                        html.Tr([html.Td("Potência Reativa Magnética Sm", style=param_cell_style), html.Td(format_parameter_value(results_data.get('pot_magnetica'), 2), style=value_cell_style), html.Td("kVA", style=unit_cell_style)]), # Labelled kVA as per image
                        ...
                        html.Tr([html.Td("Potência Capacitiva Scap", style={**param_cell_style, "fontWeight": "bold"}), html.Td(format_parameter_value(results_data.get('pcap'), 2), style={**value_cell_style, "fontWeight": "bold", "color": "red"}), html.Td("kVAr cap", style=unit_cell_style)])
                    ]), bordered=True, hover=True, size="sm", className="table-dark mb-0", style=table_style)
                ], md=4)
            else:  # Trifásico
                resultados_calculados = dbc.Col([
                    html.H6("Resultados (Trifásico)", className="text-center", style=section_title_style),
                    dbc.Table(html.Tbody([
                         html.Tr([html.Td("Tensão Aplicada BT", style=param_cell_style), html.Td(format_parameter_value(results_data.get('tensao_aplicada_bt'), 1), style=value_cell_style), html.Td("kV", style=unit_cell_style)]),
                         html.Tr([html.Td("Potência Ativa Pw", style=param_cell_style), html.Td(format_parameter_value(results_data.get('pot_ativa'), 2), style=value_cell_style), html.Td("kW", style=unit_cell_style)]),
                         ...
                         html.Tr([html.Td("Potência de Teste (Total)", style={**param_cell_style, "fontWeight": "bold"}), html.Td(format_parameter_value(results_data.get('potencia_teste'), 2), style={**value_cell_style, "fontWeight": "bold", "backgroundColor": "#FFF3CD" if results_data.get('potencia_teste', 0) > 1500 else ""}), html.Td("kVA", style=unit_cell_style)])
                ...
                if results_data.get('potencia_teste', 0) > 1500:
                    alert_content.append(html.Li("Potência de Teste > 1500 kVA: Pode exceder capacidade da fonte.", style={"fontSize": "0.7rem", "color": "#856404", "marginBottom": "0.1rem"}))
                if results_data.get('tensao_aplicada_bt', 0) > 140: # Example threshold
                     alert_content.append(html.Li(f"Tensão Aplicada em BT ({results_data.get('tensao_aplicada_bt', 0):.1f} kV) pode ser alta.", style={"fontSize": "0.7rem", "color": "#856404", "marginBottom": "0.1rem"}))

                if alert_content:
                    resultados_calculados = html.Div([
                        resultados_calculados,
                        html.Div([
                            html.P("* Atenção aos valores:", style={"fontSize": "0.7rem", "color": "#856404", "marginBottom": "0.2rem", "marginTop": "0.5rem"}),
                            html.Ul(alert_content, style={"paddingLeft": "1rem", "marginBottom": "0.2rem"})
                        ], className="mt-2")
                    ])
            ...
            results_table = dbc.Card([
                dbc.CardHeader(
                    html.H6(f"Resultados do Cálculo - {tipo_transformador}", className="m-0 text-center"),
            ...
            if tipo_transformador == 'Monofásico':
                ...
                potencia_total_kVA_recomendada = max(pot_magnetica, pot_ativa + pcap) * 1.2 # Based on image text, seems to consider worst case summation?
                ...
                if potencia_total_kVA_recomendada > 100: classe_potencia = "grande porte"
                elif potencia_total_kVA_recomendada > 50: classe_potencia = "médio porte"

                recommendations = dbc.Card([
                    dbc.CardHeader(html.H6("Recomendações para o Teste (Monofásico)", className="m-0"), style=recommendation_header_style),
                    dbc.CardBody([
                        html.P("Recomendações para o ensaio:", className="mb-1", style={"fontSize": "0.75rem", "fontWeight": "500", "color": "#000000", "backgroundColor": "#f8f9fa", "padding": "0.2rem 0.4rem"}),
                        dbc.ListGroup([
                            dbc.ListGroupItem(f"Utilizar fonte de {freq_teste:.1f} Hz com capacidade mínima de {potencia_total_kVA_recomendada:.1f} kVA ({classe_potencia})", style=recommendation_item_style),
                            dbc.ListGroupItem(f"Tensão de saída ajustável até {tensao_saida_recomendada:.1f} kV", style=recommendation_item_style),
                            dbc.ListGroupItem(f"Potência ativa mínima de {pot_ativa_recomendada:.1f} kW", style=recommendation_item_style),
                            dbc.ListGroupItem(f"Potência reativa indutiva de {pot_indutiva_recomendada:.1f} kVAr ind", style=recommendation_item_style),
                            dbc.ListGroupItem(f"Potência reativa capacitiva de {pot_capacitiva_recomendada:.1f} kVAr cap", style=recommendation_item_style),
                            dbc.ListGroupItem("Atenção à ressonância devido à alta potência capacitiva.", style=recommendation_item_style)
                        ], flush=True)
                ...
                recommendations = dbc.Card([
                    dbc.CardHeader(html.H6("Recomendações para o Teste (Trifásico)", className="m-0"), style=recommendation_header_style),
                    dbc.CardBody([
                         html.P("Recomendações para o ensaio:", className="mb-1", style={"fontSize": "0.75rem", "fontWeight": "500", "color": "#000000", "backgroundColor": "#f8f9fa", "padding": "0.2rem 0.4rem"}),
                         dbc.ListGroup([
                            dbc.ListGroupItem(f"Utilizar fonte de {freq_teste:.1f} Hz com capacidade mínima de {potencia_total_kVA_recomendada:.1f} kVA ({classe_potencia})", style=recommendation_item_style),
                            dbc.ListGroupItem(f"Tensão de saída ajustável até {tensao_saida_recomendada:.1f} kV", style=recommendation_item_style),
                            dbc.ListGroupItem(f"Corrente nominal mínima de {corrente_min_recomendada:.1f} A", style=recommendation_item_style),
                            dbc.ListGroupItem(f"Potência magnética de {pot_magnetica_total:.1f} kVA", style=recommendation_item_style), # Sm
                            dbc.ListGroupItem("Monitorar corrente e tensão durante o teste.", style=recommendation_item_style),
                            dbc.ListGroupItem(f"Verificar capacidade da fonte vs. Potência de Teste ({potencia_teste:.1f} kVA).", style=recommendation_item_style)
                         ], flush=True)
                    ], style={"padding": "0.4rem", "backgroundColor": "#f8f9fa"})
                ], style=recommendation_card_style)
            ...
            data_for_store = {
                'inputs': {
                    'freq_nominal': freq_nominal, 'freq_teste': freq_teste,
                    'tensao_at': tensao_at, 'tensao_bt': tensao_bt,
                    'tensao_prova': tensao_prova, 'capacitancia': capacitancia,
                    'inducao_nominal': inducao_nominal,
                    'peso_nucleo_ton': peso_nucleo, 'perdas_vazio_kw': perdas_vazio,
                    'tipo_transformador': tipo_transformador
                },
            ...
            print(f"\n--- [INDUCED VOLTAGE STORE SAVE DEBUG] ---")
            log.info("--- [INDUCED VOLTAGE STORE SAVE DEBUG] ---")
            ...
            print(f"--- Fim [INDUCED VOLTAGE STORE SAVE DEBUG] ---")
            ...
            error_msg = html.Div(
                f"Erro ao calcular tensão induzida: {str(e)}",
                className="alert alert-danger"
            )
    ...
    def generate_frequency_table(generate_clicks, clear_clicks, transformer_data, losses_data, current_store_data, tipo_transformador_input, capacitancia_input):
        ...
        log.debug(f"[Induced Voltage] Gerando tabela de frequências: generate_clicks={generate_clicks}")

        try:
            # Adicionar logs para diagnóstico
            log.debug(f"[Induced Voltage] Dados para tabela de frequências: transformer_data={transformer_data}, losses_data={losses_data}, current_store_data={current_store_data}")

            # Verificar se temos os dados necessários
            if transformer_data is None or losses_data is None or current_store_data is None:
                log.warning(f"[Induced Voltage] Dados insuficientes para gerar tabela de frequências: transformer_data={transformer_data}, losses_data={losses_data}, current_store_data={current_store_data}")
                return html.Div("Dados insuficientes para gerar a tabela. Por favor, preencha todos os campos necessários e execute o cálculo de tensão induzida primeiro.",
            ...
            if 'inputs' not in current_store_data or 'resultados' not in current_store_data:
                log.warning(f"[Induced Voltage] Cálculo de tensão induzida não foi executado: current_store_data={current_store_data}")
                return html.Div("Por favor, execute o cálculo de tensão induzida antes de gerar a tabela de frequências.",
                               className="alert alert-warning")
            ...
            tipo_transformador = inputs.get('tipo_transformador', tipo_transformador_input or transformer_data.get('tipo_transformador', 'Monofásico'))
            log.debug(f"[Induced Voltage] Tipo de transformador para tabela de frequências: {tipo_transformador}")

            # Verificar se o tipo de transformador é válido
            if tipo_transformador not in ['Monofásico', 'Trifásico']:
                return html.Div(f"A tabela de frequências não está disponível para o tipo de transformador '{tipo_transformador}'.",
            ...
            tensao_prova = float(inputs.get('tensao_prova', transformer_data.get('teste_tensao_induzida', 0)))
            capacitancia = float(inputs.get('capacitancia', capacitancia_input or 0))

            # Verificar se temos todos os parâmetros necessários
            missing_params = []
            if tensao_at <= 0: missing_params.append("Tensão AT")
            if tensao_bt <= 0: missing_params.append("Tensão BT")
            if freq_nominal <= 0: missing_params.append("Frequência Nominal")
            if inducao_nominal <= 0: missing_params.append("Indução Nominal")
            if peso_nucleo <= 0: missing_params.append("Peso do Núcleo")
            if tensao_prova <= 0: missing_params.append("Tensão de Prova")
            if capacitancia <= 0: missing_params.append("Capacitância AT-GND")

            if missing_params:
                return html.Div(f"Parâmetros insuficientes para gerar a tabela. Faltam: {', '.join(missing_params)}. Execute o cálculo de tensão induzida primeiro.",
                               className="alert alert-warning")
            ...
            log.debug(f"[Induced Voltage] Parâmetros para tabela de frequências: tensao_at={tensao_at}, tensao_bt={tensao_bt}, freq_nominal={freq_nominal}, inducao_nominal={inducao_nominal}, peso_nucleo={peso_nucleo}, tensao_prova={tensao_prova}, capacitancia={capacitancia}")
            log.debug(f"[Induced Voltage] Fatores de referência: fator_potencia_mag={fator_potencia_mag_ref}, fator_perdas={fator_perdas_ref}")
            ...
            for freq_teste in frequencias:
                ...
                if beta_teste > 1.9:
                    beta_teste = 1.9
                    log.warning(f"[Induced Voltage] Indução no teste limitada a 1.9T para frequência {freq_teste} Hz")
                ...
                log.debug(f"[Induced Voltage] Frequência {freq_teste} Hz: beta_teste={beta_teste:.4f} T, fator_potencia_mag={fator_potencia_mag:.2f} VAr/kg, fator_perdas={fator_perdas:.2f} W/kg")
                ...
                if pot_magnetica**2 < pot_ativa**2:
                    log.warning(f"[Induced Voltage] Frequência {freq_teste} Hz: Potência magnética ao quadrado ({pot_magnetica**2:.2f}) menor que potência ativa ao quadrado ({pot_ativa**2:.2f}). Sind será zero.")
                    pot_induzida = 0.0
                else:
                    pot_induzida = math.sqrt(pot_magnetica**2 - pot_ativa**2)
            ...
            if tipo_transformador == 'Monofásico':
                table_header = [
                    html.Thead(html.Tr([
                        html.Th("Frequência (Hz)", style={"backgroundColor": "#34495e", "color": "#ecf0f1", "textAlign": "center"}),
                        html.Th("Potência Ativa Pw (kW)", style={"backgroundColor": "#34495e", "color": "#ecf0f1", "textAlign": "center"}),
                        html.Th("Potência Reativa Magnética Sm (kVA)", style={"backgroundColor": "#34495e", "color": "#ecf0f1", "textAlign": "center"}),
                        html.Th("Componente Indutiva Sind (kVAr ind)", style={"backgroundColor": "#34495e", "color": "#ecf0f1", "textAlign": "center"}),
                        html.Th("Potência Capacitiva Scap (kVAr cap)", style={"backgroundColor": "#34495e", "color": "#ecf0f1", "textAlign": "center"}),
                        html.Th("Scap/Sind", style={"backgroundColor": "#34495e", "color": "#ecf0f1", "textAlign": "center"})
                    ]))
                ]
                ...
                for row in table_data:
                    ...
                    table_rows.append(html.Tr([
                        ...
                        html.Td(format_parameter_value(abs(row['pcap']), 2), style={"textAlign": "center", "color": "red"}),
            ...
            # Adicionar logs para diagnóstico
            log.debug(f"[Induced Voltage] Tabela de frequências gerada com sucesso: {len(table_data)} linhas")
            ...
            log.debug(f"[Induced Voltage] Criando layout com tipo_transformador={tipo_transformador}")
            ...
            result_div = html.Div([
                ...
                html.P(f"Resultados calculados para transformador {tipo_texto} com diferentes frequências de teste:", className="text-muted"),
                dbc.Row([
            ...
            return html.Div([
                ...
                html.Pre(error_traceback, style={"fontSize": "0.7rem", "backgroundColor": "#f8f9fa", "padding": "0.5rem", "maxHeight": "200px", "overflow": "auto"})
            ], className="alert alert-danger")
