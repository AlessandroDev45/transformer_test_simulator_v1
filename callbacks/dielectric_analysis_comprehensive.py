...
def force_load_dielectric_data(n_clicks, transformer_data):
    ...
    log.info(f"[FORCE LOAD] Forçando carregamento dos dados da análise dielétrica. n_clicks: {n_clicks}")
    ...
    if not transformer_data or not isinstance(transformer_data, dict):
        log.warning("[FORCE LOAD] Dados do transformador inválidos ou vazios. Criando dados padrão.")
        transformer_data = {
    ...
    def determine_connection(conn_key, neutral_volt_key):
        try:
            ...
            if 'triangulo' in conn_lower or 'delta' in conn_lower or conn_lower.startswith('d'):
                ...
                if neutral_voltage and str(neutral_voltage).strip() and float(str(neutral_voltage).strip() or 0) > 0:
                    return 'YN'
                return 'Y'
            else:
                return default_conn
    ...
    nbi_terciario = safe_str_output(transformer_data.get('nbi_terciario'), get_default_nbi(um_terciario))
    ...
    nbi_neutro_at = safe_str_output(transformer_data.get('nbi_neutro_at'), get_neutro_nbi(nbi_at, conexao_at))
    nbi_neutro_bt = safe_str_output(transformer_data.get('nbi_neutro_bt'), get_neutro_nbi(nbi_bt, conexao_bt))
    nbi_neutro_terciario = safe_str_output(transformer_data.get('nbi_neutro_terciario'), get_neutro_nbi(nbi_terciario, conexao_terciario))
    ...
    sil_bt = safe_str_output(transformer_data.get('sil_bt'), '0')  # Geralmente não aplicável para BT
    sil_terciario = safe_str_output(transformer_data.get('sil_terciario'), '0')  # Geralmente não aplicável para terciário
    ...
    tensao_aplicada_at = safe_str_output(transformer_data.get('teste_tensao_aplicada_at'), get_default_tensao_aplicada(um_at))
    tensao_aplicada_bt = safe_str_output(transformer_data.get('teste_tensao_aplicada_bt'), get_default_tensao_aplicada(um_bt))
    tensao_aplicada_terciario = safe_str_output(transformer_data.get('teste_tensao_aplicada_terciario'), get_default_tensao_aplicada(um_terciario))
    ...
    tensao_induzida_at = safe_str_output(transformer_data.get('teste_tensao_induzida_at'), get_default_tensao_induzida(um_at))
    tensao_induzida_bt = safe_str_output(transformer_data.get('teste_tensao_induzida_bt'), get_default_tensao_induzida(um_bt))
    tensao_induzida_terciario = safe_str_output(transformer_data.get('teste_tensao_induzida_terciario'), get_default_tensao_induzida(um_terciario))
    ...
    log.info(f"[FORCE LOAD] Dados gerados para AT: Um={um_at}, Conexão={conexao_at}, NBI={nbi_at}, SIL={sil_at}, IAC={iac_at}")
    log.info(f"[FORCE LOAD] Dados gerados para BT: Um={um_bt}, Conexão={conexao_bt}, NBI={nbi_bt}, SIL={sil_bt}, IAC={iac_bt}")
    log.info(f"[FORCE LOAD] Dados gerados para Terciário: Um={um_terciario}, Conexão={conexao_terciario}, NBI={nbi_terciario}, SIL={sil_terciario}, IAC={iac_terciario}")
    log.info(f"[FORCE LOAD] Tensões Aplicadas: AT={tensao_aplicada_at}, BT={tensao_aplicada_bt}, Terciário={tensao_aplicada_terciario}")
    log.info(f"[FORCE LOAD] Tensões Induzidas: AT={tensao_induzida_at}, BT={tensao_induzida_bt}, Terciário={tensao_induzida_terciario}")
...
def display_selected_parameters(dieletric_data):
    ...
    """
    log.info(f"[DISPLAY SELECTED PARAMS] Exibindo parâmetros selecionados")

    if not dieletric_data or not isinstance(dieletric_data, dict):
        return html.Div([
            html.P("Nenhum dado de análise dielétrica encontrado. Volte à página de Análise Dielétrica e preencha os campos.",
    ...
    try:
        ...
        if not enrolamentos:
            return html.P("Nenhum enrolamento encontrado nos dados de análise dielétrica.", className="text-warning")
        ...
        for enrol in enrolamentos:
            ...
            if nome and um:
                ...
                header_bg_color = COLORS['primary'] if 'Alta' in nome else (
                    COLORS['secondary'] if 'Baixa' in nome else COLORS['info']
                )
                ...
                params = [
                    ...
                    {"label": "SIL/IM (kV)", "value": im if im and im != "NA" and im != "0" else "N/A"},
                    {"label": "Tensão Aplicada", "value": f"{tensao_curta} kV" if tensao_curta else "N/A"}
                ]
        ...
        if not at_card:
            at_card = html.Div([
                html.Div("Alta Tensão", style={**header_style, "backgroundColor": COLORS['primary']}),
                html.Div("Dados não disponíveis", style={"padding": "10px", "textAlign": "center", "color": COLORS['text_muted']})
            ], style={**card_style, "height": "100%"})

        if not bt_card:
            bt_card = html.Div([
                html.Div("Baixa Tensão", style={**header_style, "backgroundColor": COLORS['secondary']}),
                html.Div("Dados não disponíveis", style={"padding": "10px", "textAlign": "center", "color": COLORS['text_muted']})
            ], style={**card_style, "height": "100%"})

        if not terciario_card:
            terciario_card = html.Div([
                html.Div("Terciário", style={**header_style, "backgroundColor": COLORS['info']}),
                html.Div("Dados não disponíveis", style={"padding": "10px", "textAlign": "center", "color": COLORS['text_muted']})
            ], style={**card_style, "height": "100%"})
        ...
        if tipo_isolamento:
            isolamento_info = html.Div([
                ...
                ], style={"padding": "8px 12px", "backgroundColor": COLORS['background_card_header'], "borderRadius": "4px"})
...
def analyze_dielectric_details(n_clicks, dieletric_data):
    ...
    log.info(f"[ANALYZE DETAILS] Iniciando análise detalhada comparativa")
    ...
    try:
        enrolamentos = dieletric_data.get('parametros', {}).get('enrolamentos', [])
        tipo_transformador = dieletric_data.get('parametros', {}).get('tipo_transformador', 'Imerso em Óleo')
        ...
        at_output = create_enrolamento_comparison(verificador, enrolamentos_map.get('Alta Tensão'), tipo_transformador, tipo_isolamento)
        bt_output = create_enrolamento_comparison(verificador, enrolamentos_map.get('Baixa Tensão'), tipo_transformador, tipo_isolamento)
        terciario_output = create_enrolamento_comparison(verificador, enrolamentos_map.get('Terciário'), tipo_transformador, tipo_isolamento)
        ...
        return no_update, no_update, no_update, no_update, dbc.Alert(
            f"Erro ao realizar análise detalhada: {str(e)}",
            color="danger",
            dismissable=True
        )
...
def create_enrolamento_comparison(verificador, enrolamento_data, tipo_transformador, _):
    ...
    try:
        ...
        clearances = verificador.get_clearances(um, tipo_transformador, ia, im) if hasattr(verificador, 'get_clearances') else {}
        ...
        components.append(create_comparison_table(
        ...
        components.append(create_comparison_table(
            ...
            {
                'BIL/NBI (kVp)': ieee_data.get('bil', 'N/A') if ieee_data else 'N/A',
                'Impulso Cortado (kVp)': f"{float(ieee_data.get('bil', 0)) * 1.15:.1f}" if ieee_data and ieee_data.get('bil') else 'N/A',
                'Forma de Onda': '1.2/50 µs'
            }
        ))
        ...
        if conexao == "YN" and neutro_um:
            ...
            if hasattr(verificador, 'nbr') and hasattr(verificador.nbr, 'get_tensao_curta_values'):
                nbr_neutro_acsd_values = verificador.nbr.get_tensao_curta_values(neutro_um, "Y", None)
            ...
            components.append(create_comparison_table(
                ...
                {
                    'Um Neutro (kV)': neutro_um,
                    'BIL/NBI Neutro (kVp)': ieee_data.get('neutro_bil', 'N/A') if ieee_data and 'neutro_bil' in ieee_data else 'N/A',
                    'Tensão Aplicada (kV)': ieee_neutro_data.get('applied', 'N/A') if 'ieee_neutro_data' in locals() and ieee_neutro_data else 'N/A'
                }
            ))
