...
from components.formatters import formatar_dados_basicos, formatar_tensao_aplicada, format_parameter_value # Formatadores
...
def analyze_resonant_system_viability(capacitance_nf, voltage_kv, enrolamento=""):
    ...
    if modulos_3par_450kv and voltage_kv <= modulos_3par_450kv[1]["tensao_max"] and \
       capacitance_nf >= modulos_3par_450kv[1]["cap_min"] and capacitance_nf <= modulos_3par_450kv[1]["cap_max"]:
    ...
    elif modulos_3par_270kv and voltage_kv <= modulos_3par_270kv[1]["tensao_max"] and \
         capacitance_nf >= modulos_3par_270kv[1]["cap_min"] and capacitance_nf <= modulos_3par_270kv[1]["cap_max"]:
    ...
    # Se não for possível usar Módulos 1||2||3 (3 Par.), tentar outras configurações
        ...
        for name, config in sorted_configs:
            if (voltage_kv <= config["tensao_max"] and
                capacitance_nf >= config["cap_min"] and
                capacitance_nf <= config["cap_max"]):
    ...
    if viable_config:
        ...
        if is_modulos_3par:
            recommendation = (f"Configuração ideal: {viable_config['nome']}. "
                             f"Limites: {viable_config['tensao_max']}kV / "
                             f"{viable_config['cap_min']:.2f}-{viable_config['cap_max']:.1f}nF.")
        else:
            recommendation = (f"Configuração alternativa: {viable_config['nome']}. "
                             f"Limites: {viable_config['tensao_max']}kV / "
                             f"{viable_config['cap_min']:.2f}-{viable_config['cap_max']:.1f}nF. "
        ...
        reason = ""
        configs_tensao_ok = [c for _, c in constants.RESONANT_SYSTEM_CONFIGS.items() if voltage_kv <= c["tensao_max"]]
        ...
        if modulos_3par_450kv and modulos_3par_270kv:
            modulos_3par_min_cap = min(modulos_3par_450kv[1]["cap_min"], modulos_3par_270kv[1]["cap_min"])
            modulos_3par_max_cap = max(modulos_3par_450kv[1]["cap_max"], modulos_3par_270kv[1]["cap_max"])
            modulos_3par_max_volt = max(modulos_3par_450kv[1]["tensao_max"], modulos_3par_270kv[1]["tensao_max"])

        if not configs_tensao_ok:
            max_sys_volt = max(c["tensao_max"] for _, c in constants.RESONANT_SYSTEM_CONFIGS.items())
            reason = f"Tensão ({voltage_kv:.1f} kV) excede o máximo do sistema ({max_sys_volt} kV)."

            if modulos_3par_max_volt:
                reason += f" Para Módulos 1||2||3 (3 Par.), a tensão máxima é {modulos_3par_max_volt} kV."
        else:
...
def applied_voltage_calculate_and_analyze(
    ...
    cap_at_pf, cap_bt_pf, cap_ter_pf,  # Parâmetros de capacitância
    transformer_data, current_store_data):
    """
    ...
    print("=" * 80)
    print(f"***** CALLBACK applied_voltage_calculate_and_analyze INICIADO - Botão clicado {n_clicks} vezes - Trigger: {callback_context.triggered_id if callback_context.triggered else 'N/A'} *****")
    print("=" * 80)
    log.info(f"***** CALLBACK applied_voltage_calculate_and_analyze INICIADO - Botão clicado {n_clicks} vezes - Trigger: {callback_context.triggered_id if callback_context.triggered else 'N/A'} *****")
    ...
    if not transformer_data or not isinstance(transformer_data, dict):
        ...
        error_msg = html.Ul([html.Li("Dados do transformador não encontrados. Configure os dados básicos do transformador primeiro.")],
                           style={"color": "red", "fontSize": "0.7rem"})
        return error_msg, "", no_update

    # Obtém os valores de tensão diretamente do transformer_data
    tensao_at_kv_str = transformer_data.get('teste_tensao_aplicada_at')
    tensao_bt_kv_str = transformer_data.get('teste_tensao_aplicada_bt')
    tensao_ter_kv_str = transformer_data.get('teste_tensao_aplicada_terciario')

    log.info(f"[Applied Voltage] Usando tensões do transformer_data: AT={tensao_at_kv_str}, BT={tensao_bt_kv_str}, Ter={tensao_ter_kv_str}")

    # Obtém a frequência diretamente do transformer_data
    frequencia_str = transformer_data.get('frequencia', '60')  # Valor padrão de 60 Hz

    inputs = {
        'Cap. AT (pF)': cap_at_pf, 'Cap. BT (pF)': cap_bt_pf,
        'Tensão AT (kV)': tensao_at_kv_str, 'Tensão BT (kV)': tensao_bt_kv_str,
    ...
    try:
        ...
        log.debug(f"[CALC Applied] Capacitância AT ajustada: {cap_at} + {cap_at_adicional} = {cap_at_ajustado} pF (divisor de tensão)")
        log.debug(f"[CALC Applied] Capacitância BT ajustada: {cap_bt} + {cap_bt_adicional} = {cap_bt_ajustado} pF (divisor de tensão)")
        if cap_ter > 0:
            log.debug(f"[CALC Applied] Capacitância Terciário ajustada: {cap_ter} + {cap_ter_adicional} = {cap_ter_ajustado} pF (divisor de tensão)")

        log.debug(f"[CALC Applied] Chamando calculate_capacitive_load para AT com cap_at={cap_at_ajustado}, tensao_at_kv={tensao_at_kv}, freq_hz={freq_hz}")
        zc_at, i_at_ma, p_at_kvar = calculate_capacitive_load(cap_at_ajustado, tensao_at_kv * 1000, freq_hz)
        log.debug(f"[CALC Applied] calculate_capacitive_load para AT retornou: zc_at={zc_at}, i_at_ma={i_at_ma}, p_at_kvar={p_at_kvar}")

        log.debug(f"[CALC Applied] Chamando calculate_capacitive_load para BT com cap_bt={cap_bt_ajustado}, tensao_bt_kv={tensao_bt_kv}, freq_hz={freq_hz}")
        zc_bt, i_bt_ma, p_bt_kvar = calculate_capacitive_load(cap_bt_ajustado, tensao_bt_kv * 1000, freq_hz)
        log.debug(f"[CALC Applied] calculate_capacitive_load para BT retornou: zc_bt={zc_bt}, i_bt_ma={i_bt_ma}, p_bt_kvar={p_bt_kvar}")

        if cap_ter_ajustado > 0:
            log.debug(f"[CALC Applied] Chamando calculate_capacitive_load para Terciário com cap_ter={cap_ter_ajustado}, tensao_ter_kv={tensao_ter_kv}, freq_hz={freq_hz}")
            zc_ter, i_ter_ma, p_ter_kvar = calculate_capacitive_load(cap_ter_ajustado, tensao_ter_kv * 1000, freq_hz)
            log.debug(f"[CALC Applied] calculate_capacitive_load para Terciário retornou: zc_ter={zc_ter}, i_ter_ma={i_ter_ma}, p_ter_kvar={p_ter_kvar}")
        else:
        ...
        analise_ter = analyze_resonant_system_viability(cap_ter_nf, tensao_ter_kv, "Terciário") if cap_ter_ajustado > 0 else None
        ...
        results_table_data = [
            ...
            ["Tensão Ensaio (kV)", f"{tensao_at_kv:.2f}", f"{tensao_bt_kv:.2f}", f"{tensao_ter_kv:.2f}" if tensao_ter_kv > 0 else "-"],
            ["Cap. Informada (pF)", f"{cap_at:.0f}", f"{cap_bt:.0f}", f"{cap_ter:.0f}" if cap_ter > 0 else "-"],
            ["Cap. Divisor (pF)", f"{cap_at_adicional}", f"{cap_bt_adicional}", f"{cap_ter_adicional}" if cap_ter > 0 else "-"],
            ["Cap. Ajustada (pF)", f"{cap_at_ajustado:.0f}", f"{cap_bt_ajustado:.0f}", f"{cap_ter_ajustado:.0f}" if cap_ter_ajustado > 0 else "-"],
            ["Corrente (mA)", format_parameter_value(i_at_ma, 2), format_parameter_value(i_bt_ma, 2), format_parameter_value(i_ter_ma, 2) if i_ter_ma else "-"],
            ["Zc (Ω)", format_parameter_value(zc_at, 1), format_parameter_value(zc_bt, 1), format_parameter_value(zc_ter, 1) if zc_ter else "-"],
            ["Potência Reativa (kVAr)", format_parameter_value(p_at_kvar, 3), format_parameter_value(p_bt_kvar, 3), format_parameter_value(p_ter_kvar, 3) if p_ter_kvar else "-"]
        ]

        # Criação manual da tabela com estilos personalizados
        header = html.Thead(html.Tr([
            html.Th(col, style={"fontSize": "0.7rem", "padding": "0.2rem", "backgroundColor": COLORS['background_card_header'], "color": COLORS['text_light']})
        ...
        for i, row_data in enumerate(results_table_data[1:]):
            ...
            rows.append(html.Tr([
                html.Td(cell, style={"fontSize": "0.7rem", "padding": "0.2rem", "backgroundColor": bg_color, "color": COLORS['text_light']})
                for cell in row_data
            ]))

        results_table = dbc.Table(
            [header, html.Tbody(rows)],
            bordered=True, hover=True, size="sm", style={"backgroundColor": "#3D3D3D"}
        )
        ...
        for i, enr in enumerate(analise_enrolamentos):
            coluna = dbc.Col([
                dbc.Alert([
                    html.Strong(f"{enr['enrolamento']} ({enr['viabilidade']}):", className=f"text-{enr['cor_alerta']}"),
                    html.P(enr["recommendation"], style={"fontSize": "0.7rem", "marginBottom": "0.2rem", "color": COLORS['text_light']}),
                ], color=enr["cor_alerta"], className="mb-1 p-1", style={"borderRadius":"4px", "backgroundColor": "#3D3D3D", "borderColor": "#6E6E6E"})
            ], width=4)  # Largura igual para cada coluna (4 de 12 = 1/3)
        ...
        data_for_store = {
             'inputs': { # Salva inputs originais da UI e do transformer_data
                 'cap_at_pf': cap_at_pf, 'cap_bt_pf': cap_bt_pf, 'cap_ter_pf': cap_ter_pf,
                 'cap_at_ajustado_pf': cap_at_ajustado, 'cap_bt_ajustado_pf': cap_bt_ajustado, 'cap_ter_ajustado_pf': cap_ter_ajustado,
                 'tensao_at_kv': transformer_data.get('teste_tensao_aplicada_at'), # Usa valor do transformer_data
                 'tensao_bt_kv': transformer_data.get('teste_tensao_aplicada_bt'), # Usa valor do transformer_data
                 'tensao_ter_kv': transformer_data.get('teste_tensao_aplicada_terciario'), # Usa valor do transformer_data
                 'frequencia_hz': frequencia_str
             },
             'resultados': {
                 'dados_calculo': { # Salva resultados numéricos
                     'zc_at': zc_at, 'i_at': i_at_ma, 'p_at': p_at_kvar,
                     'zc_bt': zc_bt, 'i_bt': i_bt_ma, 'p_bt': p_bt_kvar,
                     'zc_ter': zc_ter, 'i_ter': i_ter_ma, 'p_ter': p_ter_kvar,
        ...
        print(f"--- Fim [APPLIED VOLTAGE STORE SAVE DEBUG] ---")

        # Registra os resultados dos cálculos nos logs
        print("=" * 80)
        print("[CALC Applied] RESULTADOS DOS CÁLCULOS DE TENSÃO APLICADA:")
        print(f"[CALC Applied] AT: Cap={cap_at:.0f}pF (Ajustada={cap_at_ajustado:.0f}pF), Zc={zc_at:.2f} Ω, I={i_at_ma:.2f} mA, P={p_at_kvar:.3f} kVAr")
        print(f"[CALC Applied] BT: Cap={cap_bt:.0f}pF (Ajustada={cap_bt_ajustado:.0f}pF), Zc={zc_bt:.2f} Ω, I={i_bt_ma:.2f} mA, P={p_bt_kvar:.3f} kVAr")
        if zc_ter is not None:
            print(f"[CALC Applied] Terciário: Cap={cap_ter:.0f}pF (Ajustada={cap_ter_ajustado:.0f}pF), Zc={zc_ter:.2f} Ω, I={i_ter_ma:.2f} mA, P={p_ter_kvar:.3f} kVAr")

        # Registra a análise de viabilidade
        print("[CALC Applied] ANÁLISE DE VIABILIDADE:")
        for enr in analise_enrolamentos:
            print(f"[CALC Applied] {enr['enrolamento']} ({enr['viabilidade']}): {enr['recommendation']}")
        print("=" * 80)
        ...
        resultados_completos = html.Div([
            results_table,
            html.Div([
                html.P("Nota: A capacitância ajustada inclui a capacitância do divisor de tensão:",
                      style={"fontSize": "0.7rem", "marginTop": "10px", "marginBottom": "5px", "fontWeight": "bold", "color": COLORS['text_light']}),
                html.Ul([
                    html.Li("330 pF para tensão > 450 kV", style={"fontSize": "0.7rem", "color": COLORS['text_light']}),
                    html.Li("660 pF para tensão ≤ 450 kV", style={"fontSize": "0.7rem", "color": COLORS['text_light']})
                ], style={"marginBottom": "10px", "paddingLeft": "20px"})
            ])
        ])

        log.info("Cálculo de Tensão Aplicada e análise de viabilidade concluídos.")
        log.debug(f"[CALC Applied] Retornando: results_table={type(results_table)}, recomendacoes_div={type(recomendacoes_div)}, current_store_data={type(current_store_data)}")
...
def register_applied_voltage_callbacks(app):
    ...
    app.callback(
        ...
        prevent_initial_call=False
    )(lambda transformer_data:
        ["-", "-", "-", "60 Hz"] if not transformer_data or not isinstance(transformer_data, dict)
        else [
    ...
    app.callback(
        ...
        [State("cap-at", "value"),
         State("cap-bt", "value"),
         State("cap-ter", "value"),
         State('transformer-inputs-store', 'data'),  # Lê dados básicos para referência (incluindo frequência)
         State('applied-voltage-store', 'data')],  # Lê estado atual do store
...
if __name__ == "__main__":
    ...
    test_cases = [
        (0.5, 100, "AT"),  # Caso típico - Deve usar outra configuração (capacitância abaixo do mínimo para Módulos 1||2||3)
        ...
        (3.0, 450, "AT"),  # Dentro do range para Módulos 1||2||3 (3 Par.) 450kV - Deve ser recomendado
        (20.0, 450, "BT"),  # Dentro do range para Módulos 1||2||3 (3 Par.) 450kV - Deve ser recomendado
        (25.0, 270, "Terciário"),  # Dentro do range para Módulos 1||2||3 (3 Par.) 270kV - Deve ser recomendado
        (35.0, 270, "AT"),  # Dentro do range para Módulos 1||2||3 (3 Par.) 270kV - Deve ser recomendado

        # Testes de limites
        (1.9, 450, "BT"),  # Abaixo do mínimo para Módulos 1||2||3 (3 Par.) - Deve usar outra configuração
        (23.7, 450, "Terciário"),  # No limite entre as duas configurações - Deve usar Módulos 1||2||3 (3 Par.) 270kV
