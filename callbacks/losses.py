...
from config import (
    ...
    TABLE_HEADER_STYLE_SM, TABLE_HEADER_STYLE_MD,
    TABLE_PARAM_STYLE_SM, TABLE_PARAM_STYLE_MD,
    TABLE_VALUE_STYLE_SM, TABLE_VALUE_STYLE_MD,
    TABLE_STATUS_STYLE, TABLE_WRAPPER_STYLE,
...
from utils.constants import (
    ...
    SUT_BT_VOLTAGE, SUT_AT_MIN_VOLTAGE, SUT_AT_MAX_VOLTAGE,
    SUT_AT_STEP_VOLTAGE, EPS_CURRENT_LIMIT, DUT_POWER_LIMIT,
    DEFAULT_FREQUENCY
)
...
try:
    ...
    df_potencia_magnet[['inducao_nominal', 'frequencia_nominal']] = pd.DataFrame(df_potencia_magnet['key'].tolist(), index=df_potencia_magnet.index)
    ...
    df_perdas_nucleo[['inducao_nominal', 'frequencia_nominal']] = pd.DataFrame(df_perdas_nucleo['key'].tolist(), index=df_perdas_nucleo.index)
...
def create_input_row(label, id, placeholder, input_type="number"):
    ...
    label_style = TYPOGRAPHY.get('label', {"fontSize": "0.65rem", "fontWeight": "bold", "color": COLORS['text_light']})
    input_base_style = COMPONENTS.get('input', {'fontSize': '0.7rem', 'color': COLORS['text_light'], 'backgroundColor': COLORS['background_input'], 'border': f"1px solid {COLORS['border']}"})
    final_input_style = {**input_base_style, 'height': '26px', 'padding': '0.15rem 0.3rem', 'width': '75%'}
    return dbc.Row([
        dbc.Col(dbc.Label(label, style=label_style), width=9, className="text-end pe-1"),
        dbc.Col(dbc.Input(type=input_type, id=id, placeholder=placeholder, persistence=True, persistence_type='local', style=final_input_style), width=3)
    ], className="g-1 mb-1")

# --- Render Functions (Assumed to be in layouts/losses.py) ---
# Import render functions locally to avoid circular dependency
try:
    from layouts.losses import render_perdas_vazio, render_perdas_carga
except ImportError:
    log.error("Could not import render functions from layouts.losses. Defining placeholders.")
    def render_perdas_vazio(): return html.Div("Layout Vazio não carregado.", style=ERROR_STYLE)
    def render_perdas_carga(): return html.Div("Layout Carga não carregado.", style=ERROR_STYLE)

# --- Callbacks ---
@dash.callback(Output("conteudo-perdas", "children"), [Input("tabs-perdas", "active_tab")])
def losses_render_tab_content(tab_ativa):
    if tab_ativa == "tab-vazio": return render_perdas_vazio()
    elif tab_ativa == "tab-carga": return render_perdas_carga()
...
def losses_handle_perdas_vazio(n_clicks, perdas_vazio, peso_nucleo, corrente_excitacao, inducao, corrente_exc_1_1, corrente_exc_1_2):
    ...
    try:
        ...
        perdas_vazio = safe_float(perdas_vazio, 0.0)
        peso_nucleo = safe_float(peso_nucleo, 0.0)
        corrente_excitacao_percentual = safe_float(corrente_excitacao, 0.0)
        tensao_bt_kv = safe_float(transformer_data.get('tensao_bt', 0), 0.0) # Assume BT is in kV
        inducao = safe_float(inducao, 0.0)
        ...
        corrente_exc_1_1_input = safe_float(corrente_exc_1_1) # Keep None if empty/invalid
        corrente_exc_1_2_input = safe_float(corrente_exc_1_2) # Keep None if empty/invalid

        # --- Data Validation after Conversion ---
        if any(v is None or v <= epsilon for v in [potencia, tensao_nominal_at, tensao_bt_kv, corrente_nominal_bt, perdas_vazio, inducao, corrente_excitacao_percentual]):
             # Peso núcleo can be zero if calculated
             error_msg_detail = ", ".join([f"{k}={v}" for k,v in {
                 'Potência':potencia, 'Tensão AT':tensao_nominal_at, 'Tensão BT':tensao_bt_kv,
                 'Corrente BT':corrente_nominal_bt, 'Perdas Vazio':perdas_vazio, 'Indução':inducao,
                 'Corr Exc %':corrente_excitacao_percentual}.items() if v is None or v <= epsilon])
             error_div = html.Div(f"Dados essenciais devem ser maiores que zero ou não nulos. Inválidos: {error_msg_detail}", style=ERROR_STYLE)
             return error_div, initial_dut_volt, initial_sut, initial_legend_obs, no_update
        # Special check for peso_nucleo if it was an input > 0
        if peso_nucleo is not None and peso_nucleo <= epsilon:
             error_div = html.Div(f"Peso do núcleo de projeto ({peso_nucleo}) deve ser maior que zero.", style=ERROR_STYLE)
             return error_div, initial_dut_volt, initial_sut, initial_legend_obs, no_update
        ...
        try:
            ...
            fator_perdas = df_perdas_nucleo.loc[lookup_key, 'perdas_nucleo'] if lookup_key in df_perdas_nucleo.index else None
            fator_potencia_mag = df_potencia_magnet.loc[lookup_key, 'potencia_magnet'] if lookup_key in df_potencia_magnet.index else None
            ...
            error_div = html.Div(f"Fatores de perdas/potência não encontrados para Indução {inducao_arredondada}T @ {frequencia_arredondada}Hz.", style=ERROR_STYLE)
            return error_div, initial_dut_volt, initial_sut, initial_legend_obs, no_update
        except Exception as e:
        ...
        if fator_perdas is None or fator_perdas <= epsilon or fator_potencia_mag is None or fator_potencia_mag <= epsilon:
            error_div = html.Div(f"Fatores de perdas/potência inválidos ({fator_perdas=}, {fator_potencia_mag=}) para Indução {inducao_arredondada}T @ {frequencia_arredondada}Hz.", style=ERROR_STYLE)
            return error_div, initial_dut_volt, initial_sut, initial_legend_obs, no_update
        ...
        corrente_excitacao_calc = potencia_mag / (tensao_bt_kv * sqrt_3) if (tensao_bt_kv * sqrt_3) > epsilon else 0 # A
        corrente_excitacao_percentual_calc = (corrente_excitacao_calc / corrente_nominal_bt) * 100 if corrente_nominal_bt > epsilon else 0
        ...
        fator_excitacao_default = 3 if tipo_transformador == 'Trifásico' else 5 # Default multiplier if inputs are missing
        ...
        potencia_ensaio_1_2pu_projeto_kva = (tensao_teste_1_2_kv * corrente_excitacao_1_2 * sqrt_3) if corrente_excitacao_1_2 is not None else None

        # Project Factors
        fator_perdas_projeto = perdas_vazio / peso_nucleo if peso_nucleo > epsilon else 0 # W/kg (assuming peso is Ton -> *1000) -> Actually kW/Ton
        potencia_mag_projeto_kvar = potencia_ensaio_1pu_projeto_kva # Approximated as test power at 1pu
        ...
        fator_potencia_mag_projeto = (potencia_mag_projeto_kvar * 1000) / (peso_nucleo * 1000) if peso_nucleo > epsilon else 0 # VAR/kg
        ...
        resultados_projeto = {
            ...
            "Fator de perdas (W/kg)": fator_perdas_projeto * 1000 / 1000, # kW/Ton -> W/kg? Assuming input perdas_vazio is kW, peso_nucleo is Ton. Then perdas_vazio / peso_nucleo = kW/Ton. Need W/kg. (kW/Ton) * (1000 W/kW) / (1000 kg/Ton) = W/kg
        ...
        if corrente_excitacao_1_2 is not None:
            ...
            resultados_projeto["Potência de Ensaio (1.2 pu) (kVA)"] = potencia_ensaio_1_2pu_projeto_kva
        ...
        for pu_level in ['1.0', '1.1', '1.2']:
            ...
            if V_teste_dut_lv_kv is None or V_teste_dut_lv_kv <= epsilon or I_exc_dut_lv is None or I_exc_dut_lv <= epsilon:
                sut_analysis_data[pu_level] = {'status': 'Sem dados de corrente/tensão', 'taps_info': []}
                continue
            ...
            if len(taps_sut_hv) == 0:
                 sut_analysis_data[pu_level] = {'status': f'Faixa SUT inválida', 'taps_info': []}
                 continue

            taps_adequados = [tap for tap in taps_sut_hv if tap >= V_target_sut_hv - 1e-6] # Tolerance

            if not taps_adequados:
                highest_sut_tap_kv = taps_sut_hv[-1] / 1000 if len(taps_sut_hv) > 0 else 'N/A'
                sut_analysis_data[pu_level] = {'status': f'Tensão > {highest_sut_tap_kv}kV SUT Max', 'taps_info': []}
                continue
            ...
            for V_sut_hv_tap in top_taps:
                 if tensao_sut_bt <= epsilon: continue # Avoid division by zero
                 ratio_sut = V_sut_hv_tap / tensao_sut_bt
                 I_sut_lv = I_exc_dut_lv * ratio_sut # Simple reflection for vazio
                 percent_limite = (I_sut_lv / limite_corrente_eps) * 100 if limite_corrente_eps > epsilon else float('inf')
                 taps_info_list.append({'tap_sut_kv': V_sut_hv_tap / 1000, 'corrente_eps_a': I_sut_lv, 'percent_limite': percent_limite})
        ...
        def create_general_parameters_table(res_proj, res_m4):
            ...
            params = ["Tensão nominal teste 1.0 pu (kV)", "Corrente Nominal BT (A)", "Frequência (Hz)",
                      "Potência Mag. (kVAR)", "Fator de perdas Mag. (VAR/kg)", "Fator de perdas (W/kg)",
                      "Peso do núcleo Calculado(Ton)",
                      "Corrente de excitação percentual (%)"] # M4 Only
            ...
            for param in params:
                ...
                if param in ["Corrente Nominal BT (A)"]: disp_m4 = "-"
        ...
        def create_voltage_level_table(res_proj, res_m4):
            ...
            params = ["Tensão de teste (kV)", "Corrente de excitação (A)", "Potência de Ensaio (kVA)"]
            keys_proj = {
            ...
            exceeded_values = {"projeto": {"1.0 pu": False, "1.1 pu": False, "1.2 pu": False}, "aco_m4": {"1.0 pu": False, "1.1 pu": False, "1.2 pu": False}}
            ...
            for param_idx, param in enumerate(params):
                # Project Row
                proj_cells = [html.Td(param, rowSpan=2, style=param_style), html.Td("Projeto", style=origin_style)]
                for i, pu in enumerate(["1.0 pu", "1.1 pu", "1.2 pu"]):
                    key = keys_proj.get(param, {}).get(pu)
                    value = res_proj.get(key)
                    prec = 1 if "Tensão" in param else 1 if "Corrente" in param else 0 # kVA integer
                    ...
                    if param == "Corrente de excitação (A)" and isinstance(value, (int, float)) and corrente_nominal_bt > epsilon:
                        percentual = (value / corrente_nominal_bt) * 100
                        display = f"{value:.{prec}f} ({percentual:.1f}%)"

                    style = {**value_styles[i]}
                    if param == "Potência de Ensaio (kVA)" and isinstance(value, (int, float)) and value > limite_potencia_dut:
            ...
            body = html.Tbody(rows)
            return dbc.Table([header, body], bordered=True, hover=True, size="sm", style={"border": f"1px solid {COLORS['border']}", "borderCollapse": "collapse"}), exceeded_values
        ...
        def _create_single_sut_analysis_table(pu_data):
            ...
            def get_style(percent):
                 ...
                 if percent < 50: return current_styles["low"]
                 elif percent < 85: return current_styles["medium"]
                 elif percent <= 100: return current_styles["high"]
                 else: return current_styles["critical"]

            rows = [html.Tr([html.Th("Tap SUT (kV)", style=header_style), html.Th("Corrente EPS (A)", style=header_style)])]
            status_msg = pu_data.get('status', 'Erro')
            taps_info = pu_data.get('taps_info', [])

            if status_msg != 'OK':
                rows.append(html.Tr(html.Td(status_msg, colSpan=2, style={**cell_style, "color": COLORS['danger'], "fontWeight": "bold"}, className="table-value-cell")))
            elif not taps_info:
                rows.append(html.Tr(html.Td("Nenhum tap SUT aplicável.", colSpan=2, style=cell_style, className="table-value-cell")))
            else:
                for info in taps_info: # Already sorted by voltage and limited to 5
                    current_style = get_style(info.get('percent_limite'))
                    tap_disp = f"{info['tap_sut_kv']:.1f}" if info.get('tap_sut_kv') is not None else "-"
                    curr_disp = f"{info['corrente_eps_a']:.1f}" if info.get('corrente_eps_a') is not None else "-"
                    rows.append(html.Tr([
            ...
            return dbc.Table(html.Tbody(rows), bordered=True, hover=True, striped=True, size="sm", style={"width": "100%", "tableLayout": "fixed", "marginBottom": "0"})
        ...
        def create_sut_analysis_layout(sut_data):
            ...
            for pu_level in ['1.0', '1.1', '1.2']:
                ...
                cols.append(dbc.Col([
                    html.Div(f"Análise para {pu_level} pu", className="text-center py-1 mb-1", style=title_style),
                    html.Div(content)
                ], width=12, md=4, style=container_style)) # Responsive width
        ...
        def create_legend_section(exceeded, sut_data):
            ...
            for pu, data in sut_data.items():
                ...
                elif status == 'OK' and taps_info and all(t.get('percent_limite', 0) > 100 for t in taps_info):
                    eps_warn_texts.append(f"Ensaio a {pu_map[pu]}: Corrente EPS > {limite_corrente_eps}A para todos os taps.")
            ...
            if power_warn:
                for pu in ["1.0 pu", "1.1 pu", "1.2 pu"]:
                    ...
                    origins = [o.replace('_',' ').title() for o in ["projeto", "aco_m4"] if exceeded[o][pu]]
                    if origins:
                        power_warn_texts.append(f"Ensaio a {pu}: Potência DUT > {limite_potencia_dut} kVA em {', '.join(origins)}.")
            ...
            if all_warnings:
                items.append(html.Div([
                    ...
                    html.Ul([html.Li(text, style={"color": COLORS['danger']}) for text in all_warnings], style={"paddingLeft": "20px", "marginTop": "5px"})
                ], style={"fontSize": "0.75rem", "marginBottom": "0.5rem"}))
                items.append(html.Hr(style={"borderColor": COLORS['border'], "margin": "0.5rem 0"}))

            # Cor do texto da legenda
            legend_style = {"fontSize": "0.8rem", "marginBottom": "0.5rem", "color": "inherit", "padding": "4px", "border": f"1px solid {COLORS['border_light']}", "borderRadius": "4px", "backgroundColor": COLORS['background_faint']}
            span_style = {"display":"inline-block", "width":"12px", "height":"12px", "marginRight":"5px", "border": f"1px solid {COLORS['border']}", 'verticalAlign': 'middle'}
            legend_list = [
                 html.Div([html.Span(style={**span_style, "backgroundColor": CONFIG_COLORS['danger_bg']}), html.Span(f"Potência DUT > {limite_potencia_dut} kVA")], style=legend_style),
                 html.Div([html.Span(style={**span_style, "backgroundColor": CONFIG_COLORS['ok_bg_faint']}), html.Span(f"Corrente EPS < 50% ({limite_corrente_eps}A)")], style=legend_style),
                 html.Div([html.Span(style={**span_style, "backgroundColor": CONFIG_COLORS['warning_bg_faint']}), html.Span("Corrente EPS 50-85% Limite")], style=legend_style),
                 html.Div([html.Span(style={**span_style, "backgroundColor": CONFIG_COLORS['warning_high_bg_faint']}), html.Span("Corrente EPS 85-100% Limite")], style=legend_style),
                 html.Div([html.Span(style={**span_style, "backgroundColor": CONFIG_COLORS['danger_bg']}), html.Span("Corrente EPS > 100% Limite")], style=legend_style)
            ]
            ...
            return dbc.Card([
                ...
                dbc.CardBody(html.Div(items), style={**COMPONENTS['card_body'], "backgroundColor": COLORS['background_card']})
            ], style=COMPONENTS['card'])
        # --- End of Layout Helpers (Vazio) ---

        # --- Generate Final Content (Vazio) ---
        parametros_gerais_content = create_general_parameters_table(resultados_projeto, resultados_aco_m4)
        tabela_resultados_dut_content, valores_excedidos = create_voltage_level_table(resultados_projeto, resultados_aco_m4)
        ...
        new_data = {
            ...
            'perdas_vazio_kw': perdas_vazio, 'peso_nucleo': peso_nucleo, 'corrente_excitacao': corrente_excitacao_percentual,
            'inducao': inducao, 'corrente_exc_1_1': corrente_exc_1_1_input, 'corrente_exc_1_2': corrente_exc_1_2_input,
...
def select_target_bank_voltage(max_test_voltage_kv):
    ...
    if target_v_cf is None and cap_bank_voltages_num:
        ...
        log.warning(f"Max test voltage {max_test_voltage_kv:.2f}kV exceeds 110% of highest bank ({cap_bank_voltages_num[-1]}kV). Using highest bank.")
    ...
    if target_v_sf is None and cap_bank_voltages_num:
        ...
        log.warning(f"Max test voltage {max_test_voltage_kv:.2f}kV exceeds highest bank ({cap_bank_voltages_num[-1]}kV). Using highest bank for S/F.")
...
def get_cs_configuration(target_bank_voltage_key, use_group1_only, circuit_type):
    ...
    for switch_name in available_switches:
        ...
        if use_group1_only and circuit_type == 'Trifásico' and is_group_2_switch:
             log.debug(f"Skipping Group 2 CS switch {switch_name} because use_group1_only is True (Trifásico).")
             continue
...
def find_best_q_configuration(target_bank_voltage_key, required_power_mvar, use_group1_only):
    ...
    # Ensure lookup key exactly matches the dictionary keys
    all_available_caps = CAPACITORS_BY_VOLTAGE.get(str(target_bank_voltage_key), []) # Ensure string key
    ...
    if use_group1_only:
        ...
        if not available_caps:
             log.warning(f"No Group 1 (ending in '1') capacitors found for {target_bank_voltage_key}kV, trying all.")
    ...
    for q_comb in q_combinations:
        ...
        if current_power >= required_power_mvar - epsilon: # Add tolerance
            ...
            elif abs(current_power - min_power_above_req) < epsilon and best_combination and len(q_comb) < len(best_combination):
                 min_power_above_req = current_power # Update power in case it's slightly different but within epsilon
                 best_combination = q_comb

    if best_combination:
        q_config_str = ", ".join([f"Q{q}" for q in sorted(best_combination)]) # Sort for consistency
        return q_config_str, min_power_above_req # Return the actual power provided by this combination
    else:
        ...
        log.warning(f"Could not find suitable Q config for {target_bank_voltage_key}kV, {required_power_mvar:.2f} MVAr. Max possible: {max_possible_power:.2f} MVAr")
...
def suggest_capacitor_bank_config(max_voltage_kv, max_power_mvar, circuit_type):
    ...
    log.info(f"Suggesting config for Max V: {max_voltage_kv:.2f} kV, Max Q Req: {max_power_mvar:.2f} MVAr, Type: {circuit_type}")

    if max_voltage_kv is None or max_voltage_kv <= epsilon or max_power_mvar is None or max_power_mvar <= epsilon:
    ...
    max_power_group1 = calculate_q_combination_power([1, 2, 3, 4, 5], group1_caps) if group1_caps else 0
    # Use tolerance when comparing power requirements
    use_group1_only = max_power_mvar <= max_power_group1 + epsilon
    log.debug(f"Target Voltage Key: {target_v_cf_key}, Max Power Group 1: {max_power_group1:.2f} MVAr, Required Power: {max_power_mvar:.2f} MVAr -> Use Group 1 Only: {use_group1_only}")
    ...
    q_config_str, q_power_mvar_provided = find_best_q_configuration(target_v_cf_key, max_power_mvar, use_group1_only)
...
def calculate_sut_eps_current_compensated(
    tensao_ref_dut_kv, corrente_ref_dut_a,
    q_power_scenario_sf_mvar, cap_bank_voltage_scenario_sf_kv,
    q_power_scenario_cf_mvar, cap_bank_voltage_scenario_cf_kv,
    transformer_type,
    V_sut_hv_tap_v, tensao_sut_bt_v, limite_corrente_eps_a
):
    """
    ...
    # Basic validation
    if any(v is None or v <= epsilon for v in [tensao_ref_dut_kv, corrente_ref_dut_a, V_sut_hv_tap_v, tensao_sut_bt_v]):
        log.warning("Compensated current calc: Invalid base inputs (Vref, Iref, Vsut_hv, Vsut_bt). Returning uncompensated.")
        ...
        percent_limite = (I_dut_reflected / limite_corrente_eps_a) * 100 if limite_corrente_eps_a > epsilon else float('inf')
        return {
    ...
    sf_compensation_valid = (
        q_power_scenario_sf_mvar is not None and q_power_scenario_sf_mvar > epsilon and
        cap_bank_voltage_scenario_sf_kv is not None and cap_bank_voltage_scenario_sf_kv > epsilon
    )

    if sf_compensation_valid:
        try:
            # Calculate Corrected Reactive Power for S/F
            Cap_Correct_factor_sf = 0.25 if cap_bank_voltage_scenario_sf_kv in [13.8, 23.9] else 0.75 if cap_bank_voltage_scenario_sf_kv in [41.4, 71.7] else 1.0
            q_denominator_sf = (tensao_ref_dut_kv / cap_bank_voltage_scenario_sf_kv)**2 * Cap_Correct_factor_sf
            pteste_mvar_corrected_sf = q_power_scenario_sf_mvar * q_denominator_sf if q_denominator_sf > epsilon else 0

            # Calculate Capacitive Current for S/F
            I_cap_base_sf = (pteste_mvar_corrected_sf * 1000.0) / (tensao_ref_dut_kv * sqrt_3_factor)
            ...
            log.debug(f"S/F Calc: Tap={V_sut_hv_tap_v/1000:.1f}kV, Q_prov={q_power_scenario_sf_mvar:.2f}MVAr @ {cap_bank_voltage_scenario_sf_kv:.1f}kV")
            log.debug(f"  S/F: Factor={Cap_Correct_factor_sf}, Q_corr={pteste_mvar_corrected_sf:.2f}MVAr, I_cap_adj={I_cap_adjustment_sf:.1f}A, I_net={I_eps_sf_net:.1f}A")
        except Exception as e:
    ...
    cf_compensation_valid = (
        q_power_scenario_cf_mvar is not None and q_power_scenario_cf_mvar > epsilon and
        cap_bank_voltage_scenario_cf_kv is not None and cap_bank_voltage_scenario_cf_kv > epsilon
    )

    if cf_compensation_valid:
        try:
            # Calculate Corrected Reactive Power for C/F
            Cap_Correct_factor_cf = 0.25 if cap_bank_voltage_scenario_cf_kv in [13.8, 23.9] else 0.75 if cap_bank_voltage_scenario_cf_kv in [41.4, 71.7] else 1.0
            q_denominator_cf = (tensao_ref_dut_kv / cap_bank_voltage_scenario_cf_kv)**2 * Cap_Correct_factor_cf
            pteste_mvar_corrected_cf = q_power_scenario_cf_mvar * q_denominator_cf if q_denominator_cf > epsilon else 0

            # Calculate Capacitive Current for C/F
            I_cap_base_cf = (pteste_mvar_corrected_cf * 1000.0) / (tensao_ref_dut_kv * sqrt_3_factor)
            ...
            log.debug(f"C/F Calc: Tap={V_sut_hv_tap_v/1000:.1f}kV, Q_prov={q_power_scenario_cf_mvar:.2f}MVAr @ {cap_bank_voltage_scenario_cf_kv:.1f}kV")
            log.debug(f"  C/F: Factor={Cap_Correct_factor_cf}, Q_corr={pteste_mvar_corrected_cf:.2f}MVAr, I_cap_adj={I_cap_adjustment_cf:.1f}A, I_net={I_eps_cf_net:.1f}A")
        except Exception as e:
            log.error(f"C/F current calc error: {e}")
            # Keep default uncompensated value

    # 7. Calculate % Limit for both S/F and C/F
    percent_limite_sf = (abs(I_eps_sf_net) / limite_corrente_eps_a) * 100 if limite_corrente_eps_a > epsilon else float('inf')
    if I_eps_sf_net < 0:
        percent_limite_sf = -percent_limite_sf

    percent_limite_cf = (abs(I_eps_cf_net) / limite_corrente_eps_a) * 100 if limite_corrente_eps_a > epsilon else float('inf')
    if I_eps_cf_net < 0:
...
def losses_handle_perdas_carga(n_clicks, perdas_carga_nom, perdas_carga_min, perdas_carga_max, temperatura_referencia):
    ...
    required_transformer_fields = ['tipo_transformador', 'potencia_mva', 'tensao_at', 'tensao_at_tap_maior','tensao_at_tap_menor', 'impedancia', 'impedancia_tap_maior', 'impedancia_tap_menor', 'frequencia', 'tensao_bt', 'corrente_nominal_bt'] # Added BT info needed for Vazio losses check
    ...
    try:
        ...
        def get_formatted(res_dict, key, precision=2):
            ...
            if res_dict and key in res_dict and res_dict[key] is not None:
                try:
                    ...
                    return str(res_dict[key]) # Return original string if conversion fails (e.g., config strings)
            return "-" # Return hyphen if key missing or value is None

        temperatura_ref = int(temperatura_referencia) if temperatura_referencia is not None else 75
        ...
        # --- Input Processing ---
        perdas_totais_nom_input = safe_float(perdas_carga_nom)
        perdas_totais_min_input = safe_float(perdas_carga_min)
        perdas_totais_max_input = safe_float(perdas_carga_max)
        perdas_vazio_nom = safe_float(losses_data['resultados_perdas_vazio'].get('perdas_vazio_kw'))

        # Validate essential numeric inputs
        if any(v is None for v in [perdas_totais_nom_input, perdas_totais_min_input, perdas_totais_max_input, perdas_vazio_nom]):
             error_div = html.Div("Valores de perdas (carga e vazio) devem ser numéricos.", style=ERROR_STYLE)
             return initial_detailed_content, error_div, no_update
        if any(v is not None and v <= 0 for v in [perdas_totais_nom_input, perdas_totais_min_input, perdas_totais_max_input]):
            error_div = html.Div("Valores de perdas totais em carga devem ser maiores que zero.", style=ERROR_STYLE)
            return initial_detailed_content, error_div, no_update
        if perdas_vazio_nom is not None and perdas_vazio_nom <= 0:
            error_div = html.Div("Valor de perdas em vazio deve ser maior que zero.", style=ERROR_STYLE)
            return initial_detailed_content, error_div, no_update
        ...
        essential_transformer_nums = [potencia, tensao_nominal_at, tensao_at_tap_maior, tensao_at_tap_menor, impedancia, impedancia_tap_maior, impedancia_tap_menor]
        if any(v is None for v in essential_transformer_nums):
             missing_keys = [k for k,v in transformer_data.items() if k in ['potencia_mva','tensao_at','tensao_at_tap_maior','tensao_at_tap_menor','impedancia','impedancia_tap_maior','impedancia_tap_menor'] and v is None]
             error_div = html.Div(f"Dados numéricos essenciais do transformador estão faltando: {', '.join(missing_keys)}.", style=ERROR_STYLE)
             return initial_detailed_content, error_div, no_update
        if any(v is not None and v <= epsilon for v in essential_transformer_nums):
             invalid_keys = [k for k,v in transformer_data.items() if k in ['potencia_mva','tensao_at','tensao_at_tap_maior','tensao_at_tap_menor','impedancia','impedancia_tap_maior','impedancia_tap_menor'] and (v is not None and v <= epsilon)]
             error_div = html.Div(f"Dados essenciais do transformador devem ser maiores que zero: {', '.join(invalid_keys)}.", style=ERROR_STYLE)
             return initial_detailed_content, error_div, no_update

        sqrt_3_factor = sqrt_3 if tipo_transformador == 'Trifásico' else 1.0
        corrente_at_nom = (potencia * 1000 / (tensao_nominal_at * sqrt_3_factor)) if tensao_nominal_at > epsilon else 0
        corrente_at_max = (potencia * 1000 / (tensao_at_tap_maior * sqrt_3_factor)) if tensao_at_tap_maior > epsilon else 0
        corrente_at_min = (potencia * 1000 / (tensao_at_tap_menor * sqrt_3_factor)) if tensao_at_tap_menor > epsilon else 0

        if any(v <= epsilon for v in [corrente_at_nom, corrente_at_max, corrente_at_min]):
             error_div = html.Div("Falha ao calcular correntes nominais AT (resultado zero ou negativo). Verifique tensões e potência.", style=ERROR_STYLE)
             return initial_detailed_content, error_div, no_update
        ...
        def calculate_cap_bank(voltage, power):
             ...
             try:
                 ...
                 cap_bank_voltage_com_fator = next((v for v in local_cap_bank_voltages if voltage_f <= (v * 1.1) + 1e-6), local_cap_bank_voltages[-1])

                 # Selection SEM FATOR (Find smallest bank V where V_test <= V_bank)
                 cap_bank_voltage_sem_fator = next((v for v in local_cap_bank_voltages if voltage_f <= v + 1e-6), local_cap_bank_voltages[-1])

                 # Definir o fator de correção com base na tensão do banco
                 def get_cap_correct_factor(bank_voltage):
                     return 0.25 if bank_voltage in [13.8, 23.9] else 0.75 if bank_voltage in [41.4, 71.7] else 1.0
                 ...
                 if cap_bank_voltage_com_fator is not None and cap_bank_voltage_com_fator > epsilon:
                     ...
                     q_denominator_cf = (voltage_f / cap_bank_voltage_com_fator)**2 * Cap_Correct_factor_cf
                     pot_cap_bank_com_fator = power_f / q_denominator_cf if q_denominator_cf > epsilon else float('inf')
                 ...
                 if cap_bank_voltage_sem_fator is not None and cap_bank_voltage_sem_fator > epsilon:
                     ...
                     q_denominator_sf = (voltage_f / cap_bank_voltage_sem_fator)**2 * Cap_Correct_factor_sf
                     pot_cap_bank_sem_fator = power_f / q_denominator_sf if q_denominator_sf > epsilon else float('inf')

                 return cap_bank_voltage_com_fator, pot_cap_bank_com_fator, cap_bank_voltage_sem_fator, pot_cap_bank_sem_fator
        ...
        cenarios = [
            ...
            (tensao_at_tap_menor, corrente_at_min, impedancia_tap_menor, perdas_totais_min_input, "Menor"),
            (tensao_at_tap_maior, corrente_at_max, impedancia_tap_maior, perdas_totais_max_input, "Maior")
        ]
        resultados = []
        ...
        for i, (tensao, corrente, imp, perdas_totais, tap_label) in enumerate(cenarios):
            ...
            if perdas_carga_sem_vazio <= epsilon:
                ...
                log.error(error_msg)
                error_div = html.Div(error_msg, style=ERROR_STYLE)
                calculation_error = True; break # Stop calculation loop

            res_dict["Perdas Carga Sem Vazio (kW)"] = perdas_carga_sem_vazio

            # Calculate cold losses (at 25°C) from reference temperature
            temp_factor = (235.0 + 25.0) / (235.0 + float(temperatura_ref)) if (235.0 + float(temperatura_ref)) > epsilon else 1.0
            ...
            if perdas_cc_a_frio <= epsilon:
                 ...
                 log.error(error_msg)
                 error_div = html.Div(error_msg, style=ERROR_STYLE)
                 calculation_error = True; break
            res_dict["Perdas a Frio (25°C) (kW)"] = perdas_cc_a_frio

            # Calculate "Frio" (Cold Energization - Total Losses) test parameters
            frio_ratio_sqrt = np.sqrt(perdas_totais / perdas_cc_a_frio) if perdas_cc_a_frio > epsilon else 0
            ...
            pteste_frio_mvar = math.sqrt(max(0, pteste_frio_kva**2 - potencia_ativa_eps_frio_kw**2)) / 1000.0 if pteste_frio_kva >= potencia_ativa_eps_frio_kw else 0

            res_dict.update({
                "Tensão frio (kV)": tensao_frio, "Corrente frio (A)": corrente_frio, "Pteste frio (MVA)": pteste_frio_mva,
            ...
            res_dict.update({
                "Cap Bank Voltage Frio Com Fator (kV)": cap_bank_frio[0], "Cap Bank Power Frio Com Fator (MVAr)": cap_bank_frio[1], # Required C/F
                "Cap Bank Voltage Frio Sem Fator (kV)": cap_bank_frio[2], "Cap Bank Power Frio Sem Fator (MVAr)": cap_bank_frio[3], # Required S/F
            })
            ...
            if cap_bank_frio[1] is not None and not math.isinf(cap_bank_frio[1]): # Required C/F
                 max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_frio[1])
            if cap_bank_frio[3] is not None and not math.isinf(cap_bank_frio[3]): # Required S/F
                 max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_frio[3])


            # Calculate "Quente" (Hot Condition - Load Losses at Tref) test parameters
            quente_ratio_sqrt = np.sqrt(perdas_carga_sem_vazio / perdas_cc_a_frio) if perdas_cc_a_frio > epsilon else 0
            ...
            potencia_ativa_eps_quente_kw = perdas_carga_sem_vazio # Just the load losses at Tref for this test's active power
            pteste_quente_mvar = math.sqrt(max(0, pteste_quente_kva**2 - potencia_ativa_eps_quente_kw**2)) / 1000.0 if pteste_quente_kva >= potencia_ativa_eps_quente_kw else 0

            res_dict.update({
                 "Tensão quente (kV)": tensao_quente, "Corrente quente (A)": corrente_quente, "Pteste quente (MVA)": pteste_quente_mva,
            ...
            res_dict.update({
                 "Cap Bank Voltage Quente Com Fator (kV)": cap_bank_quente[0], "Cap Bank Power Quente Com Fator (MVAr)": cap_bank_quente[1], # Required C/F
                 "Cap Bank Voltage Quente Sem Fator (kV)": cap_bank_quente[2], "Cap Bank Power Quente Sem Fator (MVAr)": cap_bank_quente[3], # Required S/F
            })
            ...
            if cap_bank_quente[1] is not None and not math.isinf(cap_bank_quente[1]): # Required C/F
                 max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_quente[1])
            if cap_bank_quente[3] is not None and not math.isinf(cap_bank_quente[3]): # Required S/F
                 max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_quente[3])
            ...
            pteste_25c_mvar = math.sqrt(max(0, pteste_25c_kva**2 - potencia_ativa_eps_25c_kw**2)) / 1000.0 if pteste_25c_kva >= potencia_ativa_eps_25c_kw else 0

            res_dict.update({
                "Tensão 25°C (kV)": tensao_25c, "Corrente 25°C (A)": corrente_25c, "Pteste 25°C (MVA)": pteste_25c_mva,
            ...
            res_dict.update({
                "Cap Bank Voltage 25°C Com Fator (kV)": cap_bank_25c[0], "Cap Bank Power 25°C Com Fator (MVAr)": cap_bank_25c[1], # Required C/F
                "Cap Bank Voltage 25°C Sem Fator (kV)": cap_bank_25c[2], "Cap Bank Power 25°C Sem Fator (MVAr)": cap_bank_25c[3], # Required S/F
            })
            # Update overall maximums
            ...
            if cap_bank_25c[1] is not None and not math.isinf(cap_bank_25c[1]): # Required C/F
                 max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_25c[1])
            if cap_bank_25c[3] is not None and not math.isinf(cap_bank_25c[3]): # Required S/F
                 max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_25c[3])
        ...
        if overload_applicable:
             ...
             for resultado in resultados:
                ...
                if vcc is None or corrente is None or perdas_carga is None:
                    log.warning(f"Skipping overload calculation for Tap {resultado.get('Tap')} due to missing base data.")
                    continue
                ...
                potencia_ativa_eps_1_2_kw = perdas_1_2_kw # Active power for overload is the scaled losses
                pteste_1_2_mvar = math.sqrt(max(0, pteste_1_2_kva**2 - potencia_ativa_eps_1_2_kw**2)) / 1000.0 if pteste_1_2_kva >= potencia_ativa_eps_1_2_kw else 0

                resultado.update({
                    "Tensão 1.2 pu (kV)": tensao_1_2, "Corrente 1.2 pu (A)": corrente_1_2, "Pteste 1.2 pu (MVA)": pteste_1_2_mva, "Perdas 1.2 pu (kW)": perdas_1_2_kw,
                ...
                resultado.update({
                    "Cap Bank Voltage 1.2 pu Com Fator (kV)": cap_bank_1_2[0], "Cap Bank Power 1.2 pu Com Fator (MVAr)": cap_bank_1_2[1], # Required C/F
                    "Cap Bank Voltage 1.2 pu Sem Fator (kV)": cap_bank_1_2[2], "Cap Bank Power 1.2 pu Sem Fator (MVAr)": cap_bank_1_2[3], # Required S/F
                })
                # Update overall maximums
                ...
                if cap_bank_1_2[1] is not None and not math.isinf(cap_bank_1_2[1]): # Required C/F
                     max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_1_2[1])
                if cap_bank_1_2[3] is not None and not math.isinf(cap_bank_1_2[3]): # Required S/F
                     max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_1_2[3])
                ...
                potencia_ativa_eps_1_4_kw = perdas_1_4_kw # Active power for overload is the scaled losses
                pteste_1_4_mvar = math.sqrt(max(0, pteste_1_4_kva**2 - potencia_ativa_eps_1_4_kw**2)) / 1000.0 if pteste_1_4_kva >= potencia_ativa_eps_1_4_kw else 0

                resultado.update({
                    "Tensão 1.4 pu (kV)": tensao_1_4, "Corrente 1.4 pu (A)": corrente_1_4, "Pteste 1.4 pu (MVA)": pteste_1_4_mva, "Perdas 1.4 pu (kW)": perdas_1_4_kw,
                ...
                resultado.update({
                    "Cap Bank Voltage 1.4 pu Com Fator (kV)": cap_bank_1_4[0], "Cap Bank Power 1.4 pu Com Fator (MVAr)": cap_bank_1_4[1], # Required C/F
                    "Cap Bank Voltage 1.4 pu Sem Fator (kV)": cap_bank_1_4[2], "Cap Bank Power 1.4 pu Sem Fator (MVAr)": cap_bank_1_4[3], # Required S/F
                })
                # Update overall maximums
                ...
                if cap_bank_1_4[1] is not None and not math.isinf(cap_bank_1_4[1]): # Required C/F
                     max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_1_4[1])
                if cap_bank_1_4[3] is not None and not math.isinf(cap_bank_1_4[3]): # Required S/F
                     max_test_power_mvar_overall_required = max(max_test_power_mvar_overall_required, cap_bank_1_4[3])
        ...
        def calculate_and_store_cap_bank_config(res_dict, scenario_suffix):
            ...
            if cap_bank_voltage_cf is not None and cap_bank_power_cf_required is not None and not math.isinf(cap_bank_power_cf_required) and cap_bank_power_cf_required > 0:
                target_v_cf_key = None
                try:
                    # Find the index of the closest float voltage
                    closest_voltage_idx_cf = min(range(len(available_voltages_flt)), key=lambda i: abs(available_voltages_flt[i] - cap_bank_voltage_cf))
                    target_v_cf_key = available_voltages_str[closest_voltage_idx_cf] # Get the corresponding string key
                    log.debug(f"Tap {res_dict.get('Tap')}, Scen {scenario_suffix} C/F: Target V float = {cap_bank_voltage_cf:.2f}, Closest Key = {target_v_cf_key}")
                except ValueError:
                     log.error(f"Could not find closest voltage key for C/F {cap_bank_voltage_cf}")

                if target_v_cf_key:
                    # Determine if Group 1 is sufficient for C/F
                    group1_caps_cf = [cap for cap in CAPACITORS_BY_VOLTAGE.get(target_v_cf_key, []) if len(cap) > 4 and cap.endswith('1')]
                    max_power_group1_cf = calculate_q_combination_power([1, 2, 3, 4, 5], group1_caps_cf) if group1_caps_cf else 0
                    use_group1_only_cf = cap_bank_power_cf_required <= max_power_group1_cf + 1e-6 # Add tolerance

                    # Get CS Configuration for C/F
                    config_results[f"CS Config {scenario_suffix}"] = get_cs_configuration(target_v_cf_key, use_group1_only_cf, tipo_transformador)

                    # Get Q Configuration and PROVIDED power for C/F (using the required power)
                    q_config_cf, q_power_cf_provided = find_best_q_configuration(target_v_cf_key, cap_bank_power_cf_required, use_group1_only_cf)
                    config_results[f"Q Config {scenario_suffix}"] = q_config_cf
                    config_results[f"Q Power Provided {scenario_suffix} (MVAr)"] = q_power_cf_provided
                else:
                 ...
                 log.warning(f"Dados C/F insuficientes ou inválidos para config {scenario_suffix} no Tap {res_dict.get('Tap')}. V: {cap_bank_voltage_cf}, Req P: {cap_bank_power_cf_required}")
                 config_results[f"CS Config {scenario_suffix}"] = "N/A (Dados C/F Insuf.)"
                 config_results[f"Q Config {scenario_suffix}"] = "N/A"
                 config_results[f"Q Power Provided {scenario_suffix} (MVAr)"] = 0.0

            # --- Calculate S/F configuration ---
            if cap_bank_voltage_sf is not None and cap_bank_power_sf_required is not None and not math.isinf(cap_bank_power_sf_required) and cap_bank_power_sf_required > 0:
                target_v_sf_key = None
                try:
                    # Find the index of the closest float voltage
                    closest_voltage_idx_sf = min(range(len(available_voltages_flt)), key=lambda i: abs(available_voltages_flt[i] - cap_bank_voltage_sf))
                    target_v_sf_key = available_voltages_str[closest_voltage_idx_sf] # Get the corresponding string key
                    log.debug(f"Tap {res_dict.get('Tap')}, Scen {scenario_suffix} S/F: Target V float = {cap_bank_voltage_sf:.2f}, Closest Key = {target_v_sf_key}")
                except ValueError:
                    log.error(f"Could not find closest voltage key for S/F {cap_bank_voltage_sf}")

                if target_v_sf_key:
                    # Determine if Group 1 is sufficient for S/F
                    group1_caps_sf = [cap for cap in CAPACITORS_BY_VOLTAGE.get(target_v_sf_key, []) if len(cap) > 4 and cap.endswith('1')]
                    max_power_group1_sf = calculate_q_combination_power([1, 2, 3, 4, 5], group1_caps_sf) if group1_caps_sf else 0
                    use_group1_only_sf = cap_bank_power_sf_required <= max_power_group1_sf + 1e-6 # Add tolerance

                    # Get CS Configuration for S/F
                    config_results[f"CS Config {scenario_suffix} S/F"] = get_cs_configuration(target_v_sf_key, use_group1_only_sf, tipo_transformador)

                    # Get Q Configuration and PROVIDED power for S/F (using the required power)
                    q_config_sf, q_power_sf_provided = find_best_q_configuration(target_v_sf_key, cap_bank_power_sf_required, use_group1_only_sf)
                    config_results[f"Q Config {scenario_suffix} S/F"] = q_config_sf
                    config_results[f"Q Power Provided {scenario_suffix} S/F (MVAr)"] = q_power_sf_provided
                else:
                 ...
                 log.warning(f"Dados S/F insuficientes ou inválidos para config {scenario_suffix} no Tap {res_dict.get('Tap')}. V: {cap_bank_voltage_sf}, Req P: {cap_bank_power_sf_required}")
        ...
        if "N/A" not in cs_config_str and "N/A" not in q_config_str:
             ...
             log.info(f"Configuração do banco de capacitores: Tensão={max_test_voltage_kv_overall:.1f}kV (Banco {target_v_cf_key}kV), Potência={q_power_mvar_provided_overall:.1f}MVAr")
        else:
            fail_reason = cs_config_str if "N/A" in cs_config_str else q_config_str # Get the reason
            log.warning(f"Não foi possível sugerir a configuração do banco de capacitores: {fail_reason}")
        ...
        class ParameterAnalyzer:
            ...
            def __init__(self):
                self.type_keywords = {
                    ...
                    'pteste': ['Pteste', 'Cap Bank Power', 'Q Power Provided'] # Includes MVA and MVAr (required and provided)
                }
                cap_bank_voltages_num = sorted([float(v) for v in CAPACITORS_BY_VOLTAGE.keys()])
                high_voltage_threshold = 95.6 if 95.6 in cap_bank_voltages_num else (cap_bank_voltages_num[-1] if cap_bank_voltages_num else 100.0)
                ...
                self.highlight_colors = {
                    'tensao': {'backgroundColor': '#ffcdd2', 'color': 'black'}, # Soft red for voltage (kV)
                    'corrente': {'backgroundColor': '#ffcc80', 'color': 'black'}, # Soft orange for current (A)
                    'perdas': {'backgroundColor': '#d1c4e9', 'color': 'black'}, # Soft violet for losses (kW)
                    'pteste_high': {'backgroundColor': '#ffb74d', 'color': 'black'}, # Deep orange for high power (MVA/MVAr)
                    'pteste_medium': {'backgroundColor': '#fff59d', 'color': 'black'} # Soft yellow for medium power (MVA/MVAr)
                }
            ...
            def get_param_type(self, param_name):
                ...
                if any(keyword in param_name for keyword in ["Pteste", "Cap Bank Q Disp.", "Q Power Provided"]):
                    return 'pteste'
        ...
        class CapBankStatusAnalyzer:
             ...
             def __init__(self):
                 ...
                 self.tensao_eval_fator = 1.1 # Factor to evaluate Test Voltage against Bank Voltage C/F
             ...
             def validate_inputs(self, test_voltage, cap_bank_voltage_cf, power_cf_required, cap_bank_voltage_sf=None, power_sf_required=None, corrente=None, potencia_ativa=None):
                 ...
                 try:
                     if any(v is None for v in [test_voltage, cap_bank_voltage_cf, power_cf_required]):
                         log.debug(f"Status input validation failed: test_v={test_voltage}, bank_v_cf={cap_bank_voltage_cf}, power_cf_req={power_cf_required}")
                         return None
                     result = {
                     ...
                     log.error(f"Status input conversion failed: test_v={test_voltage}, bank_v_cf={cap_bank_voltage_cf}, power_cf_req={power_cf_required}, bank_v_sf={cap_bank_voltage_sf}, power_sf_req={power_sf_required}, corrente={corrente}, potencia_ativa={potencia_ativa}")
                     return None
             ...
             def get_status(self, test_voltage, cap_bank_voltage_cf, power_cf_required, cap_bank_voltage_sf=None, power_sf_required=None, corrente=None, potencia_ativa=None):
                 ...
                 inputs = self.validate_inputs(test_voltage, cap_bank_voltage_cf, power_cf_required, cap_bank_voltage_sf, power_sf_required, corrente)
                 if inputs is None: return "N/A (Dados Status Inválidos)"

                 status_parts = []; events = 0
                 ...
                 pwr_disp_cf = f"{inputs['power_cf_req']:.1f}" if inputs['power_cf_req'] is not None and not math.isinf(inputs['power_cf_req']) else "Inf"
                 pwr_disp_sf = f"{inputs['power_sf_req']:.1f}" if inputs['power_sf_req'] is not None and not math.isinf(inputs['power_sf_req']) else "Inf"
                 ...
                 # Then alert messages (if not critical)
                      ...
                      events += 1
                 elif potencia_status['alert_sf']: # Only show S/F alert if C/F wasn't alert/critical
                      status_parts.append(f"Cap Bank ↑ ({self.potencia_alerta_threshold:.1f}+ MVAr)")
                      events += 1
        ...
        class TableCellFormatter:
            ...
            def format_value(self, value, precision=2):
                ...
                if isinstance(value, (int, float)):
                    ...
                    except ValueError: return str(value) # Fallback
            ...
            def create_cell(self, value, row_idx, col_idx, apply_highlighting=False, param_type=None, param_name=None, precision=2):
                ...
                if is_split_cell:
                    ...
                    sf_config_str_safe = self.format_config_string(sf_config_str) # Format for tooltip
                    cf_config_str_safe = self.format_config_string(cf_config_str) # Format for tooltip
                    ...
                    values_are_equal = (values_are_equal and (configs_are_equal or (not has_sf_tooltip and not has_cf_tooltip))) or \
                                      (is_cap_bank_voltage and values_are_equal) or \
                                      is_cap_bank_q
                    ...
                    if values_are_equal:
                        ...
                        if is_cap_bank_q and has_sf_display and has_cf_display and sf_numeric_val is not None and cf_numeric_val is not None:
                            ...
                            if abs(sf_numeric_val - cf_numeric_val) >= 0.001:
                                single_content = [
                                    html.Span(sf_numeric_text, style={'fontSize': '0.9em', 'fontWeight': 'bold', 'color': 'black'}),
                                    html.Sup(" S/F", style={'fontSize': '0.65em', 'color': COLORS['text_muted'], 'marginLeft': '2px'}),
                                    html.Span(" / ", style={'fontSize': '0.9em', 'color': 'black'}),
                                    html.Span(cf_numeric_text, style={'fontSize': '0.9em', 'fontWeight': 'bold', 'color': 'black'}),
                                    html.Sup(" C/F", style={'fontSize': '0.65em', 'color': COLORS['text_muted'], 'marginLeft': '2px'})
                                ]
                            else:
                                # Values are the same, show as S/F=C/F
                                single_content = [
                                    html.Span(sf_numeric_text, style={'fontSize': '0.9em', 'fontWeight': 'bold', 'color': 'black'}),
                                    html.Sup(" S/F=C/F", style={'fontSize': '0.65em', 'color': COLORS['text_muted'], 'marginLeft': '2px'})
                                ]
                        else:
                        ...
                        single_div = html.Div(single_content, id=single_id, style={
                            'width': '100%',
                            'textAlign': 'center',
                            'padding': '0.15rem',
                            'backgroundColor': sf_highlight_style.get('backgroundColor', 'transparent'),
                            'cursor': 'help' if (has_sf_tooltip or has_cf_tooltip) else 'default'
                        })
                        ...
                        sf_content_inner = []
                        if has_sf_display:
                            sf_content_inner.append(html.Span(sf_numeric_text, style={'fontSize': '0.9em', 'fontWeight': 'bold', 'color': 'black'}))
                            sf_content_inner.append(html.Sup(" S/F", style={'fontSize': '0.7em', 'color': COLORS['text_muted'], 'marginLeft': '2px'}))
                        else:
                            sf_content_inner.append("-") # Display hyphen if no numeric value

                        sf_div = html.Div(sf_content_inner, id=sf_id_str if has_sf_tooltip else f"sf-no-tooltip-{base_unique_id}", style={
                            'width': '50%',
                            'float': 'left',
                        ...
                        cf_content_inner = []
                        if has_cf_display:
                            cf_content_inner.append(html.Span(cf_numeric_text, style={'fontSize': '0.9em', 'fontWeight': 'bold', 'color': 'black'}))
                            cf_content_inner.append(html.Sup(" C/F", style={'fontSize': '0.7em', 'color': COLORS['text_muted'], 'marginLeft': '2px'}))
                        else:
                            cf_content_inner.append("-") # Display hyphen if no numeric value

                        cf_div = html.Div(cf_content_inner, id=cf_id_str if has_cf_tooltip else f"cf-no-tooltip-{base_unique_id}", style={
                            'width': '50%',
                            'float': 'right',
                            'padding': '0.15rem',
                            'textAlign': 'center', # Center align the content
                            'backgroundColor': cf_highlight_style.get('backgroundColor', 'transparent'),
                    ...
                    container_div = html.Div(cell_content_components, style={'overflow': 'hidden', 'height': '100%'})

                    return html.Td(container_div, style=cell_style, className="table-value-cell split-cell")
                ...
                # Handle regular dual value [sem_fator, com_fator] (e.g., Cap Bank Voltage/Power)
                     ...
                     # Display Logic
                     if has_sf and has_cf and abs(float(sem_fator or 0) - float(com_fator or 0)) >  epsilon: # Show both if different
                         return html.Td([
                             html.Span(sem_fator_text, style={'fontSize': '0.9em'}), html.Sup(" S/F", style={'fontSize': '0.7em', 'color': COLORS['text_muted']}), " / ",
                             html.Span(com_fator_text, style={'fontSize': '0.9em'}), html.Sup(" C/F", style={'fontSize': '0.7em', 'color': COLORS['text_muted']})
                         ], style=cell_style, className="table-value-cell")
                     elif has_sf: # Show only S/F if C/F missing or same
                         return html.Td([html.Span(sem_fator_text, style={'fontSize': '0.9em'}), html.Sup(" S/F", style={'fontSize': '0.7em', 'color': COLORS['text_muted']})], style=cell_style, className="table-value-cell")
                     elif has_cf: # Show only C/F if S/F missing
                         return html.Td([html.Span(com_fator_text, style={'fontSize': '0.9em'}), html.Sup(" C/F", style={'fontSize': '0.7em', 'color': COLORS['text_muted']})], style=cell_style, className="table-value-cell")
                    ...
                    if apply_highlighting and param_type is not None and param_type != 'default':
                        ...
                        except (ValueError, TypeError): pass # Ignore highlighting if not numeric
        ...
        class StatusStyler:
             def __init__(self, column_widths):
                 self.column_widths = column_widths
                 self.base_style = {**TABLE_STATUS_STYLE, 'verticalAlign': 'middle', 'fontSize': '0.7rem', 'padding': '0.2rem'} # Adjust padding
                 # Define styles based on keywords in the status string
                 self.status_styles = {
                     '(V)': {"color": CONFIG_COLORS['danger_text'], "backgroundColor": "transparent", "fontWeight": "bold"}, # Sem fundo para voltage
                     '(A)': {"color": CONFIG_COLORS['danger_text'], "backgroundColor": "transparent", "fontWeight": "bold"}, # Sem fundo para current
                     '(P)': {"color": CONFIG_COLORS['danger_text'], "backgroundColor": "transparent", "fontWeight": "bold"}, # Sem fundo para active power
                     f'{cap_bank_analyzer.potencia_critica_threshold:.1f}+': {"color": CONFIG_COLORS['danger_text'], "backgroundColor": "transparent", "fontWeight": "bold"}, # Sem fundo para critical power
                     f'{cap_bank_analyzer.potencia_alerta_threshold:.1f}+': {"color": CONFIG_COLORS['warning_text'], "backgroundColor": "transparent", "fontWeight": "bold"}, # Sem fundo para alert power
                     'eventos': {"color": "#6a1b9a", "backgroundColor": "transparent", "fontWeight": "bold"}, # Sem fundo para eventos
                     'OK': {"color": CONFIG_COLORS['ok_text'], "backgroundColor": "transparent"}, # Green text, no background
                     'N/A': {"color": COLORS['text_muted'], "backgroundColor": "transparent"}, # Gray text, no background
                 }
             ...
             def get_style(self, status_text, col_idx):
                 ...
                 style = {**self.base_style, "width": width, "maxWidth": width, "minWidth": width, "textAlign": "center"} # Center align status
                 ...
                 elif '(' in status_text and ')' in status_text and not 'Inv' in status_text: applied_style = self.status_styles['eventos']
            ...
            for row_idx, row_data in enumerate(rows_data):
                ...
                for i, value in enumerate(row_data[1:]):
                    ...
                    if param_name == "Status":
                        ...
                        elif '(A)' in param_name: precision = 2  # Changed from 1 to 2 decimal places for current values
                        ...
                        cell = create_table_cell(value, row_idx, col_index, apply_highlighting, param_type, param_name, precision=precision)
                        cells.append(cell)
        ...
        rows_frio_keys = [
            ...
            ("Corrente frio (A)", 'Corrente frio (A)'), # Changed from split cell to simple string key
            ("Pteste frio (MVA)", 'Pteste frio (MVA)'),
            ("Potencia Ativa Frio (kW)", 'Potencia Ativa EPS Frio (kW)'),
            ("Cap Bank Power Frio Req (MVAr)", ['Cap Bank Power Frio Sem Fator (MVAr)', 'Cap Bank Power Frio Com Fator (MVAr)']), # Dual Value (Required Power)
            ("Cap Bank V Disp. (kV)", {'split_cell': True, 'sf': {'numeric':'Cap Bank Voltage Frio Sem Fator (kV)', 'config':'CS Config Frio S/F'}, 'cf': {'numeric':'Cap Bank Voltage Frio Com Fator (kV)', 'config':'CS Config Frio'}}), # Split cell config
            ("Cap Bank Q Disp. (MVAr)", {'split_cell': True, 'sf': {'numeric':'Q Power Provided Frio S/F (MVAr)', 'config':'Q Config Frio S/F'}, 'cf': {'numeric':'Q Power Provided Frio (MVAr)', 'config':'Q Config Frio'}}), # Split cell config
            ("Status", ('Tensão frio (kV)', 'Cap Bank Voltage Frio Com Fator (kV)', 'Cap Bank Power Frio Com Fator (MVAr)', 'Cap Bank Voltage Frio Sem Fator (kV)', 'Cap Bank Power Frio Sem Fator (MVAr)', 'Corrente frio (A)', 'Potencia Ativa EPS Frio (kW)')) # Status Tuple (Uses REQUIRED power)
        ]
        rows_quente_keys = [
             ("Tensão quente (kV)", 'Tensão quente (kV)'),
             ("Corrente quente (A)", 'Corrente quente (A)'), # Changed from split cell to simple string key
        ...
        rows_25c_keys = [
             ...
             ("Corrente 25°C (A)", 'Corrente 25°C (A)'), # Changed from split cell to simple string key
        ...
        if overload_applicable:
             rows_1_2_keys = [
                 ...
                 ("Corrente 1.2 pu (A)", 'Corrente 1.2 pu (A)'), # Changed from split cell to simple string key
             ...
             rows_1_4_keys = [
                 ...
                 ("Corrente 1.4 pu (A)", 'Corrente 1.4 pu (A)'), # Changed from split cell to simple string key
            ...
            for param_name, keys in keys_list:
                ...
                for res_dict in results_list:
                    if isinstance(keys, str):
                        ...
                        row.append({
                            'split_cell': True,
                            'sf': {'numeric': res_dict.get(sf_num_key), 'config': res_dict.get(sf_cfg_key)},
                            'cf': {'numeric': res_dict.get(cf_num_key), 'config': res_dict.get(cf_cfg_key)}
                        })
        ...
        legend = html.Div([
            dbc.Card([
                dbc.CardHeader(html.H6("LEGENDA", className="text-center m-0", style=CARD_HEADER_STYLE), style=COMPONENTS['card_header']),
                dbc.CardBody([
                    # Tabela de Status
                    html.Table([
                        html.Thead(html.Tr([
                            html.Th("Status", style={'fontSize': '0.8rem', 'textAlign': 'center', 'color': COLORS['text_header'], 'backgroundColor': COLORS['background_header'], 'padding': '4px'})
                        ])),
                        html.Tbody(html.Tr([
                            html.Td([
                                html.Div([
                                    html.Div([html.Span("(V) > Limite", style={**status_styler.status_styles['(V)'], "backgroundColor": COLORS['background_faint']})], style={'flex': '1', 'minWidth': '120px', 'textAlign': 'center'}),
                                    html.Div([html.Span("(A) > Limite", style={**status_styler.status_styles['(A)'], "backgroundColor": COLORS['background_faint']})], style={'flex': '1', 'minWidth': '120px', 'textAlign': 'center'}),
                                    html.Div([html.Span("(P) > Limite", style={**status_styler.status_styles['(P)'], "backgroundColor": COLORS['background_faint']})], style={'flex': '1', 'minWidth': '120px', 'textAlign': 'center'}),
                                    html.Div([html.Span(f"Cap Bank ↑ ({cap_bank_analyzer.potencia_critica_threshold:.1f}+ MVAr)", style={**status_styler.status_styles[f'{cap_bank_analyzer.potencia_critica_threshold:.1f}+'], "backgroundColor": COLORS['background_faint']})], style={'flex': '1', 'minWidth': '150px', 'textAlign': 'center'}),
                                    html.Div([html.Span(f"Cap Bank ↑ ({cap_bank_analyzer.potencia_alerta_threshold:.1f}+ MVAr)", style={**status_styler.status_styles[f'{cap_bank_analyzer.potencia_alerta_threshold:.1f}+'], "backgroundColor": COLORS['background_faint']})], style={'flex': '1', 'minWidth': '150px', 'textAlign': 'center'}),
                                    html.Div([html.Span("OK", style={**status_styler.status_styles['OK'], "backgroundColor": COLORS['background_faint']})], style={'flex': '1', 'minWidth': '60px', 'textAlign': 'center'})
                                ], style={'display': 'flex', 'flexWrap': 'wrap', 'justifyContent': 'space-between', 'gap': '5px', 'padding': '5px'})
                            ], style={'fontSize': '0.75rem'})
                        ]))
                    ], style={'width': '100%', 'borderCollapse': 'collapse', 'border': f'1px solid {COLORS["border"]}', 'marginBottom': '10px'}),

                    # Explicação S/F e C/F
                    html.Table([
                        html.Thead(html.Tr([
                            html.Th("Significado S/F e C/F", style={'fontSize': '0.8rem', 'textAlign': 'center', 'color': COLORS['text_header'], 'backgroundColor': COLORS['background_header'], 'padding': '4px'})
                        ])),
                        html.Tbody([
                            html.Tr([
                                html.Td([
                                    html.Div([
                                        html.Div([html.Span("S/F:", style={'fontWeight': 'bold'}), " Banco V_teste ≤ V_banco"], style={'flex': '1', 'textAlign': 'center'}),
                                        html.Div([html.Span("C/F:", style={'fontWeight': 'bold'}), " Banco V_teste > V_banco × 1.1"], style={'flex': '1', 'textAlign': 'center'})
                                    ], style={'display': 'flex', 'justifyContent': 'space-between', 'alignItems': 'center', 'padding': '5px', 'gap': '10px'})
                            ...
                            html.Tr([
                                html.Td([
                                    html.Div([
                                        ...
                                        html.Div([
                                            html.Div([
                                                html.Div([html.Span("12.3"), html.Sup(" S/F")], style={'width': '50%', 'float': 'left', 'borderRight': f'1px solid {COLORS["border"]}', 'padding': '2px', 'textAlign': 'center'}),
                                                html.Div([html.Span("13.8"), html.Sup(" C/F")], style={'width': '50%', 'float': 'right', 'padding': '2px', 'textAlign': 'center'})
                                            ], style={'border': f'1px solid {COLORS["border_light"]}', 'overflow': 'hidden', 'width': '120px', 'margin': '0 auto', 'borderRadius': '3px', 'backgroundColor': COLORS['background_faint'], 'fontSize': '0.8em', 'fontWeight': 'bold'}),
                                            html.Div("Valores diferentes", style={'fontSize': '0.65rem', 'color': COLORS['text_muted'], 'marginTop': '2px', 'textAlign': 'center'})
                                        ], style={'flex': '1', 'textAlign': 'center'}),

                                        # Exemplo de célula única (valores iguais)
                                        html.Div([
                                            html.Div([
                                                html.Span("12.3"), html.Sup(" S/F=C/F")
                                            ], style={'border': f'1px solid {COLORS["border_light"]}', 'padding': '2px', 'textAlign': 'center', 'width': '120px', 'margin': '0 auto', 'borderRadius': '3px', 'backgroundColor': COLORS['background_faint'], 'fontSize': '0.8em', 'fontWeight': 'bold'}),
                                            html.Div("Valores iguais", style={'fontSize': '0.65rem', 'color': COLORS['text_muted'], 'marginTop': '2px', 'textAlign': 'center'})
                                        ], style={'flex': '1', 'textAlign': 'center'})
                                    ], style={'display': 'flex', 'justifyContent': 'space-between', 'alignItems': 'center', 'padding': '5px', 'gap': '10px'})
                                ], style={'padding': '4px', 'fontSize': '0.7rem'})
                            ])
                        ])
                    ], style={'width': '100%', 'borderCollapse': 'collapse', 'border': f'1px solid {COLORS["border"]}', 'marginBottom': '5px'})
                ], style={**COMPONENTS['card_body'], "backgroundColor": COLORS['background_card'], "padding": "0.75rem"})
            ], style=COMPONENTS['card'])
        ], className="mt-3")
        ...
        def create_sut_eps_analysis_table_component_compensated(analysis_results):
             ...
             taps_info = analysis_results.get('taps_info', []) # List of {'tap_sut_kv': ..., 'corrente_eps_a': ..., 'percent_limite': ...}

             if status_msg != 'OK':
                 status_color = COLORS['danger']
                 return html.Div(status_msg, style={"fontSize": "0.7rem", "textAlign": "center", "padding": "0.5rem", "color": status_color, "fontWeight": "bold"})

             if not taps_info:
                 return html.Div("Nenhum tap SUT aplicável encontrado.", style={"fontSize": "0.7rem", "textAlign": "center", "padding": "0.5rem", "color": COLORS['text_muted']})

             header_style = {**TABLE_HEADER_STYLE_SM, "backgroundColor": COLORS['background_header'], "color": COLORS['text_header'], 'padding': '0.2rem'}
             ...
             current_styles = {
                 "negative": {**cell_style, "backgroundColor": CONFIG_COLORS['info_bg_faint'], "color": "blue", "fontStyle": "italic"},  # Azul claro para correntes negativas
                 "low": {**cell_style, "backgroundColor": CONFIG_COLORS['ok_bg_faint'], "color": "black"},  # Verde claro para < 50%
                 "medium": {**cell_style, "backgroundColor": CONFIG_COLORS['warning_bg_faint'], "color": "black"},  # Amarelo claro para 50-85%
                 "high": {**cell_style, "backgroundColor": CONFIG_COLORS['warning_high_bg_faint'], "color": "black"},  # Laranja claro para 85-100%
                 "critical": {**cell_style, "backgroundColor": CONFIG_COLORS['danger_bg'], "color": "black"}  # Vermelho claro para > 100%, texto preto para melhor legibilidade
             }
             def get_style(percent):
                 if percent is None or math.isnan(percent): return cell_style
                 # Verificar se é negativo (excesso de compensação)
                 if percent < 0: return current_styles["negative"]
             ...
             header = html.Thead([
                 ...
                 html.Tr([
                     html.Th("Tap SUT (kV)", style={**tap_header_style, "borderBottom": "none"}, rowSpan=2),
                     html.Th("I EPS (A)", style={**current_header_style, "borderBottom": "none"}, colSpan=2)
                 ]),
             ...
             for info in taps_info: # Already sorted by voltage and limited
                 tap_kv_disp = f"{info['tap_sut_kv']:.1f}" if info.get('tap_sut_kv') is not None else "-"
             ...
             body = html.Tbody(rows)
             return dbc.Table([header, body], bordered=True, hover=True, striped=True, size="sm", style={"width": "100%", "tableLayout": "fixed", "marginBottom": "0"})


        # --- Build SUT/EPS Analysis Cards Manually (WITH COMPENSATION) ---
        sut_scenarios_info = {
            '25°C':   {'tensao_key': 'Tensão 25°C (kV)',   'corrente_key': 'Corrente 25°C (A)',   'q_power_key': 'Q Power Provided 25°C (MVAr)',   'cap_volt_key': 'Cap Bank Voltage 25°C Com Fator (kV)',   'title': 'ANÁLISE SUT/EPS: PERDAS CARGA (25°C)'},
            'Frio':   {'tensao_key': 'Tensão frio (kV)',   'corrente_key': 'Corrente frio (A)',   'q_power_key': 'Q Power Provided Frio (MVAr)',   'cap_volt_key': 'Cap Bank Voltage Frio Com Fator (kV)',   'title': 'ANÁLISE SUT/EPS: ENERGIZAÇÃO A FRIO'},
            'Quente': {'tensao_key': 'Tensão quente (kV)', 'corrente_key': 'Corrente quente (A)', 'q_power_key': 'Q Power Provided Quente (MVAr)', 'cap_volt_key': 'Cap Bank Voltage Quente Com Fator (kV)', 'title': 'ANÁLISE SUT/EPS: QUENTE COMPENSADO'}
        }
        if overload_applicable:
            sut_scenarios_info['1.2 pu'] = {'tensao_key': 'Tensão 1.2 pu (kV)', 'corrente_key': 'Corrente 1.2 pu (A)', 'q_power_key': 'Q Power Provided 1.2 pu (MVAr)', 'cap_volt_key': 'Cap Bank Voltage 1.2 pu Com Fator (kV)', 'title': 'ANÁLISE SUT/EPS: SOBRECARGA 1.2 PU'}
            sut_scenarios_info['1.4 pu'] = {'tensao_key': 'Tensão 1.4 pu (kV)', 'corrente_key': 'Corrente 1.4 pu (A)', 'q_power_key': 'Q Power Provided 1.4 pu (MVAr)', 'cap_volt_key': 'Cap Bank Voltage 1.4 pu Com Fator (kV)', 'title': 'ANÁLISE SUT/EPS: SOBRECARGA 1.4 PU'}
        ...
        for scen_key, scen_info in sut_scenarios_info.items():
            ...
            for res in resultados: # Iterate through Nominal, Menor, Maior results
                ...
                analysis_result = {'status': 'Erro nos dados de entrada SUT', 'taps_info': []} # Default
                if tensao_ref_dut_kv is not None and corrente_ref_dut_a is not None:
                    ...
                    if len(taps_sut_hv_v) == 0:
                        analysis_result = {'status': f'Faixa SUT AT inválida', 'taps_info': []}
                    else:
                        ...
                        for V_sut_hv_tap_v in top_5_taps_v:
                             ...
                             q_power_scenario_cf_mvar = res.get(f"Q Power Provided {scen_key} (MVAr)")  # C/F
                             ...
                             taps_info_list_compensated.append({
                                ...
                                'corrente_eps_sf_a': comp_result['corrente_eps_sf_a'],  # S/F current
                                'percent_limite_sf': comp_result['percent_limite_sf'],  # S/F percentage
                                'corrente_eps_cf_a': comp_result['corrente_eps_cf_a'],  # C/F current
                                'percent_limite_cf': comp_result['percent_limite_cf']   # C/F percentage
                             })
                ...
                sut_cols.append(dbc.Col([
                    html.Div(f"Tap {tap_label}", className="text-center py-1",
                             style={"fontSize": "0.7rem", "fontWeight": "bold", "color": COLORS['text_header'],
                                    "backgroundColor": COLORS['background_header'],
                                    "padding": "0.1rem 0", "marginBottom":"2px", "borderRadius":"2px 2px 0 0"}),
                    create_sut_eps_analysis_table_component_compensated(analysis_result) # Use the correct render func
                ], width=sut_col_width, className="px-1"))
            ...
            if not sut_cols:
                sut_body_content = html.Div("Erro ao gerar colunas SUT.", style=ERROR_STYLE)
                 ...
                 sut_body_content = html.Div("Dados SUT/EPS não disponíveis ou inválidos para análise compensada.",
                                             style={"fontSize": "0.8rem", "textAlign": "center", "padding": "1rem", "color": COLORS['text_muted']})
            else:
                 sut_body_content = dbc.Row(sut_cols, className="g-2 justify-content-center")

            sut_analysis_cards[scen_key] = dbc.Card([
                dbc.CardHeader(html.H6(scen_info['title'], className="text-center m-0", style=CARD_HEADER_STYLE)),
                dbc.CardBody(sut_body_content, style={**COMPONENTS['card_body'], "padding": "0.3rem"})
        ...
        detailed_results_children.append(
            dbc.Row([
                dbc.Col(dbc.Card([
                    dbc.CardHeader(html.H6("PERDAS EM CARGA 25°C (Resultados)", className="text-center m-0", style=CARD_HEADER_STYLE)),
                    dbc.CardBody(html.Div(table_25c, style=TABLE_WRAPPER_STYLE), style=COMPONENTS['card_body'])
                ], style={**COMPONENTS['card'], "height": "100%"}), width=12, lg=card_col_width_lg, className="mb-2 mb-lg-0"),
                dbc.Col(sut_analysis_cards.get('25°C', html.Div()), width=12, lg=sut_col_width_lg)
            ], className="mb-3 g-2 align-items-stretch") # Use align-items-stretch
        )
        ...
        detailed_results_children.append(
            dbc.Row([
                dbc.Col(dbc.Card([
                    dbc.CardHeader(html.H6("ENERGIZAÇÃO A FRIO (Resultados)", className="text-center m-0", style=CARD_HEADER_STYLE)),
                    dbc.CardBody(html.Div(table_frio, style=TABLE_WRAPPER_STYLE), style=COMPONENTS['card_body'])
                ], style={**COMPONENTS['card'], "height": "100%"}), width=12, lg=card_col_width_lg, className="mb-2 mb-lg-0"),
                dbc.Col(sut_analysis_cards.get('Frio', html.Div()), width=12, lg=sut_col_width_lg)
        ...
        detailed_results_children.append(
            dbc.Row([
                dbc.Col(dbc.Card([
                    dbc.CardHeader(html.H6("CONDIÇÃO A QUENTE (Resultados)", className="text-center m-0", style=CARD_HEADER_STYLE)),
                    dbc.CardBody(html.Div(table_quente, style=TABLE_WRAPPER_STYLE), style=COMPONENTS['card_body'])
                ], style={**COMPONENTS['card'], "height": "100%"}), width=12, lg=card_col_width_lg, className="mb-2 mb-lg-0"),
                dbc.Col(sut_analysis_cards.get('Quente', html.Div()), width=12, lg=sut_col_width_lg)
            ], className="mb-3 g-2 align-items-stretch")
        )
        ...
        if table_1_2:
            detailed_results_children.append(
                dbc.Row([
                    dbc.Col(dbc.Card([
                        dbc.CardHeader(html.H6("SOBRECARGA 1.2 PU (Resultados)", className="text-center m-0", style=CARD_HEADER_STYLE)),
                        dbc.CardBody(html.Div(table_1_2, style=TABLE_WRAPPER_STYLE), style=COMPONENTS['card_body'])
                    ], style={**COMPONENTS['card'], "height": "100%"}), width=12, lg=card_col_width_lg, className="mb-2 mb-lg-0"),
                    dbc.Col(sut_analysis_cards.get('1.2 pu', html.Div()), width=12, lg=sut_col_width_lg)
                ], className="mb-3 g-2 align-items-stretch")
            )
        ...
        if table_1_4:
            detailed_results_children.append(
                dbc.Row([
                     dbc.Col(dbc.Card([
                        dbc.CardHeader(html.H6("SOBRECARGA 1.4 PU (Resultados)", className="text-center m-0", style=CARD_HEADER_STYLE)),
                        dbc.CardBody(html.Div(table_1_4, style=TABLE_WRAPPER_STYLE), style=COMPONENTS['card_body'])
                    ], style={**COMPONENTS['card'], "height": "100%"}), width=12, lg=card_col_width_lg, className="mb-2 mb-lg-0"),
                    dbc.Col(sut_analysis_cards.get('1.4 pu', html.Div()), width=12, lg=sut_col_width_lg)
                ], className="mb-3 g-2 align-items-stretch")
            )
        ...
        if res_nom is None or res_menor is None or res_maior is None:
            missing_taps = [tap for tap, res in [('Nominal', res_nom), ('Menor', res_menor), ('Maior', res_maior)] if res is None]
        ...
        nominal_header = html.Thead(html.Tr([
        ...
        condicoes_nominais_content = html.Div(
            dbc.Table([nominal_header, html.Tbody(basic_rows_nominal + losses_rows_nominal)],
        ...
        # Initialize the resultados_perdas_carga section if it doesn't exist
        if 'resultados_perdas_carga' not in current_data:
            current_data['resultados_perdas_carga'] = {}
        ...
        error_div = html.Div(f"Erro ao calcular: Verifique os inputs numéricos. Detalhe: {str(e)}", style=ERROR_STYLE)
        return initial_detailed_content, error_div, no_update
    except KeyError as e:
         log.error(f"KeyError in handle_perdas_carga: Missing key {e}. Transformer Data: {transformer_data.keys() if transformer_data else 'None'}. Losses Data: {losses_data.keys() if losses_data else 'None'}")
         error_div = html.Div(f"Erro: Dado necessário ({e}) não encontrado. Verifique os dados do transformador e o cálculo de perdas em vazio.", style=ERROR_STYLE)
         return initial_detailed_content, error_div, no_update
    except Exception as e:
        log.exception(f"Exception in handle_perdas_carga: {e}") # Log full traceback
        error_div = html.Div(f"Erro inesperado ao calcular perdas em carga: {str(e)}", style=ERROR_STYLE)
        return initial_detailed_content, error_div, no_update