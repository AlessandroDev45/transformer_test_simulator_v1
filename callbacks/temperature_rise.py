...
from utils.routes import ROUTE_TEMPERATURE_RISE, normalize_pathname  # Para normalização de pathname
...
def update_temperature_rise_page_info_panel(global_panel_content):
    ...
    log.debug(
        "Atualizando painel de informações do transformador na página de elevação de temperatura"
    )
...
def temperature_rise_load_data(pathname, transformer_data, stored_temp_rise_data):
    ...
    if normalized_path == ROUTE_TEMPERATURE_RISE:
        should_process = True
        log.info("[LOAD TempRise] Estamos na página de Elevação de Temperatura. Processando.")
    # Caso 2: Trigger é o store global (para pegar delta_theta)
    elif triggered_id == "transformer-inputs-store":
        should_process = True
        log.info("[LOAD TempRise] Processando atualização de transformer-inputs-store.")
    # Caso 3: Estamos em outra página e o trigger é a URL
    elif triggered_id == "url" and normalized_path != ROUTE_TEMPERATURE_RISE:
        log.debug(
            f"[LOAD TempRise] Pathname '{pathname}' não é '{ROUTE_TEMPERATURE_RISE}'. Abortando trigger de URL."
        )
        raise PreventUpdate
    ...
    local_data = (
        stored_temp_rise_data
        if stored_temp_rise_data and isinstance(stored_temp_rise_data, dict)
        else {}
    )
    global_data = (
        transformer_data if transformer_data and isinstance(transformer_data, dict) else {}
    )
    ...
    log.debug(
        f"[LOAD TempRise] Dados lidos do local store: Inputs={inputs_local}, Resultados={results_local}"
    )
    ...
    if delta_theta_oil_max_global_raw is not None:
        ...
        log.info(
            f"[LOAD TempRise] Usando 'elevacao_oleo_topo' ({delta_theta_oil_max_final}) do global store."
        )
        if delta_theta_oil_max_final is None:
            log.warning(
                f"[LOAD TempRise] Falha ao converter 'elevacao_oleo_topo' ({delta_theta_oil_max_global_raw}). Fallback para local."
            )
            delta_theta_oil_max_final = inputs_local.get(
                "input_delta_theta_oil_max"
            )  # Já deve ser float
    else:
        delta_theta_oil_max_final = inputs_local.get("input_delta_theta_oil_max")
        log.info(
            f"[LOAD TempRise] Global 'elevacao_oleo_topo' não disponível. Usando local: {delta_theta_oil_max_final}"
        )
    ...
    values_to_return = (
        inputs_local.get("input_ta"),
        inputs_local.get("input_material", "cobre"),
        inputs_local.get("input_rc"),
        inputs_local.get("input_tc"),
        inputs_local.get("input_rw"),
        inputs_local.get("input_t_oil"),
        delta_theta_oil_max_final,  # Valor numérico
        results_local.get("avg_winding_temp"),
        results_local.get("avg_winding_rise"),
        results_local.get("top_oil_rise"),
        results_local.get("ptot_used_kw"),
        results_local.get("tau0_h"),
        display_message,
    )
...
def debug_calc_button(n_clicks):
    ...
    return html.Div(
        f"Botão TESTE clicado: {n_clicks} vezes", style={"color": "red", "fontSize": "0.8rem"}
    )
...
def temperature_rise_calculate(
    ...
    res_hot_str,
    temp_top_oil_str,
    delta_theta_oil_max_str,  # Inputs locais
    transformer_data,
    losses_data,
    pathname,
):  # Dados globais e pathname
    """
    ...
    log.info(
        f"***** [CALC TempRise] CALLBACK INICIADO - Botão {button_name}, n_clicks = {n_clicks}, Trigger: {triggered_id} *****"
    )
    print(
    ...
    input_values_local = {
        "ta": safe_float(temp_amb_str),
        "material": winding_material,
        "rc": safe_float(res_cold_str),
        "tc": safe_float(temp_cold_str),
        "rw": safe_float(res_hot_str),
        "t_oil": safe_float(temp_top_oil_str),
    ...
    if errors:
        ...
        error_msg_div = html.Ul(
            [html.Li(e) for e in errors],
            style={"color": colors.get("fail", "red"), "fontSize": "0.7rem"},
        )
    ...
    temp_amb = input_values_local["ta"]
    material = input_values_local["material"]
    rc = input_values_local["rc"]
    tc = input_values_local["tc"]
    rw = input_values_local["rw"]
    t_oil = input_values_local["t_oil"]
    delta_theta_oil_max_ui = input_values_local["delta_theta_oil_max_ui"]

    # --- 2. Ler e processar dados globais para Tau0 ---
    global_transformer_data = (
        transformer_data if transformer_data and isinstance(transformer_data, dict) else {}
    )
    global_losses_data = losses_data if losses_data and isinstance(losses_data, dict) else {}
    log.debug(
        f"[CALC TempRise] Dados globais lidos: Trafo={global_transformer_data}, Losses={global_losses_data}"
    )
    ...
    perdas_vazio_kw = safe_float(
        global_losses_data.get("resultados_perdas_vazio", {}).get("perdas_vazio_kw")
    )
    perdas_carga_min_kw = safe_float(
        global_losses_data.get("resultados_perdas_carga", {}).get("perdas_carga_min")
    )
    delta_theta_oil_max_global = safe_float(global_transformer_data.get("elevacao_oleo_topo"))

    delta_theta_oil_max_calc = (
        delta_theta_oil_max_ui if delta_theta_oil_max_ui is not None else delta_theta_oil_max_global
    )
    log.debug(
        f"[CALC TempRise] Delta Theta Max UI: {delta_theta_oil_max_ui}, Global: {delta_theta_oil_max_global}, Usado: {delta_theta_oil_max_calc}"
    )
    ...
    error_message = ""
    warning_message_tau0 = ""
    try:
        log.info(
            f"[CALC TempRise] Chamando calculate_winding_temps com rc={rc}, tc={tc}, rw={rw}, temp_amb={temp_amb}, material={material}"
        )
        avg_winding_temp, avg_winding_rise = calculate_winding_temps(rc, tc, rw, temp_amb, material)
        log.info(
            f"[CALC TempRise] Resultado calculate_winding_temps: avg_winding_temp={avg_winding_temp}, avg_winding_rise={avg_winding_rise}"
        )

        log.info(
            f"[CALC TempRise] Chamando calculate_top_oil_rise com t_oil={t_oil}, temp_amb={temp_amb}"
        )
        ...
        log.info(
            f"[CALC TempRise] Cálculos básicos OK: Tw={avg_winding_temp:.1f}, ΔTw={avg_winding_rise:.1f}, ΔToil={top_oil_rise:.1f}"
        )

        # Cálculo tau0
        required_tau0 = {
            "Ptot": ptot_used_kw,
            "ΔΘoil_max": delta_theta_oil_max_calc,
            "Peso Total": peso_total_ton,
            "Peso Óleo": peso_oleo_ton,
        }
        missing_tau0 = [name for name, val in required_tau0.items() if val is None]
        if missing_tau0:
            warning_message_tau0 = (
                f"Aviso: τ₀ não calculado. Dados faltantes: {', '.join(missing_tau0)}."
            )
            ...
            log.debug(
                f"[CALC TempRise] Calculando τ₀ com: Ptot={ptot_used_kw:.2f}kW, ΔΘmax={delta_theta_oil_max_calc:.1f}K, mT={peso_total_kg:.0f}kg, mO={peso_oleo_kg:.0f}kg"
            )
            tau0_h = calculate_thermal_time_constant(
                ptot_used_kw, delta_theta_oil_max_calc, peso_total_kg, peso_oleo_kg
            )
            if tau0_h is None:
                warning_message_tau0 = "Aviso: Erro interno ao calcular τ₀."
                log.error(f"[CALC TempRise] {warning_message_tau0}")
            else:
                log.info(f"[CALC TempRise] τ₀ calculado: {tau0_h:.2f} h")

    except ValueError as ve:
        log.warning(f"[CALC TempRise] Erro cálculo: {ve}")
        error_message = f"Erro: {ve}"
        avg_winding_temp = avg_winding_rise = top_oil_rise = tau0_h = None
    except Exception as e:
        log.exception("[CALC TempRise] Erro inesperado")
    ...
    store_to_save = {
        "inputs_temp_rise": {  # Salva os valores originais da UI
            "input_ta": temp_amb_str,
            "input_material": material,
            "input_rc": res_cold_str,
            "input_tc": temp_cold_str,
            "input_rw": res_hot_str,
            "input_t_oil": temp_top_oil_str,
            "input_delta_theta_oil_max": delta_theta_oil_max_str,
        },
        "resultados_temp_rise": {  # Salva resultados numéricos e mensagem
            "avg_winding_temp": round(avg_winding_temp, 1)
            if avg_winding_temp is not None
            else None,
            "avg_winding_rise": round(avg_winding_rise, 1)
            if avg_winding_rise is not None
            else None,
    ...
    if final_message:
        ...
        display_message = html.Div(
            "Cálculo realizado com sucesso!", style={"color": "green", "fontSize": "0.7rem"}
        )
