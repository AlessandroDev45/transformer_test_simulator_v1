...
from utils.store_utils import prepare_data_for_store, update_app_cache
...
@app.callback(
    ...
    [Input("url", "pathname"), Input("search-button", "n_clicks")],
...
def load_history_table(pathname, n_clicks, search_term):
    ...
    print(
        f"[DEBUG] Callback load_history_table acionado. Pathname: {pathname}, Trigger: {ctx.triggered_id}"
    )

    if pathname != "/historico" and ctx.triggered_id != "search-button":
        print(
            f"[DEBUG] PreventUpdate em load_history_table. Pathname: {pathname}, Trigger: {ctx.triggered_id}"
        )
        raise PreventUpdate
    ...
    try:
        ...
        for session in sessions:
            ...
            row = html.Div(
                [
                    ...
                    html.Div(
                        [
                            html.Button(
                                "Carregar",
                                id={"type": "load-btn", "index": session_id},
                                className="btn btn-sm btn-primary me-2",
                            ),
                            html.Button(
                                "Excluir",
                                id={"type": "delete-btn", "index": session_id},
                ...
                className="row",
                style={"borderBottom": f'1px solid {COLORS["border"]}', "padding": "5px 0"},
            )
            rows.append(row)
...
def save_current_session(n_clicks, session_name, notes, *transformer_store_data):
    ...
    if not session_name:
        return (
            dbc.Alert("É necessário fornecer um nome para a sessão.", color="danger"),
            no_update,
            no_update,
            no_update,
        )
    ...
    if session_name_exists(session_name):
        return (
            dbc.Alert(
                f"Já existe uma sessão com o nome '{session_name}'. Por favor, escolha outro nome.",
                color="warning",
            ),
            no_update,
            no_update,
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print("[DEBUG DB] INSPEÇÃO DETALHADA DOS DADOS RECEBIDOS (ANTES DA PREPARAÇÃO)")
        print(f"{'='*80}")
            ...
            if data is None:
                ...
                if num_keys > 0:
                    ...
                    for k, v in list(data.items())[:3]:
                        ...
                        print(
                            f"    '{k}': (Tipo: {v_type}) {v_repr}{'...' if len(repr(v)) > 60 else ''}"
                        )
                    ...
                    for k, v in data.items():
                        if v is not None:
                            ...
                            break
                    if not has_values:
                        print("   ⚠ AVISO: Todos os valores são None!")
                        log.warning(
                            f"[HISTORY SAVE] Store {store_name}: Todos os valores são None!"
                        )
                else:
                    print("  Conteúdo: Dicionário Vazio {}")
                    log.info("    -> Conteúdo: Dicionário Vazio {}")
                ...
                if num_items > 0:
                    ...
                    for item in data[:3]:
                        ...
                        print(
                            f"    - (Tipo: {item_type}) {item_repr}{'...' if len(repr(item)) > 60 else ''}"
                        )
                        log.debug(f"      -> Item Lista: Tipo={item_type}")
                else:
                    print("  Conteúdo: Lista Vazia []")
                    log.info("    -> Conteúdo: Lista Vazia []")
            else:
                print(f"  Tipo: {type(data).__name__}")
                print(f"  Conteúdo: {repr(data)[:100]}{'...' if len(repr(data)) > 100 else ''}")
                log.warning(
                    f"    -> Store '{store_name}' não é dict/list/None, é {type(data).__name__}"
                )
        ...
        if not any_data_found:
            print(
                "!!! AVISO: Nenhum dado útil encontrado em nenhum dos stores no momento do salvamento! Verifique se os dados estão sendo preenchidos e salvos nos stores corretos antes."
            )
            log.warning("[HISTORY SAVE] Nenhum dado útil encontrado nos stores para salvar.")
            return (
                dbc.Alert("Nenhum dado para salvar. Preencha as seções primeiro.", color="warning"),
                no_update,
            )
        ...
        for i, (name, data) in enumerate(zip(store_names, store_data_list)):
            ...
            if data is None:
                ...
                keys = list(data.keys())
                print(
                    f"{i+1}. {name}: dict com {len(keys)} chaves: {keys[:10]}{'...' if len(keys) > 10 else ''}"
                )
            ...
            if isinstance(data, dict):
                ...
                if data.get("_conversion_failed"):
                    ...
                    if "_problematic_paths" in data and data["_problematic_paths"]:
                        print("   Caminhos problemáticos:")
                        for path in data["_problematic_paths"][
                            :5
                        ]:  # Limitar a 5 para não sobrecarregar o log
                            print(f"   - {path}")
                else:
                    keys = list(data.keys())
                    print(
                        f"✓ {name}: dict com {len(keys)} chaves: {keys[:10]}{'...' if len(keys) > 10 else ''}"
                    )
        ...
        if conversion_failures:
            ...
            log.warning(
                f"[HISTORY SAVE] {len(conversion_failures)} stores tiveram falhas de conversão: {conversion_failures}"
            )

        try:
            # Salvar diretamente no banco de dados (sem MCP)
            log.info(
                f"[HISTORY SAVE] Salvando sessão diretamente no banco de dados: {session_name}"
            )
        ...
        return (
            dbc.Alert(f"Erro ao salvar sessão: {str(e)}", color="danger"),
            no_update,
            no_update,
            no_update,
        )
...
@app.callback(
    [Output("delete-session-modal", "is_open"), Output("delete-session-id-store", "data")],
    [
...
def toggle_delete_modal(delete_clicks, cancel_clicks, confirm_clicks, is_open):
    ...
    if (
        ...
        and ctx.triggered_id.get("type") == "delete-btn"
    ):
...
def delete_session(n_clicks, session_data):
    ...
    try:
        ...
        if not success:
            return (
                dbc.Alert("Erro ao excluir sessão: Sessão não encontrada.", color="danger"),
                no_update,
            )
            ...
            try:
                ...
                if all_sessions:
                    ...
                    sorted_sessions = sorted(
                        all_sessions, key=lambda x: x["timestamp"], reverse=True
                    )
                    last_session = (
                        sorted_sessions[0]["session_name"] if sorted_sessions else "Nenhuma"
                    )
            except Exception as e:
                log.error(f"Erro ao buscar última sessão após exclusão: {e}")

        return (
            dbc.Alert(f"Sessão '{session_name}' excluída com sucesso!", color="success"),
            rows,
            total_sessions,
            last_session,
        )
        ...
        return (
            dbc.Alert(f"Erro ao excluir sessão: {str(e)}", color="danger"),
            no_update,
            no_update,
            no_update,
        )
...
def load_session(load_clicks, transformer_inputs=None):
    ...
    if (
        ...
        or triggered_id.get("type") != "load-btn"
    ):
...
def process_loaded_data(temp_store_data):
    ...
    log_detailed(
        log,
        "info",
        "Iniciando processamento de dados carregados",
        module="history",
        ...
        data={
            ...
            "temp_store_data_keys": list(temp_store_data.keys())
            if isinstance(temp_store_data, dict)
            else None,
        },
    )
    ...
    log_detailed(
        log,
        "info",
        "Processando dados carregados da sessão diretamente",
        module="history",
        function="process_loaded_data",
    ...
    try:
                ...
                if update_app_cache(prepared_data):
                    log_detailed(
                        ...
                        data={
                            ...
                            "data_keys": list(prepared_data.keys())
        ...
        log_detailed(
            log,
            "error",
            "Erro ao atualizar cache de dados",
            module="history",
            function="process_loaded_data",
    ...
    stores_status = {
        "transformer-inputs-store": check_store_exists(
        ...
        "dieletric-analysis-store": check_store_exists(
            dieletric_analysis_store, "dieletric-analysis-store"
        ),
        ...
        "temperature-rise-store": check_store_exists(
            temperature_rise_store, "temperature-rise-store"
        ),
    }
    ...
    log_detailed(
        ...
        data={
            ...
            "transformer_inputs_keys": len(transformer_inputs)
            if isinstance(transformer_inputs, dict)
            else 0,
            ...
            "dieletric_analysis_store_keys": len(dieletric_analysis_store)
            if isinstance(dieletric_analysis_store, dict)
            else 0,
            "applied_voltage_store_keys": len(applied_voltage_store)
            if isinstance(applied_voltage_store, dict)
            else 0,
            "induced_voltage_store_keys": len(induced_voltage_store)
            if isinstance(induced_voltage_store, dict)
            else 0,
            "short_circuit_store_keys": len(short_circuit_store)
            if isinstance(short_circuit_store, dict)
            else 0,
            "temperature_rise_store_keys": len(temperature_rise_store)
            if isinstance(temperature_rise_store, dict)
            else 0,
        },
    )
    ...
    log.info("PROCESS_LOADED_DATA End - Returning data for stores")
    print("[DEBUG] PROCESS_LOADED_DATA End - Returning data for stores")
