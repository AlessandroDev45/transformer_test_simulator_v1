...
from components.formatters import (
    formatar_dados_basicos, formatar_perdas_vazio, formatar_perdas_carga,
    formatar_elevacao_temperatura, formatar_curto_circuito, formatar_analise_dieletrica,
    formatar_tensao_aplicada, formatar_tensao_induzida, formatar_impulso
)
...
@app.callback(
    ...
    [State('transformer-inputs-store', 'data'),
     ...
     State('limit-status-store', 'data')], # State para o limite
    prevent_initial_call=True
)
def report_generation_generate_pdf(n_clicks, basic_data, losses_data, impulse_data,
                             dieletric_data, applied_data, induced_data,
                             short_circuit_data, temp_rise_data,
                             limit_status_data): # Argumento para o limite
    ...
    print(f"[report_generation.py] Callback acionado. n_clicks={n_clicks}, limite_atingido={limite_atingido}") # PRINT
    log.debug(f"Generate report callback triggered. Clicks: {n_clicks}, Limit hit: {limite_atingido}")
    ...
    log.info("="*50); log.info("=== INICIANDO GERAÇÃO PDF ==="); log.info("="*50)
    print("[report_generation.py] Iniciando formatação...") # PRINT

    all_data = {
        'basic': basic_data or {}, 'losses': losses_data or {}, 'impulse': impulse_data or {},
        'dieletric': dieletric_data or {}, 'applied': applied_data or {}, 'induced': induced_data or {},
        'short_circuit': short_circuit_data or {}, 'temp_rise': temp_rise_data or {}
    }
        ...
        if data and isinstance(data, dict):
            try:
                ...
                if formatted_content and isinstance(formatted_content, dict):
                    ...
                    log.info(f"Seção '{section_name}' formatada. Chaves: {list(formatted_content.keys())}")
                elif not formatted_content: log.info(f"Seção '{section_name}' formatada, conteúdo vazio.")
                else:
                    log.warning(f"Formatador '{section_name}' tipo inesperado: {type(formatted_content)}")
                ...
                report_data_formatted[f"Erro - {section_name}"] = {"Erro": f"Falha formatar: {e_fmt}"}
        else: log.info(f"Dados seção '{section_name}' não disponíveis/inválidos.")
    ...
    if all_data['losses']:
        try:
            ...
            if vazio: losses_report_section.update(vazio)
        except Exception as e: log.exception(f"Erro formatando vazio: {e}"); format_errors.append(f"Erro P.Vazio:{e}")
        try:
            carga = formatar_perdas_carga(all_data['losses'])
            if carga: losses_report_section.update(carga)
        except Exception as e: log.exception(f"Erro formatando carga: {e}"); format_errors.append(f"Erro P.Carga:{e}")
    if losses_report_section: report_data_formatted["Resultados de Perdas"] = losses_report_section

    format_section("Elevação de Temperatura", all_data['temp_rise'], formatar_elevacao_temperatura)
    format_section("Suportabilidade a Curto-Circuito", all_data['short_circuit'], formatar_curto_circuito)
    ...
    try:
        ...
        log.info("="*50); log.info("=== FIM TENTATIVA GERAÇÃO PDF ==="); log.info("="*50)
