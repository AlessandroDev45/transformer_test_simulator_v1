...
RF_REFERENCE_DATA_LI = [
    (0.5, 500), (1, 350), (2, 220), (4, 140), (8, 90), (16, 60), (32, 40)
]
RF_REFERENCE_DATA_SI = [
    (0.5, 5000), (1, 3500), (2, 2200), (4, 1400), (8, 900), (16, 600), (32, 400)
]
RT_DEFAULT_PER_COLUMN = {
    "lightning": 100, "switching": 2500, "chopped": 120
}
...
def parse_resistor_expression(expression):
    ...
    if cap_match:
        try:
            ...
            logger.info(f"Capacitor de acoplamento {cap_value*1e12:.0f} pF detectado em expressão Rf.")
        except ValueError:
        ...
        for part in series_parts:
            ...
            try:
                ...
                if part_value <= 0 or math.isinf(part_value):
                    logger.warning(f"Parte da expressão resultou em valor inválido: {part} -> {part_value}")
                    return float('inf'), special_components
...
def get_generator_params(config_value):
    ...
    if config:
        ...
        except (ValueError, TypeError): logger.warning(f"Valor de energia inválido para config '{config_value}': {energy_kj}. Usando 0."); energy_kj_float = 0.0
        return config["stages"], config["parallel"], config["max_voltage_kv"], energy_kj_float
    logger.warning(f"Configuração do gerador '{config_value}' não encontrada. Usando padrão 12S-1P.")
...
def get_divider_capacitance(generator_config_value):
    ...
    try:
        ...
        if max_voltage_kv <= 1200: return C_DIVIDER_LOW_VOLTAGE
        else: return C_DIVIDER_HIGH_VOLTAGE
    except Exception as e: logger.error(f"Erro ao obter capacitância do divisor: {e}. Usando padrão."); return C_DIVIDER_HIGH_VOLTAGE

def calculate_effective_gen_params(n_stages, n_parallel):
    """Calcula a capacitância efetiva do gerador."""
    if n_stages <= 0 or n_parallel <=0: logger.error("Número de estágios/paralelo deve ser positivo."); return 0
...
def calculate_transformer_inductance(voltage_kv, power_mva, impedance_percent, freq_hz=60):
    ...
    try:
        voltage_v = float(voltage_kv) * 1000; power_va = float(power_mva) * 1e6; impedance_pu = float(impedance_percent) / 100.0; freq_hz = float(freq_hz)
        if voltage_v <= 0 or power_va <= 0 or freq_hz <= 0: return 0.05
        omega = 2 * np.pi * freq_hz; z_base = (voltage_v**2) / power_va; z_cc_ohm = z_base * impedance_pu; l_cc = z_cc_ohm / omega
        logger.info(f"Indutância do trafo calculada: {l_cc:.4f} H"); return l_cc
    except (ValueError, TypeError, ZeroDivisionError) as e: logger.error(f"Erro ao calcular indutância do trafo: {e}. Usando 0.05H."); return 0.05
...
def calculate_circuit_efficiency(n_stages, n_parallel, c_test_object_pf, c_stray_pf, impulse_type, generator_config_value):
    ...
    try:
        c_gen_effective = calculate_effective_gen_params(n_stages, n_parallel); c_divider_f = get_divider_capacitance(generator_config_value)
        c_test_object_f = c_test_object_pf * 1e-12 if c_test_object_pf is not None else 0; c_stray_f = c_stray_pf * 1e-12 if c_stray_pf is not None else 0
        c_load_extra_f = C_CHOPPING_GAP_PF * 1e-12 if impulse_type == "chopped" else 0 # Convert pF to F
        ...
        logger.debug(f"Eficiência: Total={total_efficiency:.3f}, Circuito={circuit_efficiency:.3f}, Forma={shape_efficiency:.2f}")
        return total_efficiency, circuit_efficiency, shape_efficiency, c_load
    except Exception as e: logger.exception(f"Erro ao calcular eficiência: {e}"); return 0, 0, 0, 0
...
def calculate_energy_requirements(actual_test_voltage_kv, c_load_f):
    ...
    try:
        ...
        logger.debug(f"Energia requerida pela carga @ {actual_test_voltage_kv:.1f} kV: {energy_kj:.2f} kJ"); return energy_kj
    except (ValueError, TypeError) as e: logger.error(f"Erro ao calcular energia requerida: {e}"); return 0.0
    except Exception as e: logger.error(f"Erro não esperado ao calcular energia: {e}"); return 0.0

def _calculate_waveform_parameters(n_stages, n_parallel, rf_per_column, rt_per_column, c_test_object_pf,
                                  c_stray_pf, l_extra_h, transformer_inductance_h, inductor_value_h,
                                  impulse_type, generator_config_value,
                                  inductance_factor=1.0, tail_resistance_factor=1.0):
    ...
    logger.debug(f"Cálculo completo de l_total_initial: l_gen={l_gen:.3e}, l_extra_h={l_extra_h:.3e}, transformer_inductance_h={transformer_inductance_h:.3e}, inductor_value_h={inductor_value_h:.3e}")
    rf_total = rf_total_initial; l_total = l_total_initial * inductance_factor; rt_total = rt_total_initial * tail_resistance_factor
    logger.debug(f"Parâmetros RLC Efetivos (Ajustados): Rf={rf_total:.2f}, Rt={rt_total:.2f}, L={l_total:.3e}, Cg={c_gen_effective:.3e}, Cl={c_load:.3e}, Ceq={c_eq:.3e}")
    ...
    if beta <= alpha + 1e-9:
        adjust_factor = 1.05; add_factor = 1e3; new_beta = alpha * adjust_factor + add_factor
        logger.warning(f"Beta ({beta:.2e}) <= Alpha ({alpha:.2e}). Ajustando Beta para {new_beta:.2e}"); beta = new_beta
    r_eq_damping = rf_total + R_PARASITIC_OHM; omega0 = 1 / math.sqrt(l_total * c_eq) if l_total * c_eq > 1e-18 else 0
    zeta = r_eq_damping / (2 * l_total * omega0) if l_total * omega0 > 1e-9 else float('inf')
    is_oscillatory = zeta < 1.0 if omega0 > 0 else False
    logger.debug(f"Alpha={alpha:.2e}, Beta={beta:.2e}, Zeta={zeta:.3f}, Oscilatório={is_oscillatory}")
...
def rlc_solution(t_sec, v0, r_total, l_total, c_eq):
   ...
   if l_total <= 1e-12 or c_eq <= 1e-15 or r_total <= 1e-9: logger.warning(f"Parâmetros RLC inválidos: R={r_total}, L={l_total}, Ceq={c_eq}"); return np.zeros_like(t_sec)
   try:
       omega0_sq = 1.0 / (l_total * c_eq); alpha_damp = r_total / (2.0 * l_total)
       if alpha_damp <= 0: logger.warning("Amortecimento RLC não positivo."); return np.zeros_like(t_sec)
       delta = alpha_damp**2 - omega0_sq; v_out = np.zeros_like(t_sec); t_valid = t_sec[t_sec >= 0]
       if abs(delta) < 1e-12 * omega0_sq: a = alpha_damp; v_out[t_sec >= 0] = v0 * (1 + a * t_valid) * np.exp(-a * t_valid)
       elif delta < 0: omega_d = np.sqrt(-delta); a = alpha_damp; v_out[t_sec >= 0] = v0 * np.exp(-a * t_valid) * (np.cos(omega_d * t_valid) + (a / omega_d) * np.sin(omega_d * t_valid)) if omega_d > 1e-9 else v0 * (1 + a * t_valid) * np.exp(-a * t_valid)
       else:
           sqrt_delta = np.sqrt(delta); s1 = -alpha_damp + sqrt_delta; s2 = -alpha_damp - sqrt_delta
           if abs(s1 - s2) < 1e-9 * abs(s1 + s2): a = alpha_damp; v_out[t_sec >= 0] = v0 * (1 + a * t_valid) * np.exp(-a * t_valid)
           else:
               if s1 > 1e-9 or s2 > 1e-9: logger.warning(f"Raízes instáveis RLC: s1={s1:.2e}, s2={s2:.2e}."); v_out = np.zeros_like(t_sec) if s2 >= 0 else v0 / (s1 - s2) * (-s2 * np.exp(s1 * t_valid))
               else: v_out[t_sec >= 0] = v0 / (s1 - s2) * (s1 * np.exp(s2 * t_valid) - s2 * np.exp(s1 * t_valid))
       v_out[t_sec < 0] = 0; return v_out
   except (ValueError, OverflowError, ZeroDivisionError) as e: logger.error(f"Erro solução RLC: {e}"); return np.zeros_like(t_sec)

def double_exp_func(t, V0_norm, alpha, beta):
   """ Função de dupla exponencial normalizada para ter pico V0_norm. """
   if alpha <= 1e-12 or beta <= 1e-12 or beta <= alpha + 1e-9 : logger.warning(f"Alpha/Beta inválidos: a={alpha:.2e}, b={beta:.2e}"); return np.zeros_like(t)
   try:
       ...
       if abs(norm_factor) < 1e-12: logger.warning("Fator norm. dupla exp zero."); return np.zeros_like(t)
       k_norm = V0_norm / norm_factor; t_safe = np.maximum(t, 0)
       result = k_norm * (np.exp(-alpha * t_safe) - np.exp(-beta * t_safe)); return result
   except (ValueError, OverflowError, ZeroDivisionError) as e: logger.error(f"Erro dupla exp: {e}"); return np.zeros_like(t)

def calculate_k_factor_transform(v, t, return_params=False):
   """ Aplica a transformação K-factor conforme IEC 61083-2. """
   v_test = np.copy(v); v_base = np.copy(v); v_residual_filtered = np.zeros_like(v); overshoot_rel = 0.0; v_base_success = False; alpha_fit, beta_fit = None, None
   if len(v) < 10 or len(t) < 10 or np.std(v) < 1e-9 or t[-1] <= t[0]: logger.warning("Dados insuficientes/inválidos para k-factor."); return (v_test, v_base, v_residual_filtered, overshoot_rel, (alpha_fit, beta_fit)) if return_params else (v_test, v_base, v_residual_filtered, overshoot_rel)
   peak_value_orig = np.max(v)
   if peak_value_orig < 1e-6: logger.warning("Amplitude baixa para k-factor."); return (v_test, v_base, v_residual_filtered, overshoot_rel, (alpha_fit, beta_fit)) if return_params else (v_test, v_base, v_residual_filtered, overshoot_rel)
   def double_exp_func_fit(t_fit, A_norm, alpha, beta):
       if alpha <= 1e-12 or beta <= 1e-12 or beta <= alpha + 1e-9: return np.full_like(t_fit, 1e12)
       try:
           t_peak = math.log(beta / alpha) / (beta - alpha) if beta > alpha else 0; norm_factor = math.exp(-alpha * t_peak) - math.exp(-beta * t_peak) if t_peak >= 0 else 0
           if abs(norm_factor) < 1e-12: return np.zeros_like(t_fit)
           k_norm = A_norm / norm_factor; t_safe = np.maximum(t_fit, 0); result = k_norm * (np.exp(-alpha * t_safe) - np.exp(-beta * t_safe)); return result
       except (ValueError, OverflowError, ZeroDivisionError) as e: logger.error(f"Erro interno dupla exp ajuste: {e}"); return np.full_like(t_fit, 1e12)
   try:
       t_sec = t * 1e-6; peak_idx = np.argmax(v)
       t_half_idx = np.argmin(np.abs(v[peak_idx:] - 0.5 * peak_value_orig)) + peak_idx if peak_idx < len(v)-1 else len(v)-1; T2_approx_sec = max(1e-7, t_sec[t_half_idx])
       t_30_idx = np.argmin(np.abs(v[:peak_idx+1] - 0.3 * peak_value_orig)) if peak_idx > 0 else 0; t_90_idx = np.argmin(np.abs(v[:peak_idx+1] - 0.9 * peak_value_orig)) if peak_idx > 0 else peak_idx
       T1_approx_sec = (1.67 / 1e6) * max(1e-3, (t[t_90_idx] - t[t_30_idx])) if t_90_idx > t_30_idx else (LIGHTNING_IMPULSE_FRONT_TIME_NOM * 1e-6); T1_approx_sec = max(0.1e-6, T1_approx_sec)
       alpha_est = 0.7 / T2_approx_sec if T2_approx_sec > 1e-9 else 1e7; beta_est = 2.5 / T1_approx_sec if T1_approx_sec > 1e-9 else 1e8
       if beta_est <= alpha_est: beta_est = alpha_est * 5
       A_est = peak_value_orig; lower_bounds = [peak_value_orig * 0.5, alpha_est * 0.1, beta_est * 0.5]; upper_bounds = [peak_value_orig * 1.5, alpha_est * 10, beta_est * 10]
       popt, pcov = curve_fit(double_exp_func_fit, t_sec, v, p0=[A_est, alpha_est, beta_est], bounds=(lower_bounds, upper_bounds), maxfev=5000, method='trf')
       A_fit, alpha_fit, beta_fit = popt; logger.debug(f"Ajuste curva base: A={A_fit:.2f}, alpha={alpha_fit:.2e}, beta={beta_fit:.2e}")
       v_base = double_exp_func_fit(t_sec, A_fit, alpha_fit, beta_fit); v_base_success = True
   except (RuntimeError, ValueError, Exception) as e: logger.warning(f"Ajuste curva base K-Factor falhou: {e}. Usando onda original."); v_base = np.copy(v); v_base_success = False; alpha_fit, beta_fit = None, None
   v_base = np.maximum(v_base, 0); v_residual = v - v_base; v_residual_filtered = np.zeros_like(v_residual)
   if len(t) > 1 and abs(t[1] - t[0]) > 1e-12 and v_base_success:
       try:
           dt_fft = (t[1] - t[0]) * 1e-6; frequencies = fftfreq(len(t), dt_fft); v_residual_fft = fft(v_residual)
           k_factor_filter = np.ones_like(frequencies, dtype=complex); f_MHz = np.abs(frequencies) * 1e-6; f_MHz_sq = f_MHz**2
           non_zero_freq_mask = f_MHz_sq > 1e-12; denominator = 1.0 + 2.2 * f_MHz_sq[non_zero_freq_mask]
           filter_values = np.divide(1.0, denominator, where=denominator > 1e-12, out=np.zeros_like(denominator)); k_factor_filter[non_zero_freq_mask] = filter_values
           v_residual_filtered_fft = v_residual_fft * k_factor_filter; v_residual_filtered = np.real(ifft(v_residual_filtered_fft))
       except Exception as e_fft: logger.error(f"Erro FFT/IFFT K-factor: {e_fft}."); v_residual_filtered = np.zeros_like(v_residual)
   elif not v_base_success: logger.info("K-factor não aplicado (ajuste base falhou)."); v_residual_filtered = np.zeros_like(v)
   else: logger.warning("Vetor tempo inválido para FFT K-factor."); v_residual_filtered = np.zeros_like(v_residual)
   v_test = v_base + v_residual_filtered; v_test = np.maximum(v_test, 0)
   peak_value_base = np.max(v_base) if len(v_base) > 0 else 0; overshoot_rel = 0.0
   if peak_value_orig > 1e-9 and peak_value_base > 1e-9: overshoot_rel = max(0.0, (peak_value_orig - peak_value_base) / peak_value_orig) * 100
   if return_params: return v_test, v_base, v_residual_filtered, overshoot_rel, (alpha_fit, beta_fit)
   return v_test, v_base, v_residual_filtered, overshoot_rel

def simulate_hybrid_impulse(t_sec, v0_charge, rf, rt, l_total, c_gen, c_load, impulse_type, gap_distance_cm=None):
   """ Simula o circuito usando a abordagem híbrida RLC + K-Factor + Dupla Exp. """
   collapse_time = 0.1e-6; logger.info(f"Simulando Híbrido: Tipo={impulse_type}, V0_carga={v0_charge/1000:.1f}kV")
   c_eq = (c_gen * c_load) / (c_gen + c_load) if (c_gen + c_load) > 1e-15 else 0; r_rlc = rf + R_PARASITIC_OHM
   logger.debug(f"Simulando RLC: R={r_rlc:.1f}, L={l_total:.2e}, Ceq={c_eq:.2e}"); v_rlc = rlc_solution(t_sec, v0_charge, r_rlc, l_total, c_eq)
   v_rlc_kv = v_rlc / 1000; t_us = t_sec * 1e6
   ...
   if alpha_fit is None or beta_fit is None:
       logger.warning("Ajuste K-factor falhou. Usando estimativas."); alpha = 1 / (rt * (c_gen + c_load)) if rt > 1e-9 and (c_gen + c_load) > 1e-15 else 0; beta = 1 / (rf * c_eq) if rf > 1e-9 and c_eq > 1e-15 else 0
       if beta <= alpha + 1e-9: adjust_factor = 1.05; add_factor = 1e3 if impulse_type in ["lightning", "chopped"] else 1e2; beta = alpha * adjust_factor + add_factor
   else: alpha = alpha_fit; beta = beta_fit; logger.info(f"Parâmetros K-factor: alpha={alpha:.2e}, beta={beta:.2e}")
   ...
   if impulse_type == "chopped" and gap_distance_cm is not None and gap_distance_cm > 0:
       ...
       if len(times_above_threshold) > 0:
           ...
           if chop_idx < len(t_sec):
               chop_time_sec = t_sec[chop_idx]; logger.info(f"Corte detectado: t={chop_time_sec*1e6:.2f} µs, V={v_final[chop_idx]/1000:.1f} kV")
               collapse_idx = np.where(t_sec >= chop_time_sec)[0]
               if len(collapse_idx) > 0:
                   chop_voltage_final = v_final[chop_idx]; dt_after_chop = t_sec[collapse_idx] - chop_time_sec
                   mask_collapse = dt_after_chop <= collapse_time; mask_osc = dt_after_chop > collapse_time; idx_collapse = collapse_idx[mask_collapse]; idx_osc = collapse_idx[mask_osc]
                   if len(idx_collapse) > 0: v_final[idx_collapse] = chop_voltage_final * (1 - (t_sec[idx_collapse] - chop_time_sec) / collapse_time)
                   if len(idx_osc) > 0:
                       freq_osc = 5e6; damp_factor = 1.5; undershoot_ratio = 0.25; time_after_collapse = t_sec[idx_osc] - chop_time_sec - collapse_time
                       amp = -chop_voltage_final * undershoot_ratio * np.exp(-damp_factor * time_after_collapse * 1e6); osc = amp * np.cos(2 * np.pi * freq_osc * time_after_collapse)
                       v_final[idx_osc] = osc; v_final[idx_osc] = np.clip(v_final[idx_osc], -0.3 * abs(chop_voltage_final), float('inf')); v_final[idx_osc[time_after_collapse > 3/freq_osc]] = 0
       else: logger.warning(f"Tensão não atingiu breakdown ({breakdown_voltage/1000:.1f} kV) para gap={gap_distance_cm} cm")
...
def analyze_lightning_impulse(t_us, v_kv):
   ...
   results = {
       "peak_value": None, "test_value": None, "base_value": None, "peak_time": None,
       "t_30": None, "t_90": None, "t_0_virtual": None, "t_front": None, "t_tail": None,
       "overshoot": 0.0, "conforme_frente": False, "conforme_cauda": False,
       "conforme_overshoot": True, "conforme_pico": False, "error": None
   }

   try:
       if v_kv is None or t_us is None or len(v_kv) < 10 or len(t_us) != len(v_kv) or np.std(v_kv) < 1e-9:
       ...
       if len(v_before_kv) > 1 and test_peak_value_kv > 1e-6: # Evita divisão por zero se pico for zero
           v_max_before = np.max(v_before_kv); v_min_before = np.min(v_before_kv)
           v_30_target = 0.3 * test_peak_value_kv; v_90_target = 0.9 * test_peak_value_kv

           if v_max_before >= v_90_target and v_min_before <= v_30_target:
               try:
                   t_30_us = np.interp(v_30_target, v_before_kv, t_before_us, left=np.nan, right=np.nan)
                   t_90_us = np.interp(v_90_target, v_before_kv, t_before_us, left=np.nan, right=np.nan)
                   results["t_30"] = t_30_us; results["t_90"] = t_90_us

                   if not np.isnan(t_30_us) and not np.isnan(t_90_us) and t_90_us > t_30_us:
                       delta_t_front = t_90_us - t_30_us; delta_v_front = v_90_target - v_30_target
                       if delta_t_front > 1e-9:
                            results["t_front"] = 1.67 * delta_t_front
                            slope = delta_v_front / delta_t_front
                            if abs(slope) > 1e-9: t_0_virtual_us = t_30_us - (v_30_target / slope)
                            else: t_0_virtual_us = t_30_us
                       else: logger.warning("Intervalo t90-t30 muito pequeno para T1."); results["t_front"] = None
                   else: logger.warning("Não foi possível interpolar t30 ou t90 na frente."); results["t_front"] = None
               except Exception as e_interp_front: logger.error(f"Erro interpolação frente LI: {e_interp_front}"); results["t_front"] = None
           else: logger.warning("Níveis 30%/90% não encontrados antes do pico.")
       results["t_0_virtual"] = t_0_virtual_us

       # 4. Análise da Cauda (T2)
       mask_after_peak = t_us >= test_peak_time_us
       t_after_us = t_us[mask_after_peak]; v_after_kv = v_test_kv[mask_after_peak]
       v_50_target = 0.5 * test_peak_value_kv

       if len(v_after_kv) > 1 and np.min(v_after_kv) <= v_50_target + 1e-6: # Adiciona tolerância pequena
           try:
               indices_below_50 = np.where(v_after_kv <= v_50_target)[0]
               if len(indices_below_50) > 0:
                   idx2 = indices_below_50[0]; idx1 = idx2 - 1 if idx2 > 0 else 0
                   t1_tail, v1_tail = t_after_us[idx1], v_after_kv[idx1]; t2_tail, v2_tail = t_after_us[idx2], v_after_kv[idx2]
                   if abs(v2_tail - v1_tail) > 1e-9:
                       t_50_us = t1_tail + (v_50_target - v1_tail) * (t2_tail - t1_tail) / (v2_tail - v1_tail)
                       if not np.isnan(t_50_us): results["t_tail"] = t_50_us - t_0_virtual_us
                   elif v1_tail >= v_50_target: results["t_tail"] = t1_tail - t_0_virtual_us
               else: logger.warning("Não encontrou ponto < 50% na cauda (lógica np.where).")
           except Exception as e_interp_tail: logger.error(f"Erro interpolação cauda LI: {e_interp_tail}"); results["t_tail"] = None
       else: logger.warning("Dados insuficientes ou tensão não caiu abaixo de 50% na cauda.")
       ...
       results["conforme_pico"] = results["test_value"] is not None # Conformidade do pico é apenas se ele foi calculado

       logger.info(f"Análise LI: Vt={results.get('test_value', 'N/A'):.2f} kV, T1={results.get('t_front', 'N/A'):.2f} µs, T2={results.get('t_tail', 'N/A'):.1f} µs, β={results.get('overshoot', 'N/A'):.1f}%")
       logger.info(f"Conformidade LI: Frente={results['conforme_frente']}, Cauda={results['conforme_cauda']}, Overshoot={results['conforme_overshoot']}")
       return results
   except Exception as e:
...
def analyze_switching_impulse(t_us, v_kv):
   ...
   results = {
       "peak_value": None, "peak_time": None, "tp": None, "td": None, "t_half": None,
       "t2_calculated": None, "tz": None, "tz_calculated": None, "T_AB": None,
       "t_30": None, "t_90": None, "conforme_pico": False, "conforme_meia_onda": False,
       "conforme_td": False, "conforme_tz": False, "error": None
   }
   try:
       if v_kv is None or t_us is None or len(v_kv) < 10 or len(t_us) != len(v_kv) or np.std(v_kv) < 1e-9:
       ...
       if len(v_before_kv) > 1 and peak_value_kv > 1e-6:
           ...
           if v_max_before >= v_90_target and v_min_before <= v_30_target:
               try:
                   results["t_30"] = np.interp(v_30_target, v_before_kv, t_before_us, left=np.nan, right=np.nan)
                   results["t_90"] = np.interp(v_90_target, v_before_kv, t_before_us, left=np.nan, right=np.nan)
                   if not np.isnan(results["t_30"]) and not np.isnan(results["t_90"]) and results["t_90"] > results["t_30"]:
       ...
       if len(v_after_kv) > 1 and np.min(v_after_kv) <= v_50_target + 1e-6:
           try:
               ...
               if len(indices_below_50) > 0:
                   ...
                   if abs(v2_tail - v1_tail) > 1e-9:
                       t_half_us = t1_tail + (v_50_target - v1_tail) * (t2_tail - t1_tail) / (v2_tail - v1_tail)
       ...
       if len(v_after_kv) > 0 and np.min(v_after_kv) <= 1e-6: # Verifica se cruza zero (com tolerância)
           try:
       ...
       logger.info(f"Análise SI: Vp={results.get('peak_value', 'N/A'):.1f} kV, Tp={results.get('tp', 'N/A'):.1f} µs, T2={results.get('t2_calculated', 'N/A'):.1f} µs, Td={results.get('td', 'N/A'):.1f} µs, Tz={results.get('tz_calculated', 'N/A'):.1f} µs")
       logger.info(f"Conformidade SI: Pico={results['conforme_pico']}, Cauda={results['conforme_meia_onda']}, Td={results['conforme_td']}, Tz={results['conforme_tz']}")
       return results
   except Exception as e:
...
def analyze_chopped_impulse(t_us, v_kv, chop_time_actual_us):
   ...
   results = {
       "peak_value_full_wave": None, "peak_time_full_wave": None, "t_front": None,
       "t_0_virtual": None, "chop_time": None, "chop_voltage": None, "chop_voltage_orig": None,
       "t_30": None, "t_90": None, "chop_collapse_time": None, "undershoot": 0.0,
       "conforme_frente": False, "conforme_corte": False, "conforme_undershoot": True,
       "conforme_pico_corte": False, "error": None
   }
   ...
   try:
       ...
       if len(t_before_chop_us) < 5:
           ...
           logger.error(results["error"])
           return results
       v_test_before_chop, v_base_before_chop, _, overshoot_before = calculate_k_factor_transform(v_before_chop_kv, t_before_chop_us)
       if v_test_before_chop is None:
           results["error"] = "Falha K-factor pré-corte."
           logger.error(results["error"])
           return results
       chop_voltage_kv = np.interp(chop_time_us, t_before_chop_us, v_test_before_chop, left=v_test_before_chop[0], right=v_test_before_chop[-1])
       ...
       if len(v_front_kv) > 1 and peak_value_kv_before > 1e-6:
           ...
           if v_max_front >= v_90_target and v_min_front <= v_30_target:
               try:
                   results["t_30"] = np.interp(v_30_target, v_front_kv, t_front_us, left=np.nan, right=np.nan)
                   results["t_90"] = np.interp(v_90_target, v_front_kv, t_front_us, left=np.nan, right=np.nan)
                   if not np.isnan(results["t_30"]) and not np.isnan(results["t_90"]) and results["t_90"] > results["t_30"]:
       ...
       if len(v_after_chop_kv) > 0:
           ...
           if min_after_chop < 0 and abs(results["chop_voltage"]) > 1e-9:
               results["undershoot"] = max(0.0, abs(min_after_chop) / abs(results["chop_voltage"])) * 100
       if results["t_front"] is not None:
       ...
       if results["chop_time"] is not None:
           results["conforme_corte"] = CHOPPED_IMPULSE_CHOP_TIME_MIN <= results["chop_time"] <= CHOPPED_IMPULSE_CHOP_TIME_MAX
       results["conforme_undershoot"] = results["undershoot"] <= CHOPPED_UNDERSHOOT_MAX
       logger.info(f"Análise LIC: Vc={results.get('chop_voltage', 'N/A'):.1f} kV, T1_subj={results.get('t_front', 'N/A'):.2f} µs, Tc={results.get('chop_time', 'N/A'):.2f} µs, Undershoot={results.get('undershoot', 'N/A'):.1f}%")
       logger.info(f"Conformidade LIC: Frente={results['conforme_frente']}, Corte={results['conforme_corte']}, Undershoot={results['conforme_undershoot']}")
       return results
   except Exception as e:
...
def format_parameter_value(param_value, precision=2, unit=""):
   ...
   if param_value is None or (isinstance(param_value, float) and (np.isnan(param_value) or np.isinf(param_value))):
       return "N/A"
   try:
       if isinstance(param_value, bool):
           return "Sim" if param_value else "Não"
       val = float(param_value) # Tenta converter para float
       if abs(val) >= 1e6: return f"{val/1e6:.{precision}f}  M{unit}"
       elif abs(val) >= 1e3: return f"{val/1e3:.{precision}f}  k{unit}"
       elif abs(val) >= 1: return f"{val:.{precision}f}  {unit}"
       elif abs(val) >= 1e-3: return f"{val*1e3:.{precision}f}  m{unit}"
       elif abs(val) >= 1e-6: return f"{val*1e6:.{precision}f}  µ{unit}"
       elif abs(val) >= 1e-9: return f"{val*1e9:.{precision}f}  n{unit}"
       elif abs(val) >= 1e-12: return f"{val*1e12:.{precision}f}  p{unit}"
       elif abs(val) < 1e-15: return f"0.00  {unit}" # Evita notação científica para zero
       else: return f"{val:.{precision}e}  {unit}"
...
def create_circuit_parameters_display(params_data):
   ...
   if not params_data or not isinstance(params_data, dict):
       return dbc.Alert("Dados de parâmetros do circuito indisponíveis.", color="warning", style={"fontSize": "0.7rem"})

   # Verificação explícita de erro
   if "error" in params_data:
       return dbc.Alert(f"Erro nos parâmetros do circuito: {params_data['error']}", color="danger", style={"fontSize": "0.7rem"})

   header = html.Thead(html.Tr([html.Th("Parâmetro"), html.Th("Valor")]))
   rows = []

   def add_row(label, value, unit="", precision=2):
       rows.append(html.Tr([html.Td(label, style={"fontWeight": "bold"}), html.Td(format_parameter_value(value, precision, unit))]))
   ...
   try:
       ...
       return dbc.Alert(f"Erro ao processar parâmetros do circuito: {str(e)}", color="danger", style={"fontSize": "0.7rem"})

   return dbc.Table([header, html.Tbody(rows)], bordered=True, hover=True, striped=True, size="sm", className="mb-0")
...
def create_energy_details_table(energy_required_kj, max_energy_kj, generator_config_label):
   ...
   if energy_required_kj is None or max_energy_kj is None:
       return dbc.Alert("Dados de energia indisponíveis.", color="warning", style={"fontSize": "0.7rem"})
   try:
       rows = [
           html.Tr([html.Td("Energia Requerida (Carga)"), html.Td(format_parameter_value(energy_required_kj, 2, "kJ"))]),
           html.Tr([html.Td("Energia Disponível (Gerador)"), html.Td(format_parameter_value(max_energy_kj, 1, "kJ"))]),
           html.Tr([html.Td("Configuração Gerador"), html.Td(generator_config_label or "N/A")])
       ]
       energy_margin = max_energy_kj / energy_required_kj if energy_required_kj > 1e-9 else float('inf')
       margin_class = "text-success" if energy_margin >= 1.2 else "text-warning" if energy_margin >= 1.0 else "text-danger"
       rows.append(html.Tr([html.Td("Margem de Energia"), html.Td(f"{energy_margin:.2f}x" if energy_margin != float('inf') else "∞", className=margin_class)]))
       return dbc.Table([html.Tbody(rows)], bordered=True, hover=True, striped=True, size="sm", className="mb-0")
   except Exception as e:
       return dbc.Alert(f"Erro ao criar tabela de energia: {str(e)}", color="danger", style={"fontSize": "0.7rem"})

# Moved definition before usage in run_simulation
def create_waveform_analysis_table(analysis_results, impulse_type, v_test_kv_input=None, gap_distance_cm=None):
    """ Cria uma tabela com os resultados da análise da forma de onda e status de conformidade. """
    if not analysis_results or not isinstance(analysis_results, dict):
        return dbc.Alert("Resultados da análise indisponíveis.", color="warning", style={"fontSize": "0.7rem"})
    if "error" in analysis_results and analysis_results["error"]:
        return dbc.Alert(f"Erro na análise: {analysis_results['error']}", color="danger", style={"fontSize": "0.7rem"})

    header = html.Thead(html.Tr([html.Th("Parâmetro"), html.Th("Valor Simulado"), html.Th("Requisito Norma"), html.Th("Status")]))
    rows = []
    ...
    def check_compliance(value, req_min, req_max, is_max_limit=False):
        ...
        status = False
        if is_max_limit: status = val <= high
        elif low != -np.inf and high == np.inf: status = val >= low
        elif low != -np.inf and high != np.inf: status = low <= val <= high
        else: return "", "" # Sem requisito específico
        if not status: overall_compliance = False
    ...
    def add_analysis_row(label, param_key, unit, req_min, req_max, is_max_limit=False, precision=2):
        ...
        req_str = ""
        if is_max_limit and req_max is not None: req_str = f"≤ {req_max:.{precision}f} {unit}"
        elif req_min is not None and req_max is None: req_str = f"≥ {req_min:.{precision}f} {unit}"
        elif req_min is not None and req_max is not None: req_str = f"{req_min:.{precision}f} - {req_max:.{precision}f} {unit}"
        else: req_str = "-"
        compliance_class, status_symbol = check_compliance(value, req_min, req_max, is_max_limit)
        rows.append(html.Tr([html.Td(label), html.Td(val_str), html.Td(req_str), html.Td(status_symbol)], className=compliance_class))
    ...
    if impulse_type == "lightning":
        ...
        req = { 'T1_min': LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1-LIGHTNING_FRONT_TOLERANCE), 'T1_max': LIGHTNING_IMPULSE_FRONT_TIME_NOM * (1+LIGHTNING_FRONT_TOLERANCE), 'T2_min': LIGHTNING_IMPULSE_TAIL_TIME_NOM * (1-LIGHTNING_TAIL_TOLERANCE), 'T2_max': LIGHTNING_IMPULSE_TAIL_TIME_NOM * (1+LIGHTNING_TAIL_TOLERANCE), 'Beta_max': LIGHTNING_OVERSHOOT_MAX, 'Vt_min': vt_min, 'Vt_max': vt_max}
        add_analysis_row("Vt (Pico Ensaio)", 'test_value', "kV", req['Vt_min'], req['Vt_max'], precision=1)
        ...
        add_analysis_row("β (Overshoot)", 'overshoot', "%", None, req['Beta_max'], is_max_limit=True, precision=1)
        ...
        req = {'Tp_min': SWITCHING_IMPULSE_PEAK_TIME_NOM*(1-SWITCHING_PEAK_TIME_TOLERANCE),
                'Tp_max': SWITCHING_IMPULSE_PEAK_TIME_NOM*(1+SWITCHING_PEAK_TIME_TOLERANCE),
                'T2_min': SWITCHING_IMPULSE_TAIL_TIME_NOM*(1-SWITCHING_TAIL_TOLERANCE),
                'T2_max': SWITCHING_IMPULSE_TAIL_TIME_NOM*(1+SWITCHING_TAIL_TOLERANCE),
                ...
                'Vt_min': vt_min,
                'Vt_max': vt_max}
        add_analysis_row("Vp (Pico Ensaio)", 'peak_value', "kV", req['Vt_min'], req['Vt_max'], precision=1)
        add_analysis_row("Tp (Pico)", 'tp', "µs", req['Tp_min'], req['Tp_max'], precision=0)
        add_analysis_row("T₂ (Meio Valor)", 't2_calculated', "µs", req['T2_min'], req['T2_max'], precision=0)
        ...
        req = {'T1_min': LIGHTNING_IMPULSE_FRONT_TIME_NOM*(1-CHOPPED_FRONT_TOLERANCE),
                'T1_max': LIGHTNING_IMPULSE_FRONT_TIME_NOM*(1+CHOPPED_FRONT_TOLERANCE),
                ...
                'Vc_min': vc_min,
                'Vc_max': vc_max}
        add_analysis_row("Vc (Tensão Corte)", 'chop_voltage', "kV", req['Vc_min'], req['Vc_max'], precision=1)
        ...
        add_analysis_row("Undershoot", 'undershoot', "%", None, req['Undershoot_max'], is_max_limit=True, precision=1)
    else:
        return dbc.Alert(f"Tipo de impulso desconhecido para análise: {impulse_type}", color="warning")
...
@app.callback(
    ...
    [Input("simulate-button", "n_clicks"),
     Input("auto-simulate-interval", "n_intervals")],
...
def toggle_simulation(n_clicks, n_intervals, status_data):
    ...
    running = status_data.get("running", False) if status_data else False # Default to False if None
...
def update_dynamic_controls(impulse_type):
    ...
    inductor_style = {"display": "block"} if impulse_type in ["lightning", "chopped"] else {"display": "none"}
...
@app.callback(
    Output("front-resistor-expression", "value"),
    [Input("rf-up", "n_clicks"),
     Input("rf-down", "n_clicks")],
...
@app.callback(
    Output("tail-resistor-expression", "value"),
    [Input("rt-up", "n_clicks"),
     Input("rt-down", "n_clicks")],
...
def calculate_transformer_inductance_callback(n_clicks, voltage, power, impedance, frequency):
    ...
    try:
        ...
        result_data = {"inductance": l_trafo, "params": {"voltage": voltage, "power": power, "impedance": impedance, "frequency": frequency}}
...
def use_transformer_data(n_clicks, transformer_data):
    ...
    try:
        voltage_kv = transformer_data.get('tensao_at')
        power_mva = transformer_data.get('potencia_mva')
        impedance_percent = transformer_data.get('impedancia')
        freq_hz = transformer_data.get('frequencia', 60)

        # Calcular indutância do transformador
        l_trafo = calculate_transformer_inductance(voltage_kv, power_mva, impedance_percent, freq_hz)

        logger.info(f"Dados do transformador utilizados: {voltage_kv} kV, {power_mva} MVA, {impedance_percent}%, {freq_hz} Hz")
...
def update_impulse_simulation(n_clicks, r_front, r_tail, capacitance, inductance, stray_cap, shunt_res, sim_time, time_step, impulse_type, current_store_data):
    if n_clicks is None:
        raise dash.exceptions.PreventUpdate
    ...
    try:
        # Validar entradas
        if None in [r_front, r_tail, capacitance, inductance, stray_cap, shunt_res, sim_time, time_step]:
            return dash.no_update, html.Div("Preencha todos os campos para simular", style={"color": "red"})
        ...
        t, v = simulate_impulse_circuit(r_front, r_tail, capacitance, inductance, stray_cap, shunt_res, sim_time, time_step)
        ...
        if isinstance(data_for_store, dict):
            ...
            if isinstance(inputs, dict): print(f"    inputs chaves: {list(inputs.keys())}")
            print(f"  results (tipo): {type(results).__name__}")
            if isinstance(results, dict): print(f"    results chaves: {list(results.keys())}")
        else:
        ...
        print(f"--- Fim [IMPULSE STORE SAVE DEBUG] ---")
...
def simulate_impulse_circuit(r_front, r_tail, capacitance, inductance, stray_cap, shunt_res, sim_time, time_step):
...
def create_impulse_graph(t, v, peak_voltage, rise_time, tail_time):
    ...
    fig.add_shape(type="line", x0=0, y0=0.3, x1=max(t_us), y1=0.3,
                 line=dict(color="gray", width=1, dash="dash"))
    fig.add_shape(type="line", x0=0, y0=0.9, x1=max(t_us), y1=0.9,
                 line=dict(color="gray", width=1, dash="dash"))
    fig.add_shape(type="line", x0=0, y0=0.5, x1=max(t_us), y1=0.5,
                 line=dict(color="gray", width=1, dash="dash"))
...
def register_impulse_callbacks(app):
    ...
    """
    log.info(f"Callbacks do módulo de impulso já registrados via decoradores @dash.callback para app {app.title}.")
