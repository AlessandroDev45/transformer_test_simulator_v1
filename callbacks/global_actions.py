...
def global_actions_clear_all_data(n_clicks, pathname):
    ...
    if n_clicks is None or n_clicks == 0:
        ...
        log.info(f"CLEAR_ALL_DATA Aborted - n_clicks is None or 0")
        raise PreventUpdate
    ...
    try:
        if hasattr(app, 'mcp') and app.mcp is not None:
            ...
            # Log de finalização
            log.info(f"CLEAR_ALL_DATA End - Returning cleared data via MCP")
            print(f"[DEBUG] CLEAR_ALL_DATA End - Returning cleared data via MCP")
            ...
            # Log de finalização
            log.info(f"CLEAR_ALL_DATA End - Returning cleared data via fallback method")
            print(f"[DEBUG] CLEAR_ALL_DATA End - Returning cleared data via fallback method")
