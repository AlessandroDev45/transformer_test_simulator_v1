...
import datetime
import logging
from dash import html, Input, Output, State, ALL, callback, ctx, no_update
...
@app.callback(
    ...
    [Input({'type': 'conexao', 'index': ALL}, 'value'),
     Input('url', 'pathname')],  # Adicionado para garantir que seja executado quando a página carrega
...
def dieletric_analysis_toggle_neutro_visibility(conexoes, pathname=None):
    if not conexoes: return no_update
...
def dieletric_analysis_update_ia_options(um_values, tipo_isolamento):
    num_outputs = len(ctx.outputs_list[0]) if ctx.outputs_list and ctx.outputs_list[0] else len(um_values) if um_values else 0
    if num_outputs == 0: return no_update
    verificador = get_verificador_instance()
    if verificador is None: return [[] for _ in range(num_outputs)]
        ...
        if um:
            try:
                ...
                for valor in valores_combinados_floats:
                    ...
                    value_str = str(int(valor)) if np.isclose(valor, round(valor)) else str(valor) # Garante string
                    opcoes_ia.append({'label': label, 'value': value_str})
            except Exception as e: log.exception(f"Erro opções IA (Um={um}, Isol={tipo_isolamento}): {e}")
        options_list.append(opcoes_ia)
    while len(options_list) < num_outputs: options_list.append([])
...
def dieletric_analysis_update_ia_neutro_options(neutro_um_values):
    num_outputs = len(ctx.outputs_list[0]) if ctx.outputs_list and ctx.outputs_list[0] else len(neutro_um_values) if neutro_um_values else 0
    if num_outputs == 0: return no_update
    verificador = get_verificador_instance()
    if verificador is None: return [[] for _ in range(num_outputs)]
    options_list = []
...
def dieletric_analysis_update_im_options(um_values, ia_values, tipo_isolamento):
    ...
    if len(ia_values) < len(um_values): ia_values = list(ia_values) + [None] * (len(um_values) - len(ia_values))
    for um, ia in zip(um_values, ia_values):
        opcoes_im = []
        if um:
            try:
                um_float = safe_float_convert(um)
                if um_float is None: options_list.append([]); continue
...
def dieletric_analysis_update_tensao_curta_options(um_values, conexoes, neutro_um_values, tipo_isolamento):
    num_outputs = len(ctx.outputs_list[0]) if ctx.outputs_list and ctx.outputs_list[0] else len(um_values) if um_values else 0
    if num_outputs == 0: return no_update
    verificador = get_verificador_instance()
    if verificador is None: return [[] for _ in range(num_outputs)]
    tipo_isolamento = tipo_isolamento or 'uniforme'
    options_list = []
    if len(conexoes) < len(um_values): conexoes = list(conexoes) + [None] * (len(um_values) - len(conexoes))
    if len(neutro_um_values) < len(um_values): neutro_um_values = list(neutro_um_values) + [None] * (len(um_values) - len(neutro_um_values))
    for i, um in enumerate(um_values):
        opcoes_tc = []
        if um:
            conexao = conexoes[i]; neutro_um = neutro_um_values[i]
            try:
            ...
            except Exception as e: log.exception(f"Erro opções T.Curta (Um={um}, Isol={tipo_isolamento}): {e}")
...
def dieletric_analysis_update_espacamentos_display(um_values, ia_values, im_values, transformer_data):
    ...
    try:
        ...
        num_outputs_nbr = len(ctx.outputs_list[0]) if ctx.outputs_list and ctx.outputs_list[0] else 0
        num_outputs_ieee = len(ctx.outputs_list[1]) if ctx.outputs_list and ctx.outputs_list[1] else 0
        ...
        if verificador is None:
            ...
            no_data_msg = html.P("Erro Verif.", style={"fontSize": "0.7rem", "color": default_error_color})
            return ([no_data_msg] * num_outputs, [no_data_msg] * num_outputs)

        # Determina o tipo de transformador
        tipo_transformador_geral = transformer_data.get('tipo_transformador', 'Trifásico') if transformer_data and isinstance(transformer_data, dict) else 'Trifásico'
        transformer_type_ieee = "power" if tipo_transformador_geral == 'Trifásico' else "distribution"
        log.info(f"Tipo de transformador para espaçamentos: {tipo_transformador_geral} (IEEE: {transformer_type_ieee})")
        ...
        for i, (um, ia, im) in enumerate(zip(um_values, ia_values, im_values)):
            ...
            if um:
                try:
                    log.info(f"Calculando espaçamentos para enrolamento {i}: Um={um}, IA={ia}, IM={im}")
                    ...
                    if nbr_data:
                        ...
                        nbr_display = html.P("NBR: N/A", style={"fontSize": "0.7rem", "color": default_muted_color})

                    # Processa dados IEEE
                    ieee_data = resultados.get("IEEE")
                    if ieee_data:
                        log.debug(f"Dados IEEE encontrados: {ieee_data}")
                        clearance_key = 'internal_clearance_phase_ground' if transformer_type_ieee == 'power' else 'clearance'
                        strike_key = 'internal_strike_phase_ground' if transformer_type_ieee == 'power' else None
                        ...
                        ieee_display = html.Div([
                            ...
                            html.P(" | ".join(ieee_parts), style={"fontSize": "0.7rem", "marginBottom": "0.1rem"})
                        ])
                    else:
                        log.warning(f"Nenhum dado IEEE encontrado para Um={um}")
                        ieee_display = html.P("IEEE: N/A", style={"fontSize": "0.7rem", "color": default_muted_color})

                except Exception as e:
                    log.exception(f"Erro ao calcular espaçamentos para Um={um}, IA={ia}, IM={im}: {e}")
                    error_msg = html.P("Erro Calc.", style={"fontSize": "0.7rem", "color": default_error_color})
                    nbr_display = error_msg
                    ieee_display = error_msg
            else:
                log.debug(f"Nenhum valor Um para enrolamento {i}, usando placeholders")
                nbr_display = html.P("NBR: -", style={"fontSize": "0.7rem", "color": default_muted_color})
                ieee_display = html.P("IEEE: -", style={"fontSize": "0.7rem", "color": default_muted_color})
        ...
        default_placeholder = html.P("-", style={"fontSize": "0.7rem", "color": default_muted_color})
        ...
        log.info(f"Espaçamentos calculados com sucesso: {len(esp_nbr_list)} NBR, {len(esp_ieee_list)} IEEE")
...
def populate_dielectric_fields(pathname_input, transformer_data, pathname_state):
    ...
    current_pathname = pathname_state if triggered_id == 'transformer-inputs-store' else pathname_input
    log.info(f"[POPULATE START @ {start_time:.4f}] Trigger: {triggered_id}, Pathname: {current_pathname}")

    if not current_pathname: raise PreventUpdate
    clean_path = normalize_pathname(current_pathname)
    if clean_path != ROUTE_DIELECTRIC_ANALYSIS:
        log.debug(f"[POPULATE @ {time.perf_counter():.4f}] Não na página '{ROUTE_DIELECTRIC_ANALYSIS}'. Prevenindo update.")
        raise PreventUpdate
    ...
    log.info(f"   -> [VERIFICANDO STORE @ {time.perf_counter():.4f}]: Keys={list(transformer_data.keys())}")
    ...
    valores_retorno = [um_at, um_bt, um_terciario, conexao_at, conexao_bt, conexao_terciario, nbi_at, nbi_bt, nbi_terciario, sil_at, sil_bt, sil_terciario, tipo_isolamento]
    log.info(f"[POPULATE END @ {end_time:.4f}] Duração: {(end_time - start_time)*1000:.2f} ms. Retornando: {valores_retorno}")
...
def populate_dielectric_fields_auto(pathname, transformer_data):
    ...
    log.info(f"[AUTO FILL] Preenchendo campos automaticamente na página '{clean_path}'.")
    log.info(f"[AUTO FILL] Dados do transformador: {transformer_data}")

    # Verifica se temos dados válidos
    if not transformer_data or not isinstance(transformer_data, dict):
        log.warning("[AUTO FILL] Dados do transformador inválidos ou vazios.")
        return [None] * 29  # 29 outputs (adicionados 3 para tensão induzida)

    # --- Lógica de Processamento (Copiada/Adaptada de populate_dielectric_fields) ---
    um_at, um_bt, um_terciario = None, None, None
    conexao_at, conexao_bt, conexao_terciario = None, None, None
    nbi_at, nbi_bt, nbi_terciario = None, None, None
    sil_at, sil_bt, sil_terciario = None, None, None
    neutro_um_at, neutro_um_bt, neutro_um_terciario = None, None, None
    nbi_neutro_at, nbi_neutro_bt, nbi_neutro_terciario = None, None, None
    tensao_induzida_at, tensao_induzida_bt, tensao_induzida_terciario = None, None, None
    tipo_isolamento = transformer_data.get('tipo_isolamento', 'uniforme')

    # Obter tipo de transformador e tensões aplicadas
    ...
    # Obter valores de tensão induzida e formatar corretamente
    ...
    verificador = get_verificador_instance() # Necessário para fallback de Um

    # (Lógica para obter Um com fallback - igual ao populate_dielectric_fields)
    um_at = safe_str_output(transformer_data.get('classe_tensao_at'))
    if um_at is None and transformer_data.get('tensao_at'):
        try:
            tensao_at_f = float(transformer_data.get('tensao_at'))
    ...
    # (Lógica para determinar conexões - igual ao populate_dielectric_fields)
    def determine_connection(conn_key, neutral_volt_key):
        connection_stored = transformer_data.get(conn_key)
        default_conn = 'estrela' if 'at' in conn_key else ('triangulo' if 'bt' in conn_key else None)
        if not connection_stored or not isinstance(connection_stored, str):
    ...
    # Função para obter NBI do neutro
    def get_neutro_nbi(conn_key, neutral_volt_key, neutral_nbi_key):
        ...
        # Ignora estrela sem neutro
        if conn_lower == 'estrela_sem_neutro':
            return None
        # Verifica se é estrela com neutro ou estrela sem especificação
        elif 'estrela' in conn_lower or 'star' in conn_lower or conn_lower.startswith('y'):
            neutral_voltage = transformer_data.get(neutral_volt_key)
            try:
                if neutral_voltage is not None and str(neutral_voltage).strip() != '':
    ...
    # (Lógica para obter NBI/SIL - igual ao populate_dielectric_fields)
    nbi_at = safe_str_output(transformer_data.get('nbi_at'))
    nbi_bt = safe_str_output(transformer_data.get('nbi_bt'))
    nbi_terciario = safe_str_output(transformer_data.get('nbi_terciario'))
    ...
    # Calcular IAC como 1.1 vezes o valor de IA/BIL
    iac_at = None
    if nbi_at:
        try:
            nbi_at_float = float(nbi_at)
            iac_at = f"{nbi_at_float * 1.1:.1f}"
    ...
    if nbi_bt:
        try:
            nbi_bt_float = float(nbi_bt)
            iac_bt = f"{nbi_bt_float * 1.1:.1f}"
        except (ValueError, TypeError):
            iac_bt = None
    ...
    if nbi_terciario:
        try:
            nbi_terciario_float = float(nbi_terciario)
            iac_terciario = f"{nbi_terciario_float * 1.1:.1f}"
        except (ValueError, TypeError):
            iac_terciario = None
    # --- Fim da Lógica Reutilizada ---

    log.info(f"[AUTO FILL] Valores processados: Um=[{um_at},{um_bt},{um_terciario}], Conn=[{conexao_at},{conexao_bt},{conexao_terciario}], NBI=[{nbi_at},{nbi_bt},{nbi_terciario}], SIL=[{sil_at},{sil_bt},{sil_terciario}], Isol={tipo_isolamento}")
    log.info(f"[AUTO FILL] NBI Neutro=[{nbi_neutro_at},{nbi_neutro_bt},{nbi_neutro_terciario}]")
    log.info(f"[AUTO FILL] IAC=[{iac_at},{iac_bt},{iac_terciario}]")
    log.info(f"[AUTO FILL] T.Induzida=[{tensao_induzida_at},{tensao_induzida_bt},{tensao_induzida_terciario}]")
    log.info(f"[AUTO FILL] Valores adicionais: Tipo Trafo={tipo_transformador}, T.Aplicada AT={tensao_aplicada_at}, T.Aplicada BT={tensao_aplicada_bt}, T.Aplicada Ter.={tensao_aplicada_terciario}")

    return (
        um_at, um_bt, um_terciario,
        conexao_at, conexao_bt, conexao_terciario,
        nbi_at, nbi_bt, nbi_terciario,
        sil_at, sil_bt, sil_terciario,
        tipo_isolamento,
        tipo_transformador,
        tensao_aplicada_at,
        tensao_aplicada_bt,
        tensao_aplicada_terciario,
        tensao_induzida_at, tensao_induzida_bt, tensao_induzida_terciario,
        neutro_um_at, neutro_um_bt, neutro_um_terciario,
...
@app.callback(
    [Output('dieletric-analysis-store', 'data'),
     Output('dielectric-save-confirmation', 'children')], # NOVO Output
    [Input("save-dielectric-params-btn", "n_clicks"), # Trigger PRINCIPAL é o botão
     Input('url', 'pathname')], # Trigger secundário para carregar
...
def store_dielectric_analysis_data(
    n_clicks_save, pathname, # Inputs
    um_values, conexoes, neutro_ums, ia_values, ia_neutro_values, im_values, tensao_curta_values,
    tensao_induzida_values, tipo_isolamento, iac_values, transformer_data, # States
    current_store_data):
    ...
    if triggered_id == 'url':
        ...
        log.debug(f"[STORE DATA] Trigger não esperado ou n_clicks={n_clicks_save}. Prevenindo update.")
        raise PreventUpdate
    ...
    if pathname:
        clean_path = normalize_pathname(pathname)
        if clean_path != ROUTE_DIELECTRIC_ANALYSIS:
            log.debug(f"[STORE DATA] Não na página '{ROUTE_DIELECTRIC_ANALYSIS}'. Prevenindo update.")
            raise PreventUpdate

    triggered_input_info = ctx.triggered[0] if ctx.triggered else {}; triggered_input = triggered_input_info.get('prop_id', 'No trigger')
    ...
    verificador = get_verificador_instance()
    if verificador is None: log.error("Verificador não inicializado, salvando dados dielétricos incompletos.")
    ...
    tipo_transformador_geral = transformer_data.get('tipo_transformador') if transformer_data and isinstance(transformer_data, dict) else None

    data_to_store = {
        'parametros': {'enrolamentos': [], 'tipo_transformador': tipo_transformador_geral, 'tipo_isolamento': tipo_isolamento},
    ...
    try:
        ...
        ia_neutro_values = list(ia_neutro_values) + [None] * (num_enrolamentos - len(ia_neutro_values))
        im_values = list(im_values) + [None] * (num_enrolamentos - len(im_values))
        tensao_curta_values = list(tensao_curta_values) + [None] * (num_enrolamentos - len(tensao_curta_values))
        tensao_induzida_values = list(tensao_induzida_values) + [None] * (num_enrolamentos - len(tensao_induzida_values))
        ...
        for i in range(num_enrolamentos):
             um = um_values[i]; conexao = conexoes[i];
             neutro_um = neutro_ums[i] if conexao in ['YN', 'Y'] else None
             ia = ia_values[i]; ia_neutro = ia_neutro_values[i] if conexao == 'YN' else None
             im = im_values[i]; tensao_curta = tensao_curta_values[i]; tensao_induzida = tensao_induzida_values[i]; iac = iac_values[i]

             if um: # Save winding only if Um is defined
                 # Get NBI Neutro value based on index
                 nbi_neutro_value = None
                 if i == 0 and conexao == 'YN' and transformer_data and isinstance(transformer_data, dict):
                     nbi_neutro_value = safe_str_output(transformer_data.get('nbi_neutro_at'))
                 elif i == 1 and conexao == 'YN' and transformer_data and isinstance(transformer_data, dict):
                     nbi_neutro_value = safe_str_output(transformer_data.get('nbi_neutro_bt'))
                 elif i == 2 and conexao == 'YN' and transformer_data and isinstance(transformer_data, dict):
                     nbi_neutro_value = safe_str_output(transformer_data.get('nbi_neutro_terciario'))

                 data_to_store['parametros']['enrolamentos'].append({
                     'nome': nomes[i], 'um': um, 'conexao': conexao, 'neutro_um': neutro_um, 'ia': ia,
                     'ia_neutro': ia_neutro, 'im': im, 'tensao_curta': tensao_curta, 'tensao_induzida': tensao_induzida,
                 ...
                 if verificador:
                     transformer_type_ieee = "power" if tipo_transformador_geral == 'Trifásico' else "distribution" if tipo_transformador_geral else None
                     resultados_enrolamento['espacamentos'] = verificador.get_clearances(um, transformer_type_ieee, ia, im)
                 else: resultados_enrolamento['espacamentos'] = {}
        ...
        if isinstance(data_to_store, dict):
            ...
            if isinstance(parametros, dict):
                ...
                print(f"    enrolamentos (tipo): {type(enrolamentos).__name__}, tamanho: {len(enrolamentos)}")
            ...
            if isinstance(resultados, dict):
                ...
                print(f"    enrolamentos (tipo): {type(enrolamentos_res).__name__}, tamanho: {len(enrolamentos_res)}")
        else:
        ...
        if hasattr(app, 'mcp') and app.mcp is not None:
            ...
            print("[STORE DATA] MCP não disponível. Usando método antigo para salvar dados dielétricos.")
        ...
        print(f"--- Fim [DIELETRIC STORE SAVE DEBUG] ---")
        ...
        if um:
            try:
                ...
                if ti_vals and len(ti_vals) > 0:
                    ...
                    ti_vals_float = [safe_float_convert(v) for v in ti_vals if safe_float_convert(v) is not None]
                    if ti_vals_float:
