...
def populate_dielectric_fields_auto(pathname, dieletric_store_data, transformer_data):
    ...
    nbi_neutro_terciario = get_neutro_nbi(
        "conexao_terciario", "tensao_bucha_neutro_terciario", "nbi_neutro_terciario"
    )
    ...
    log.info(
        f"[AUTO FILL] Valores processados: Um=[{um_at},{um_bt},{um_terciario}], Conn=[{conexao_at},{conexao_bt},{conexao_terciario}], NBI=[{nbi_at},{nbi_bt},{nbi_terciario}], SIL=[{sil_at},{sil_bt},{sil_terciario}], Isol={tipo_isolamento}"
    )
    ...
    log.info(
        f"[AUTO FILL] T.Induzida=[{tensao_induzida_at},{tensao_induzida_bt},{tensao_induzida_terciario}]"
    )
    log.info(
        f"[AUTO FILL] Valores adicionais: Tipo Trafo={tipo_transformador}, T.Aplicada AT={tensao_aplicada_at}, T.Aplicada BT={tensao_aplicada_bt}, T.Aplicada Ter.={tensao_aplicada_terciario}"
    )
