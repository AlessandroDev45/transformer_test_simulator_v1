...
from dash import Input, Output, State, ALL, callback, ctx, html, dcc
...
def register_standards_management_callbacks(app):
    ...
    @app.callback(
        ...
        [Input("url", "pathname"),
         Input("standards-processing-status-store", "data")],
            ...
            if standards_by_status["completed"]:
                ...
                items.append(
                    dbc.Table([
                        ...
                        html.Tbody([
                            html.Tr([
                                html.Td(f"{std.get('organization', '')} {std.get('standard_number', 'N/A')}"),
                                html.Td(std.get('title', '')),
                                html.Td(std.get('year', '')),
                                html.Td([
                                    dbc.Button(
                                        html.I(className="fas fa-trash"),
                                        id={"type": "delete-standard-btn", "index": std["id"]},
                                        color="danger",
                                        size="sm",
                                        className="me-1",
                                        title="Excluir norma"
                            ...
                            ]) for std in sorted(standards_by_status["completed"], key=lambda x: (x.get("organization", ""), x.get("standard_number", "")))
                        ])
                            ...
                            html.Tr([
                                ...
                                html.Td(
                                    dbc.Button(
                                        ...
                                        id={"type": "retry-standard-btn", "index": std["id"]},
                                        color="warning",
                                        size="sm",
    ...
    def process_standard_action(n_clicks, delete_clicks, retry_clicks,
                               contents, filename, title, number, organization,
                               year, categories):
        ...
        if isinstance(trigger_id, dict) and trigger_id.get("type") == "delete-standard-btn":
            ...
            try:
                ...
                if success:
                    ...
                    action_data = {"action": "delete", "id": standard_id, "timestamp": datetime.datetime.now().isoformat()}
                    return status, action_data
                else:
                    status = dbc.Alert("Erro ao excluir norma", color="danger")
                    action_data = {"action": "error", "error": "delete_failed", "timestamp": datetime.datetime.now().isoformat()}
                    return status, action_data

            except Exception as e:
                log.exception(f"Erro ao excluir norma {standard_id}: {e}")
                status = dbc.Alert(f"Erro ao excluir norma: {str(e)}", color="danger")
                action_data = {"action": "error", "error": str(e), "timestamp": datetime.datetime.now().isoformat()}
                return status, action_data
        ...
        # Ação de reprocessamento
            ...
            try:
                ...
                if success:
                    status = dbc.Alert("Norma marcada para reprocessamento. Faça o upload do PDF novamente.", color="warning")
                    action_data = {"action": "retry", "id": standard_id, "timestamp": datetime.datetime.now().isoformat()}
                    return status, action_data
                else:
                    status = dbc.Alert("Erro ao atualizar status da norma", color="danger")
                    action_data = {"action": "error", "error": "update_failed", "timestamp": datetime.datetime.now().isoformat()}
                    return status, action_data
        ...
        # Ação de processamento
            ...
            if not contents:
                ...
                action_data = {"action": "error", "error": "no_file", "timestamp": datetime.datetime.now().isoformat()}
                return status, action_data

            if not filename or not filename.lower().endswith('.pdf'):
                status = dbc.Alert("Apenas arquivos PDF são aceitos", color="danger")
                action_data = {"action": "error", "error": "invalid_file", "timestamp": datetime.datetime.now().isoformat()}
                return status, action_data

            if not title or not title.strip():
                status = dbc.Alert("Título da norma é obrigatório", color="danger")
                action_data = {"action": "error", "error": "missing_title", "timestamp": datetime.datetime.now().isoformat()}
                return status, action_data

            if not number or not number.strip():
                status = dbc.Alert("Número da norma é obrigatório", color="danger")
                action_data = {"action": "error", "error": "missing_number", "timestamp": datetime.datetime.now().isoformat()}
                return status, action_data

            if not organization or not organization.strip():
                status = dbc.Alert("Organização é obrigatória", color="danger")
                action_data = {"action": "error", "error": "missing_organization", "timestamp": datetime.datetime.now().isoformat()}
                return status, action_data

            if not year or not str(year).isdigit():
                status = dbc.Alert("Ano de publicação inválido", color="danger")
                action_data = {"action": "error", "error": "invalid_year", "timestamp": datetime.datetime.now().isoformat()}
                return status, action_data
            ...
            try:
                ...
                try:
                    import sqlite3
                    conn = sqlite3.connect(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'standards.db'))
                    cursor = conn.cursor()
                    ...
                    if result and result[0] == 'processing':
                        status = dbc.Alert("Esta norma já está em processamento. Aguarde a conclusão.", color="warning")
                        action_data = {"action": "error", "error": "already_processing", "timestamp": datetime.datetime.now().isoformat()}
                        return status, action_data
                ...
                script_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "scripts", "process_standard.py")
                ...
                try:
                    ...
                    use_fallback = True
                    log.warning("Não foi possível verificar se Docling está instalado. Usando processador simplificado.")
