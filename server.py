...
try:
    ...
    # Formatar logs
    formatter = logging.Formatter(fmt=config.LOGGING_FORMAT, datefmt=config.LOGGING_DATE_FORMAT)
...
if hasattr(app, "mcp") and app.mcp is not None:
    ...
    if transformer_data:
        log.info(
            f"MCP contém dados do transformador: {len(transformer_data) if isinstance(transformer_data, dict) else 'Não é dict'} campos"
        )
    else:
